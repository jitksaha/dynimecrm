import { a as e } from "./application-yzkKx8Nd.mjs";
import { appendFile as t, copyFile as n, cp as r, mkdir as i, readdir as a, rename as o, rm as s, writeFile as c } from "node:fs/promises";
import { dirname as l, join as u, parse as ee, relative as d, resolve as f } from "node:path";
import { build as p } from "esbuild";
import { assertValidSchema as te, buildSchema as ne, getNamedType as m, isEnumType as h, isInputObjectType as g, isInterfaceType as _, isListType as re, isNamedType as ie, isNonNullType as ae, isObjectType as v, isScalarType as y, isUnionType as b, lexicographicSortSchema as oe, printSchema as se } from "graphql";
import ce from "lodash/camelCase.js";
import { promises as x, readFileSync as le } from "node:fs";
import { homedir as ue } from "node:os";
import de from "prettier/standalone";
import * as fe from "prettier/plugins/graphql";
import * as pe from "prettier/plugins/typescript";
import * as S from "prettier/plugins/estree";
import C from "lodash/uniq.js";
import { fileURLToPath as me } from "node:url";
//#region src/generate/client-wrapper.ts
var he = "// __STRIPPED_DURING_INJECTION_START__", ge = "// __STRIPPED_DURING_INJECTION_END__", w = "// __UPLOAD_FILE_START__", T = "// __UPLOAD_FILE_END__", E = (e) => e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), D = (e, t) => {
	let n = e;
	return n = n.replace(RegExp(`${E(he)}[\\s\\S]*?${E(ge)}\\n?`), ""), n = n.replace("'__TWENTY_DEFAULT_URL__'", t.defaultUrl), n = n.replace(/TwentyGeneratedClient/g, t.apiClientName), t.includeUploadFile ? (n = n.replace(RegExp(`\\s*${E(w)}\\n`), "\n"), n = n.replace(RegExp(`\\s*${E(T)}\\n`), "\n")) : n = n.replace(RegExp(`\\s*${E(w)}[\\s\\S]*?${E(T)}\\n?`), "\n"), `\n// ${t.apiClientName} (auto-injected by twenty-client-sdk)\n${n}`;
}, O = (e) => i(e, { recursive: !0 }), k = async (e) => {
	let t;
	try {
		t = await a(e);
	} catch (t) {
	...
	await Promise.all(t.map((t) => s(u(e, t), {
		recursive: !0,
		force: !0
	})));
}, _e = async (e, t) => {
	try {
		...
		if (n instanceof Error && "code" in n && n.code === "EXDEV") await r(e, t, { recursive: !0 }), await s(e, {
			recursive: !0,
			force: !0
		});
		else throw n;
	...
	let { root: t } = ee(e), n = e.slice(t.length).split(/[\\/]/).filter(Boolean).length, r = f(ue()), i = e === r || r.startsWith(`${e}/`);
	if (e === t || n < 2 || i) throw Error(`Refusing to recursively clear unsafe path: ${e}`);
}, j = async (e, t = !1) => {
	let n = f(...e);
	t && (ve(n), await x.rm(n, {
		recursive: !0,
	...
	let n = f(...e, "..");
	await x.mkdir(n, { recursive: !0 }), await x.writeFile(f(...e), t);
}, N = "./runtime", P = (e) => e ? `${e.name}GenqlSelection` : "", F = (e) => {
	let t = e.config?.endpoint ? `"${e.config.endpoint}"` : "undefined", n = e.config?.fetchImport;
	return `
function(options${t ? "?" : ""}: ClientOptions): Client {
  return createClientOriginal({
	...
	let n = e.getQueryType(), r = e.getMutationType(), i = e.getSubscriptionType(), a = t.config?.fetchImport || "";
	t.addCodeBlock(`
${a}
${be({
		mutationType: r,
  ...
  export type { FieldsSelection } from '${N}'
  export { GenqlError }
  ...
  export const createClient = ${F(t)}

  export const everything = {
    __scalar: true
  }
  `), n && t.addCodeBlock(`
        export type QueryResult<fields extends ${P(n)}> = FieldsSelection<${n.name}, fields>
        export const generateQueryOp: (fields: ${P(n)} & { __name?: string }) => GraphqlOperation = function(fields) {
        return generateGraphqlOperation('query', typeMap.Query!, fields as any)
      }
    `), r && t.addCodeBlock(`
        export type MutationResult<fields extends ${P(r)}> = FieldsSelection<${r.name}, fields>
        export const generateMutationOp: (fields: ${P(r)} & { __name?: string }) => GraphqlOperation = function(fields) {
        return generateGraphqlOperation('mutation', typeMap.Mutation!, fields as any)
      }
    `), i && t.addCodeBlock(`
        export type SubscriptionResult<fields extends ${P(i)}> = FieldsSelection<${i.name}, fields>
        export const generateSubscriptionOp: (fields: ${P(i)} & { __name?: string }) => GraphqlOperation = function(fields) {
        return generateGraphqlOperation('subscription', typeMap.Subscription!, fields as any)
      }
    `);
};
function be({ queryType: e, mutationType: t, subscriptionType: n }) {
	let r = [];
	return e && r.push(P(e), e.name), t && r.push(P(t), t.name), n && r.push(P(n), n.name), r.length > 0 ? `import type {${r.join(",")}} from './schema'` : "";
}
function xe({ queryType: e, mutationType: t, subscriptionType: n }) {
	let r = "";
	return e && (r += `
      query<R extends ${P(e)}>(
...
//#endregion
//#region src/generate/genql/render/common/excludedTypes.ts
var I = [
	"__Schema",
	"__Type",
	"__TypeKind",
	...
	constructor(e, t) {
		this.schema = e, this.config = t, this.codeBlocks = [], this.imports = {}, this.importAliasCounter = 0;
	}
	addCodeBlock(e) {
		e && this.codeBlocks.push(e);
	}
	addImport(e, t, n, r, i) {
		this.config && this.config.output && (e = r ? e : Ce(this.config.output, e)), this.imports[e] || (this.imports[e] = []);
		let a = this.imports[e], o = a.find((e) => t && e.isDefault || !t && e.module === n);
		if (o) return o.alias;
		this.importAliasCounter++;
		let s = i ? void 0 : `a${this.importAliasCounter}`;
		return a.push({
			isDefault: t,
			module: n,
			alias: s
	...
	getImportBlock() {
		let e = [];
		if (Object.keys(this.imports).forEach((t) => {
			let n = this.imports[t].find((e) => e.isDefault), r = this.imports[t].filter((e) => !e.isDefault), i = [];
			n && i.push(n.alias || ""), r.length > 0 && i.push(`{${r.map((e) => e.alias ? `${e.module} as ${e.alias}` : e.module).join(",")}}`), e.push(`import ${i.join(",")} from '${t}'`);
		}), e.length > 0) return e.join("\n");
	}
	async toCode(e, t = !1) {
		let n = [...this.codeBlocks];
		if (e && (e === "typescript" || e === "babel")) {
			let e = this.getImportBlock();
			e && n.unshift(e);
		}
		return e && t ? Se(n.join("\n\n"), e) : e ? n.join("\n\n") : n.join("");
	}
}, R = (e) => {
	let t = [];
	return e.deprecated && t.push(`@deprecated ${e.deprecated.replace(/\s/g, " ")}`), e.text && t.push(...e.text.split("\n")), t.length > 0 ? t.length === 1 ? `\n/** ${t[0]} */\n` : `\n/**\n${t.map((e) => ` * ${e}`).join("\n")}\n */\n` : "";
}, z = (e) => R({ text: e.description }), B = (e) => R({
	deprecated: e.deprecationReason,
	text: e.description
}), V = (e) => R({ text: e.description }), H = (e, t, n, r, i, a = (e) => e) => {
	if (n) if (i) {
		if (ae(e)) return `: ${H(e.ofType, !0, !1, r, i, a)}`;
		{
			let t = H(e, !0, !1, r, i, a);
			return r ? `?: ${t}` : `?: (${t} | null)`;
		}
	} else return `: ${H(e, !1, !1, r, i, a)}`;
	if (ie(e)) {
		let n = e.name;
		y(e) && (n = `Scalars['${n}']`);
		let i = a(n);
		return r ? t ? i : `(${i} | undefined)` : t ? i : `(${i} | null)`;
	}
	if (re(e)) {
		let n = `${H(e.ofType, !1, !1, r, i, a)}[]`;
		return r ? t ? n : `(${n} | undefined)` : t ? n : `(${n} | null)`;
	}
	return H(e.ofType, !0, !1, r, i, a);
}, U = (e, t, n, r = !0, i = void 0) => H(e, !1, r, t, n, i);
//#endregion
//#region src/generate/genql/render/common/support.ts
function W(e) {
	e ||= {};
	let t = {};
...
//#endregion
//#region src/generate/genql/render/requestTypes/inputObjectType.ts
var we = (e, t) => {
	let n = e.getFields();
	t.config?.sortProperties && (n = W(n));
	let r = Object.keys(n).map((e) => {
		let t = n[e];
		return `${V(t)}${t.name}${U(t.type, !1, !0)}`;
	});
	t.addCodeBlock(`${z(e)}export interface ${e.name} {${r.join(",")}}`);
}, Te = "    ", Ee = (e, t) => {
	let n = e.getFields();
	t.config?.sortProperties && (n = W(n));
	let r = Object.keys(n).map((e) => {
		let t = n[e], r = [], i = m(t.type), a = !(h(i) || y(i)), o = t.args.length > 0, s = De(t), c = !s.match(/[^?]:/);
		return o && (a ? r.push(`(${P(i)} & { __args${c ? "?" : ""}: ${s} })`) : r.push(`{ __args: ${s} }`)), c && !a && r.push("boolean | number"), !o && a && r.push(P(i)), `${B(t)}${t.name}?: ${r.join(" | ")}`;
	});
	if (_(e) && t.schema) {
		let n = t.schema.getPossibleTypes(e).map((e) => `on_${e.name}?: ${P(e)}`);
		t.config?.sortProperties && (n = n.sort()), r = r.concat(n);
	}
	r.push("__typename?: boolean | number"), r.push("__scalar?: boolean | number"), r = r.map((e) => e.split("\n").filter(Boolean).map((e) => Te + e).join("\n")), t.addCodeBlock(`${z(e)}export interface ${P(e)}{\n${r.join("\n")}\n}`);
}, De = (e) => `{${e.args.map((e) => `${V(e)}${e.name}${U(e.type, !1, !0)}`).join(", ")}}`, Oe = (e, t) => {
	let n = [...e.getTypes()];
	t.config?.sortProperties && (n = n.sort());
	let r = n.map((e) => `on_${e.name}?:${P(e)}`), i = C(n.map((e) => e.getInterfaces()).flat());
	r.push(...i.map((e) => `on_${e.name}?: ${P(e)}`)), r.push("__typename?: boolean | number"), t.addCodeBlock(`${z(e)}export interface ${P(e)}{\n${r.map((e) => "    " + e).join(",\n")}\n}`);
}, ke = (e, t) => {
	let n = e.getTypeMap();
	t.config?.sortProperties && (n = W(n));
	for (let e in n) {
		...
		let r = n[e];
		(v(r) || _(r)) && Ee(r, t), g(r) && we(r, t), b(r) && Oe(r, t);
	}
	let r = [
		{
			type: e.getQueryType(),
		...
		{
			type: e.getMutationType(),
			name: "MutationGenqlSelection"
		},
		{
			type: e.getSubscriptionType(),
			name: "SubscriptionGenqlSelection"
		}
	].map(Ae).filter(Boolean).join("\n");
	t.addCodeBlock(r);
};
function Ae({ type: e, name: t }) {
	return e && P(e) !== t ? `export type ${t} = ${P(e)}` : "";
}
//#endregion
//#region src/generate/genql/render/responseTypes/enumType.ts
var je = (e, t) => {
	let n = e.getValues().map((e) => `'${e.name}'`);
	t.addCodeBlock(`${z(e)}export type ${e.name} = ${n.join(" | ")}`);
}, Me = "    ", G = (e, t) => {
	let n = e.getFields();
	t.config?.sortProperties && (n = W(n));
	let r = Object.keys(n).map((e) => n[e]);
	if (!t.schema) throw Error("no schema provided");
	let i = v(e) ? [e.name] : t.schema.getPossibleTypes(e).map((e) => e.name), a = r.map((e) => `${B(e)}${e.name}${U(e.type, !0, !0)}`).concat([`__typename: ${i.length > 0 ? i.map((e) => `'${e}'`).join("|") : "string"}`]);
	a = a.map((e) => e.split("\n").filter(Boolean).map((e) => Me + e).join("\n")), t.addCodeBlock(`${z(e)}export interface ${e.name} {\n${a.join("\n")}\n}`);
}, Ne = {
	Int: "number",
	Float: "number",
	String: "string",
	...
	...Ne,
	...t?.config?.scalarTypes || {}
})[e.name] || "any";
//#endregion
//#region src/generate/genql/render/responseTypes/scalarType.ts
function Pe(e, t) {
	let n = "";
	return t.forEach((t) => {
		n += `    ${t.name}: ${K(t, e)},\n`;
...
//#endregion
//#region src/generate/genql/render/responseTypes/unionType.ts
var Fe = (e, t) => {
	let n = e.getTypes().map((e) => e.name);
	t.config?.sortProperties && (n = n.sort()), t.addCodeBlock(`${z(e)}export type ${e.name} = (${n.join(" | ")}) & { __isUnion?: true }`);
}, Ie = (e, t) => {
	if (!t.schema) throw Error("schema is required to render unionType");
	let n = t.schema.getPossibleTypes(e).map((e) => e.name);
	n.length ? t.addCodeBlock(`${z(e)}export type ${e.name} = (${n.join(" | ")}) & { __isUnion?: true }`) : G(e, t);
}, Le = (e, t) => {
	let n = e.getTypeMap();
	t.config?.sortProperties && (n = W(n)), t.addCodeBlock(Pe(t, Object.values(n).filter((e) => y(e))));
	for (let e in n) {
		if (I.includes(e)) continue;
		let r = n[e];
		h(r) && je(r, t), b(r) && Fe(r, t), v(r) && G(r, t), _(r) && Ie(r, t);
	}
	let r = [
		{
			type: e.getQueryType(),
	...
	].map(Re).filter(Boolean).join("\n");
	t.addCodeBlock(r);
};
function Re({ type: e, name: t }) {
	return e && e.name !== t ? `export type ${t} = ${e.name}` : "";
}
//#endregion
//#region src/generate/genql/render/schema/renderSchema.ts
var ze = (e, t) => {
	t.addCodeBlock(se(e));
}, q = (e, t) => `
	...
	for (let r in n) {
		...
		let i = n[r];
		if (b(i)) {
			let e = i.getTypes().map((e) => e.name);
			t.addCodeBlock(q(i.name, e));
		} else if (_(i)) {
			let n = e.getPossibleTypes(i).map((e) => e.name);
			t.addCodeBlock(q(i.name, n));
		} else v(i) && t.addCodeBlock(q(i.name, [i.name]));
	}
};
//#endregion
//#region src/generate/genql/render/typeMap/support.ts
function J(e) {
	return e ? Object.keys(e).length === 0 : !0;
}
//#endregion
//#region src/generate/genql/render/typeMap/objectType.ts
var Y = (e, t) => {
	let n = Object.keys(e.getFields()).reduce((t, n) => {
		let r = e.getFields()[n], i = { type: m(r.type).name };
		t[n] = i;
		let a = r.args || [];
		return a.length > 0 && (i.args = a.reduce((e, t) => {
			let n = t.type.toString(), r = m(t.type).name;
			return e[t.name] = [r], r !== n && e[t.name]?.push(n), e;
		}, {})), t;
	}, {});
	return _(e) && t.schema && t.schema.getPossibleTypes(e).map((e) => {
		J(n) || (n[`on_${e.name}`] = { type: e.name });
	}), J(n) || (n.__typename = { type: "String" }), n;
}, Ve = (e, t) => {
	let n = e.getTypes(), r = n.reduce((e, t) => (e[`on_${t.name}`] = { type: t.name }, e), {});
	return C(n.map((e) => e.getInterfaces()).flat()).forEach((e) => {
		r[`on_${e.name}`] = { type: e.name };
	}), r.__typename = { type: "String" }, r;
}, He = (e, t) => {
	let n = {
		scalars: [],
		types: {}
	};
	Object.keys(e.getTypeMap()).filter((e) => !I.includes(e)).map((t) => e.getTypeMap()[t]).map((e) => {
		v(e) || _(e) || g(e) ? n.types[e.name] = Y(e, t) : b(e) ? n.types[e.name] = Ve(e, t) : (y(e) || h(e)) && (n.scalars.push(e.name), n.types[e.name] = {});
	});
	let r = e.getQueryType();
	r?.name && r?.name !== "Query" && (delete n.types[r.name], n.types.Query = Y(r, t));
	let i = e.getMutationType();
	i?.name && i.name !== "Mutation" && (delete n.types[i.name], n.types.Mutation = Y(i, t));
	let a = e.getSubscriptionType();
	a?.name && a.name !== "Subscription" && (delete n.types[a.name], n.types.Subscription = Y(a, t)), t.addCodeBlock(JSON.stringify(Ue(n), null, 4));
};
function Ue(e) {
	let t = Object.assign({}, ...Object.keys(e.types).map((e, t) => ({ [e]: t })));
	return {
		scalars: e.scalars.map((e) => t[e]),
		types: Object.assign({}, ...Object.keys(e.types || {}).map((n) => {
			let r = e.types[n] || {}, i = Object.assign({}, ...Object.keys(r).map((e) => {
				let n = r[e];
				if (!n) throw Error("no content in field " + e);
				let [i, a] = [n.type, n.args], o = [i ? t[i] : -1];
				return a && (o[1] = Object.assign({}, ...Object.keys(a || {}).map((e) => {
					let n = a?.[e];
					if (!n) throw Error("replaceTypeNamesWithIndexes: no arg for " + e);
					return { [e]: [t[n[0]], ...n.slice(1)] };
				}))), { [e]: o };
			}));
			return { [n]: { ...i } };
		}))
...
var ot = async (e) => {
	...
	try {
		...
		let i = D(a, {
			...
			defaultUrl: `\`\${process.env.${e}}/graphql\``,
			includeUploadFile: !0
		});
		await t(u(o, "index.ts"), i), await A(r), await _e(o, r), await lt(r);
	} catch (e) {
		throw await A(o), e;
	}
}, ct = async ({ packageRoot: e, schema: t }) => {
	let r = u(e, "dist", Q);
	await $({
		schema: t,
		outputPath: r
	}), await n(u(r, "index.mjs"), u(e, "dist", "core.mjs")), await n(u(r, "index.cjs"), u(e, "dist", "core.cjs"));
}, lt = async (e) => {
	let t = u(e, "index.ts"), n = u(e, "index.mjs");
	await p({
		entryPoints: [t],
		outfile: n,
		bundle: !0,
		...
		sourcemap: !1,
		minify: !1
	}), await c(u(e, "package.json"), JSON.stringify({
		type: "module",
		main: "index.mjs",
		module: "index.mjs"
	}, null, 2));
	...
	JSON: "Record<string, unknown>",
	UUID: "string"
}, dt = async ({ schema: n, outputPath: r, clientWrapperTemplateSource: i }) => {
	let a = i ?? "import type { TwentyClientRunAs } from '../shared/twenty-client-run-as.type';\n\n// Ambient type stubs for the genql-generated code this template gets\n// injected into. They enable full typecheck/lint on this file.\n// __STRIPPED_DURING_INJECTION_START__\ntype QueryGenqlSelection = Record<string, unknown>;\ntype MutationGenqlSelection = Record<string, unknown>;\ntype GraphqlOperation = Record<string, unknown>;\n\ntype ClientOptions = Omit<RequestInit, 'body' | 'headers'> & {\n  url?: string;\n  headers?: HeadersInit | (() => HeadersInit | Promise<HeadersInit>);\n  fetcher?: (\n    operation: GraphqlOperation | GraphqlOperation[],\n  ) => Promise<unknown>;\n  fetch?: typeof globalThis.fetch;\n  batch?: unknown;\n};\n\ntype Client = {\n  query: (\n    request: QueryGenqlSelection & { __name?: string },\n  ) => Promise<unknown>;\n  mutation: (\n    request: MutationGenqlSelection & { __name?: string },\n  ) => Promise<unknown>;\n};\n\ndeclare function createClient(options: ClientOptions): Client;\n\ndeclare class GenqlError extends Error {\n  constructor(errors: unknown, data: unknown);\n}\n// __STRIPPED_DURING_INJECTION_END__\n\nconst APP_ACCESS_TOKEN_ENV_KEY = 'TWENTY_APP_ACCESS_TOKEN';\nconst APP_APPLICATION_ACCESS_TOKEN_ENV_KEY =\n  'TWENTY_APP_APPLICATION_ACCESS_TOKEN';\nconst API_KEY_ENV_KEY = 'TWENTY_API_KEY';\n\nexport type TwentyGeneratedClientOptions = ClientOptions & {\n  runAs?: TwentyClientRunAs;\n};\n\ntype ProcessEnvironment = Record<string, string | undefined>;\n\ntype GraphqlErrorPayloadEntry = {\n  message?: string;\n  extensions?: { code?: string };\n};\n\ntype GraphqlResponsePayload = {\n  data?: Record<string, unknown>;\n  errors?: GraphqlErrorPayloadEntry[];\n};\n\ntype GraphqlResponse = {\n  status: number;\n  statusText: string;\n  payload: GraphqlResponsePayload | null;\n  rawBody: string;\n};\n\nconst getProcessEnvironment = (): ProcessEnvironment => {\n  const processObject = (\n    globalThis as { process?: { env?: ProcessEnvironment } }\n  ).process;\n\n  return processObject?.env ?? {};\n};\n\nconst getTokenFromAuthorizationHeader = (\n  authorizationHeader: string | undefined,\n): string | null => {\n  if (typeof authorizationHeader !== 'string') {\n    return null;\n  }\n\n  const trimmedAuthorizationHeader = authorizationHeader.trim();\n\n  if (trimmedAuthorizationHeader.length === 0) {\n    return null;\n  }\n\n  if (trimmedAuthorizationHeader === 'Bearer') {\n    return null;\n  }\n\n  if (trimmedAuthorizationHeader.startsWith('Bearer ')) {\n    return trimmedAuthorizationHeader.slice('Bearer '.length).trim();\n  }\n\n  return trimmedAuthorizationHeader;\n};\n\nconst getTokenFromHeaders = (\n  headers: HeadersInit | undefined,\n): string | null => {\n  if (!headers) {\n    return null;\n  }\n\n  if (headers instanceof Headers) {\n    return getTokenFromAuthorizationHeader(\n      headers.get('Authorization') ?? undefined,\n    );\n  }\n\n  if (Array.isArray(headers)) {\n    const matchedAuthorizationHeader = headers.find(\n      ([headerName]) => headerName.toLowerCase() === 'authorization',\n    );\n\n    return getTokenFromAuthorizationHeader(matchedAuthorizationHeader?.[1]);\n  }\n\n  const headersRecord = headers as Record<string, string | undefined>;\n\n  return getTokenFromAuthorizationHeader(\n    headersRecord.Authorization ?? headersRecord.authorization,\n  );\n};\n\nconst hasAuthenticationErrorInGraphqlPayload = (\n  payload: GraphqlResponsePayload | null,\n): boolean => {\n  if (!payload?.errors) {\n    return false;\n  }\n\n  return payload.errors.some((graphqlError) => {\n    return (\n      graphqlError.extensions?.code === 'UNAUTHENTICATED' ||\n      graphqlError.message?.toLowerCase() === 'unauthorized'\n    );\n  });\n};\n\nconst defaultOptions: TwentyGeneratedClientOptions = {\n  url: '__TWENTY_DEFAULT_URL__',\n  headers: {\n    'Content-Type': 'application/json',\n  },\n};\n\nexport class TwentyGeneratedClient {\n  private client: Client;\n  private url: string;\n  private requestOptions: RequestInit;\n  private headers: HeadersInit | (() => HeadersInit | Promise<HeadersInit>);\n  private fetchImplementation: typeof globalThis.fetch | null;\n  private authorizationToken: string | null;\n  private refreshAccessTokenPromise: Promise<string | null> | null = null;\n\n  constructor(options?: TwentyGeneratedClientOptions) {\n    const merged: TwentyGeneratedClientOptions = {\n      ...defaultOptions,\n      ...options,\n    };\n\n    const {\n      url,\n      headers,\n      fetch: customFetchImplementation,\n      fetcher: _fetcher,\n      batch: _batch,\n      runAs,\n      ...requestOptions\n    } = merged;\n\n    this.url = url ?? '';\n    this.requestOptions = requestOptions;\n    this.headers = headers ?? {};\n    this.fetchImplementation =\n      customFetchImplementation ?? globalThis.fetch ?? null;\n\n    const processEnvironment = getProcessEnvironment();\n    const tokenFromHeaders = getTokenFromHeaders(\n      typeof headers === 'function' ? undefined : headers,\n    );\n\n    // Priority: explicit header > the token for the requested access > api key\n    // (legacy).\n    this.authorizationToken =\n      tokenFromHeaders ??\n      processEnvironment[\n        runAs === 'application'\n          ? APP_APPLICATION_ACCESS_TOKEN_ENV_KEY\n          : APP_ACCESS_TOKEN_ENV_KEY\n      ] ??\n      processEnvironment[API_KEY_ENV_KEY] ??\n      null;\n\n    this.client = createClient({\n      ...merged,\n      headers: undefined,\n      fetcher: async (operation) =>\n        this.executeGraphqlRequestWithOptionalRefresh({\n          operation,\n        }),\n    });\n  }\n\n  query<R extends QueryGenqlSelection>(request: R & { __name?: string }) {\n    return this.client.query(request);\n  }\n\n  mutation<R extends MutationGenqlSelection>(request: R & { __name?: string }) {\n    return this.client.mutation(request);\n  }\n\n  // __UPLOAD_FILE_START__\n  async uploadFile(\n    fileBuffer: Buffer,\n    filename: string,\n    contentType: string = 'application/octet-stream',\n    fieldMetadataUniversalIdentifier: string,\n  ): Promise<{\n    id: string;\n    path: string;\n    size: number;\n    createdAt: string;\n    url: string;\n  }> {\n    const form = new FormData();\n\n    form.append(\n      'operations',\n      JSON.stringify({\n        query: `mutation UploadFilesFieldFileByUniversalIdentifier($file: Upload!, $fieldMetadataUniversalIdentifier: String!) {\n        uploadFilesFieldFileByUniversalIdentifier(file: $file, fieldMetadataUniversalIdentifier: $fieldMetadataUniversalIdentifier) { id path size createdAt url }\n      }`,\n        variables: {\n          file: null,\n          fieldMetadataUniversalIdentifier,\n        },\n      }),\n    );\n    form.append('map', JSON.stringify({ '0': ['variables.file'] }));\n    form.append(\n      '0',\n      new Blob([fileBuffer as BlobPart], { type: contentType }),\n      filename,\n    );\n\n    const result = await this.executeGraphqlRequestWithOptionalRefresh({\n      operation: form,\n      headers: {},\n      requestInit: {\n        method: 'POST',\n      },\n    });\n\n    if (result.errors) {\n      throw new GenqlError(result.errors, result.data);\n    }\n\n    const data = result.data as Record<string, unknown>;\n\n    return data.uploadFilesFieldFileByUniversalIdentifier as {\n      id: string;\n      path: string;\n      size: number;\n      createdAt: string;\n      url: string;\n    };\n  }\n  // __UPLOAD_FILE_END__\n\n  private async executeGraphqlRequestWithOptionalRefresh({\n    operation,\n    headers,\n    requestInit,\n  }: {\n    operation: GraphqlOperation | GraphqlOperation[] | FormData;\n    headers?: HeadersInit;\n    requestInit?: RequestInit;\n  }) {\n    const firstResponse = await this.executeGraphqlRequest({\n      operation,\n      headers,\n      requestInit,\n      token: this.authorizationToken,\n    });\n\n    if (this.shouldRefreshToken(firstResponse)) {\n      const refreshedAccessToken = await this.requestRefreshedAccessToken();\n\n      if (refreshedAccessToken) {\n        const retryResponse = await this.executeGraphqlRequest({\n          operation,\n          headers,\n          requestInit,\n          token: refreshedAccessToken,\n        });\n\n        return this.assertResponseIsSuccessful(retryResponse);\n      }\n    }\n\n    return this.assertResponseIsSuccessful(firstResponse);\n  }\n\n  private async executeGraphqlRequest({\n    operation,\n    headers,\n    requestInit,\n    token,\n  }: {\n    operation: GraphqlOperation | GraphqlOperation[] | FormData;\n    headers?: HeadersInit;\n    requestInit?: RequestInit;\n    token: string | null;\n  }): Promise<GraphqlResponse> {\n    if (!this.fetchImplementation) {\n      throw new Error(\n        'Global `fetch` function is not available, ' +\n          'pass a fetch implementation to the Twenty client',\n      );\n    }\n\n    const resolvedHeaders = await this.resolveHeaders();\n    const requestHeaders = new Headers(resolvedHeaders);\n\n    if (headers) {\n      new Headers(headers).forEach((value, key) =>\n        requestHeaders.set(key, value),\n      );\n    }\n\n    if (operation instanceof FormData) {\n      requestHeaders.delete('Content-Type');\n    } else {\n      requestHeaders.set('Content-Type', 'application/json');\n    }\n\n    if (token) {\n      requestHeaders.set('Authorization', `Bearer ${token}`);\n    } else {\n      requestHeaders.delete('Authorization');\n    }\n\n    const response = await this.fetchImplementation.call(globalThis, this.url, {\n      ...this.requestOptions,\n      ...requestInit,\n      method: requestInit?.method ?? 'POST',\n      headers: requestHeaders,\n      body:\n        operation instanceof FormData ? operation : JSON.stringify(operation),\n    });\n\n    const rawBody = await response.text();\n    let payload: GraphqlResponsePayload | null = null;\n\n    if (rawBody.trim().length > 0) {\n      try {\n        payload = JSON.parse(rawBody) as GraphqlResponsePayload;\n      } catch {\n        payload = null;\n      }\n    }\n\n    return {\n      status: response.status,\n      statusText: response.statusText,\n      payload,\n      rawBody,\n    };\n  }\n\n  private async resolveHeaders(): Promise<HeadersInit> {\n    if (typeof this.headers === 'function') {\n      return (await this.headers()) ?? {};\n    }\n\n    return this.headers ?? {};\n  }\n\n  private shouldRefreshToken(response: GraphqlResponse): boolean {\n    if (response.status === 401) {\n      return true;\n    }\n\n    return hasAuthenticationErrorInGraphqlPayload(response.payload);\n  }\n\n  private assertResponseIsSuccessful(response: GraphqlResponse) {\n    if (response.status < 200 || response.status >= 300) {\n      throw new Error(`${response.statusText}: ${response.rawBody}`);\n    }\n\n    if (response.payload === null) {\n      throw new Error('Invalid JSON response');\n    }\n\n    return response.payload;\n  }\n\n  private async requestRefreshedAccessToken(): Promise<string | null> {\n    const refreshAccessTokenFunction = (\n      globalThis as {\n        frontComponentHostCommunicationApi?: {\n          requestAccessTokenRefresh?: () => Promise<string>;\n        };\n      }\n    ).frontComponentHostCommunicationApi?.requestAccessTokenRefresh;\n\n    if (typeof refreshAccessTokenFunction !== 'function') {\n      return null;\n    }\n\n    if (!this.refreshAccessTokenPromise) {\n      this.refreshAccessTokenPromise = refreshAccessTokenFunction()\n        .then((refreshedAccessToken) => {\n          if (\n            typeof refreshedAccessToken !== 'string' ||\n            refreshedAccessToken.length === 0\n          ) {\n            return null;\n          }\n\n          this.setAuthorizationToken(refreshedAccessToken);\n\n          return refreshedAccessToken;\n        })\n        .catch((refreshError: unknown) => {\n          console.error('Twenty client: token refresh failed', refreshError);\n\n          return null;\n        })\n        .finally(() => {\n          this.refreshAccessTokenPromise = null;\n        });\n    }\n\n    return this.refreshAccessTokenPromise;\n  }\n\n  private setAuthorizationToken(token: string) {\n    this.authorizationToken = token;\n\n    const processEnvironment = getProcessEnvironment();\n\n    processEnvironment[APP_ACCESS_TOKEN_ENV_KEY] = token;\n  }\n}\n";
	await O(r), await k(r), await Z({
		schema: n,
		output: r,
		scalarTypes: {
			...ut,
	...
	let o = D(a, {
		...
		defaultUrl: `\`\${process.env.${e}}/metadata\``,
		includeUploadFile: !0
	});
	await t(u(r, "index.ts"), o);
};
//#endregion
export { Q as GENERATED_CORE_DIR, $ as generateCoreClientFromSchema, dt as generateMetadataClient, ct as replaceCoreClient };
