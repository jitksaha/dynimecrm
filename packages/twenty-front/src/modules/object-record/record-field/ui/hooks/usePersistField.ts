  ...
  const persistField = useCallback(
    async ({
      ...
      if (isValuePersistable) {
        ...
        try {
          const objectName = objectMetadataItem?.nameSingular ?? 'record';
          const storageKey = `dcrm_draft_${objectName}_${recordId}`;
          const existingDraft = JSON.parse(
            localStorage.getItem(storageKey) || '{}',
          );
          existingDraft[fieldName] = valueToPersist;
          existingDraft._lastSaved = new Date().toISOString();
          localStorage.setItem(storageKey, JSON.stringify(existingDraft));
          localStorage.setItem(
            'dcrm_last_active_draft',
            JSON.stringify({
              recordId,
              objectName,
