...
let ClickHouseService = class ClickHouseService {
    ...
    async insert(table, values, options = {}) {
        try {
            const client = options.clientId ? await this.connectToClient(options.clientId) : this.mainClient;
            if (!client) {
                return {
                    success: false,
                    error: new Error(`No ClickHouse client available${options.clientId ? ` for client ${options.clientId}` : ''}`)
                };
            }
            ...
            return {
                success: false,
                error: err instanceof Error ? err : Object.assign(new Error(String(err)), {
                    cause: err
                })
            };
        }
    }
    async select(query, // oxlint-disable-next-line typescript/no-explicit-any
    params, clientId) {
        try {
    ...
    async executeCommand(query, // oxlint-disable-next-line typescript/no-explicit-any
    params, clientId) {
        try {
            const client = clientId ? await this.connectToClient(clientId) : this.mainClient;
            if (!client) {
            ...
            await client.insert({
                ...
                clickhouse_settings: {
                    ...
                    ...options.asyncInsertBusyTimeoutMaxMs !== undefined ? {
                        async_insert_busy_timeout_max_ms: options.asyncInsertBusyTimeoutMaxMs
                    } : {},
