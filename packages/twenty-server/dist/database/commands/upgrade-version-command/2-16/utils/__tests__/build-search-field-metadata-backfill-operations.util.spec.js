...
const CUSTOM_APPLICATION_UNIVERSAL_IDENTIFIER = 'c0c1c2c3-c4c5-4000-8000-000000000001';
const CUSTOM_APPLICATION_ID = 'custom-application-id';
const buildUniversalIdentifiersByApplicationId = (flatObjectMetadatas)=>flatObjectMetadatas.reduce((universalIdentifiersByApplicationId, flatObjectMetadata)=>{
        const applicationUniversalIdentifiers = universalIdentifiersByApplicationId[flatObjectMetadata.applicationId] ?? [];
        applicationUniversalIdentifiers.push(flatObjectMetadata.universalIdentifier);
        universalIdentifiersByApplicationId[flatObjectMetadata.applicationId] = applicationUniversalIdentifiers;
...
const buildFlatObjectMetadataMaps = (flatObjectMetadatas)=>({
        ...
        universalIdentifiersByApplicationId: buildUniversalIdentifiersByApplicationId(flatObjectMetadatas)
    });
...
const buildSearchFieldMetadata = ({ id, universalIdentifier, objectMetadataId, fieldMetadataId, objectMetadataUniversalIdentifier, fieldMetadataUniversalIdentifier, applicationUniversalIdentifier, position = 0, tsVectorFieldMetadataId = 'ts-vector-field-metadata-id', tsVectorFieldMetadataUniversalIdentifier = 'ts-vector-field-metadata-universal-identifier' })=>{
...
const buildSearchVectorField = ({ objectMetadataId, objectMetadataUniversalIdentifier, applicationUniversalIdentifier })=>(0, _getflatfieldmetadatamock.getFlatFieldMetadataMock)({
...
const buildCustomObjectFixture = ()=>{
    ...
    const nameDescriptionField = (0, _getflatfieldmetadatamock.getFlatFieldMetadataMock)({
...
describe('buildSearchFieldMetadataBackfillOperations', ()=>{
    it('selects only the name field for a custom object, ignoring a prefix-overlapping field name', ()=>{
        const { customObject, nameField, nameDescriptionField, searchVectorField } = buildCustomObjectFixture();
        ...
        const customApplicationRows = flatSearchFieldMetadatasToCreateByApplicationUniversalIdentifier[CUSTOM_APPLICATION_UNIVERSAL_IDENTIFIER];
        expect(customApplicationRows).toHaveLength(1);
        expect(customApplicationRows?.[0].fieldMetadataUniversalIdentifier).toBe(nameField.universalIdentifier);
        expect(customApplicationRows?.[0].objectMetadataUniversalIdentifier).toBe(customObject.universalIdentifier);
        // The row points at the object's searchVector field via the new FK.
        expect(customApplicationRows?.[0].tsVectorFieldMetadataUniversalIdentifier).toBe(searchVectorField.universalIdentifier);
        ...
        expect(customApplicationRows?.some((row)=>row.fieldMetadataUniversalIdentifier === nameDescriptionField.universalIdentifier)).toBe(false);
    });
    it('creates no row for a searchable custom object that has no name field (junction object whose label identifier is a UUID id field)', ()=>{
        ...
        const idField = (0, _getflatfieldmetadatamock.getFlatFieldMetadataMock)({
            id: idFieldId,
        ...
        const junctionObject = (0, _getflatobjectmetadatamock.getFlatObjectMetadataMock)({
            id: junctionObjectId,
    ...
    it('groups rows per application: standard-object rows under the standard application, custom-object rows under the custom application', ()=>{
        ...
        const standardField = (0, _getflatfieldmetadatamock.getFlatFieldMetadataMock)({
            id: standardFieldId,
        ...
        const standardObject = (0, _getflatobjectmetadatamock.getStandardFlatObjectMetadataMock)({
            id: standardObjectId,
        ...
        expect(Object.keys(flatSearchFieldMetadatasToCreateByApplicationUniversalIdentifier).sort()).toEqual([
            CUSTOM_APPLICATION_UNIVERSAL_IDENTIFIER,
            _application.TWENTY_STANDARD_APPLICATION_UNIVERSAL_IDENTIFIER
        ].sort());
        const standardApplicationRows = flatSearchFieldMetadatasToCreateByApplicationUniversalIdentifier[_application.TWENTY_STANDARD_APPLICATION_UNIVERSAL_IDENTIFIER];
        expect(standardApplicationRows).toHaveLength(1);
        expect(standardApplicationRows?.[0].fieldMetadataUniversalIdentifier).toBe(standardFieldUniversalIdentifier);
        expect(standardApplicationRows?.[0].tsVectorFieldMetadataUniversalIdentifier).toBe(standardSearchVectorField.universalIdentifier);
        const customApplicationRows = flatSearchFieldMetadatasToCreateByApplicationUniversalIdentifier[CUSTOM_APPLICATION_UNIVERSAL_IDENTIFIER];
        expect(customApplicationRows).toHaveLength(1);
        expect(customApplicationRows?.[0].fieldMetadataUniversalIdentifier).toBe(nameField.universalIdentifier);
        expect(customApplicationRows?.[0].tsVectorFieldMetadataUniversalIdentifier).toBe(searchVectorField.universalIdentifier);
    ...
    it('selects only the standard-set field on a standard object, never a prefix-overlapping sibling (phone vs phoneNumber)', ()=>{
        ...
        const phoneField = (0, _getflatfieldmetadatamock.getFlatFieldMetadataMock)({
            id: phoneFieldId,
        ...
        const phoneNumberField = (0, _getflatfieldmetadatamock.getFlatFieldMetadataMock)({
        ...
        const standardApplicationRows = flatSearchFieldMetadatasToCreateByApplicationUniversalIdentifier[_application.TWENTY_STANDARD_APPLICATION_UNIVERSAL_IDENTIFIER];
        expect(standardApplicationRows).toHaveLength(1);
        expect(standardApplicationRows?.[0].fieldMetadataUniversalIdentifier).toBe(phoneFieldUniversalIdentifier);
        // The prefix-overlapping `phoneNumber` field is never selected.
        expect(standardApplicationRows?.some((row)=>row.fieldMetadataUniversalIdentifier === phoneNumberFieldUniversalIdentifier)).toBe(false);
    });
    it('carries the position from the standard maps row onto the backfilled standard row', ()=>{
        ...
        const emailsField = (0, _getflatfieldmetadatamock.getFlatFieldMetadataMock)({
            id: emailsFieldId,
    ...
    it('is a no-op when the searchFieldMetadata rows already exist (idempotent)', ()=>{
        const { customObject, nameField, nameDescriptionField } = buildCustomObjectFixture();
    ...
    it('skips a custom object whose name field is not a searchable type', ()=>{
        ...
        const relationNameField = (0, _getflatfieldmetadatamock.getFlatFieldMetadataMock)({
        ...
        const customObject = (0, _getflatobjectmetadatamock.getFlatObjectMetadataMock)({
            ...
            fieldUniversalIdentifiers: [
                relationNameField.universalIdentifier
            ]
        });
        const { flatSearchFieldMetadatasToCreateByApplicationUniversalIdentifier } = (0, _buildsearchfieldmetadatabackfilloperationsutil.buildSearchFieldMetadataBackfillOperations)({
            flatObjectMetadataMaps: buildFlatObjectMetadataMaps([
    ...
    it('skips a searchable object owned by a third-party application (only the workspace custom application is backfilled)', ()=>{
        ...
        const thirdPartyApplicationUniversalIdentifier = 'third-party-application-uid';
        const { customObject, nameField, nameDescriptionField } = buildCustomObjectFixture();
