...
Object.defineProperty(exports, "BackfillMissingStandardSkillsCommand", {
    enumerable: true,
    get: function() {
        return BackfillMissingStandardSkillsCommand;
    }
});
...
let BackfillMissingStandardSkillsCommand = class BackfillMissingStandardSkillsCommand extends _provisionedworkspacecommandrunner.ProvisionedWorkspaceCommandRunner {
    async runOnWorkspace({ workspaceId, options }) {
        ...
        this.logger.log(`Found ${skillsToCreate.length} missing standard skill(s) for workspace ${workspaceId}: ${skillsToCreate.map((skill)=>skill.name).join(', ')}`);
        if (isDryRun) {
...
BackfillMissingStandardSkillsCommand = _ts_decorate([
    (0, _registeredworkspacecommanddecorator.RegisteredWorkspaceCommand)('2.27.0', 1785499350000),
    (0, _nestcommander.Command)({
        name: 'upgrade:2-27:backfill-standard-skills',
        description: 'Backfill standard skills missing from existing workspaces, such as the roles skill'
    }),
...
//# sourceMappingURL=2-27-workspace-command-1785499350000-backfill-standard-skills.command.js.map