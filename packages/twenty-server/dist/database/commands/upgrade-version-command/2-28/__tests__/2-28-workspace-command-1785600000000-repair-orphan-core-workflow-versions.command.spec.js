...
const isDelete = (sql)=>typeof sql === 'string' && sql.trimStart().startsWith('DELETE');
const setup = ({ total = 0, active = 0, objectMissing = false, deleteFails = false } = {})=>{
    const query = jest.fn(async (...args)=>{
        const sql = args[0];
        if (isDelete(sql)) {
    ...
    const count = objectMissing ? jest.fn().mockRejectedValue(new _twentyormexception.TwentyOrmException('Object "workflowVersion" does not exist in this workspace', _twentyormexception.TwentyOrmExceptionCode.UNKNOWN_OBJECT)) : jest.fn().mockResolvedValue(0);
    const dataSource = {
        createQueryRunner: ()=>queryRunner
    };
    const workspaceOrmManager = {
        executeInWorkspaceContext: (fn)=>fn(),
        getRepository: ()=>({
                count
            })
    };
    const recompute = jest.fn();
    const command = new _228workspacecommand1785600000000repairorphancoreworkflowversionscommand.RepairOrphanCoreWorkflowVersionsCommand({}, {
        invalidateAndRecompute: recompute
    }, workspaceOrmManager);
    const run = (dryRun = false)=>command.runOnWorkspace({
...
describe('RepairOrphanCoreWorkflowVersionsCommand', ()=>{
    ...
    it('deletes orphans with an atomic NOT EXISTS and recomputes the trigger map', async ()=>{
        const { run, query, recompute } = setup({
            total: 2,
            active: 1
        });
        await run();
...
//# sourceMappingURL=2-28-workspace-command-1785600000000-repair-orphan-core-workflow-versions.command.spec.js.map