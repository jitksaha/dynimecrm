...
describe('AddTimelineActivityTypeSnapshotCommand', ()=>{
    it('repairs rolling-upgrade rows while compatibility fields stay nullable', async ()=>{
        ...
        const getOrRecompute = jest.fn().mockResolvedValueOnce({
            ...
            flatTimelineActivityTypeMaps: {
                byUniversalIdentifier: {
                    linked: {
                        ...
                        universalIdentifier: '00000000-0000-4000-8000-000000000009',
                        name: 'recordLinked',
                        label: 'was linked by',
                        action: 'linked',
                        icon: null,
        ...
        const command = new _234workspacecommand1787471608319addtimelineactivitytypesnapshotcommand.AddTimelineActivityTypeSnapshotCommand({}, {
        ...
        expect(query.mock.calls[0][0]).toContain('WHERE "timelineActivityTypeId" IS NULL');
        expect(query.mock.calls[1][0]).toContain('WHERE timeline_activity."timelineActivityTypeSnapshot" IS NULL');
        const createOperation = validateBuildAndRunWorkspaceMigration.mock.calls[0][0].allFlatEntityOperationByMetadataName.fieldMetadata;
    ...
    it('uses the last updated identifier to keyset large snapshot backfills', async ()=>{
        const lastTimelineActivityId = 'ffffffff-ffff-4fff-8fff-ffffffffffff';
        ...
        const command = new _234workspacecommand1787471608319addtimelineactivitytypesnapshotcommand.AddTimelineActivityTypeSnapshotCommand({}, {
            ...
            getOrRecompute: jest.fn().mockResolvedValueOnce({
                flatObjectMetadataMaps: {
                    byUniversalIdentifier: {
                        [_metadata.STANDARD_OBJECTS.timelineActivity.universalIdentifier]: {
                            universalIdentifier: _metadata.STANDARD_OBJECTS.timelineActivity.universalIdentifier
                ...
                flatFieldMetadataMaps: {
                    byUniversalIdentifier: {
                        [_metadata.STANDARD_OBJECTS.timelineActivity.fields.timelineActivityTypeSnapshot.universalIdentifier]: {}
                    }
                }
            }).mockResolvedValueOnce({
                flatTimelineActivityTypeMaps: {
        ...
        expect(query.mock.calls[2][1]).toEqual([
            lastTimelineActivityId,
            5000
        ]);
    });
});

//# sourceMappingURL=2-34-workspace-command-1787471608319-add-timeline-activity-type-snapshot.command.spec.js.map