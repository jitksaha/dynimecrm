...
const TARGET_FIELD_UNIVERSAL_IDENTIFIER = '00000000-0000-4000-8000-000000000006';
...
const TARGET_INDEX_UNIVERSAL_IDENTIFIER = '00000000-0000-4000-8000-000000000009';
const TARGET_OBJECT_UNIVERSAL_IDENTIFIER = '00000000-0000-4000-8000-000000000004';
const buildCommand = ({ targetFieldName = 'targetPhoneNumber', columnNames = [
    'targetPhoneNumberId'
], migrationResult = {
    status: 'success'
} } = {})=>{
    const validateBuildAndRunLegacyWorkspaceMigration = jest.fn().mockResolvedValue(migrationResult);
    const command = new _235workspacecommand1787641226000repairtimelineactivitytargetfieldnamescommand.RepairTimelineActivityTargetFieldNamesCommand({}, {
                ...
                byUniversalIdentifier: {
                    [TARGET_INDEX_UNIVERSAL_IDENTIFIER]: {
                        ...
                        universalFlatIndexFieldMetadatas: [
                            {
                                fieldMetadataUniversalIdentifier: TARGET_FIELD_UNIVERSAL_IDENTIFIER,
                                order: 0,
                                subFieldName: null
                            }
                        ]
    ...
    const dataSource = {
        query: jest.fn().mockResolvedValue(columnNames.map((name)=>({
                column_name: name
            })))
    };
    return {
...
const runCommand = ({ command, dataSource }, { dryRun = false, withDataSource = true } = {})=>command.runOnWorkspace({
...
describe('RepairTimelineActivityTargetFieldNamesCommand', ()=>{
    it('submits the renamed field and its recomputed index in one migration', async ()=>{
        ...
        expect(context.validateBuildAndRunLegacyWorkspaceMigration).toHaveBeenCalledWith(expect.objectContaining({
            isSystemBuild: true,
            workspaceId: WORKSPACE_ID,
            applicationUniversalIdentifier: APPLICATION_UNIVERSAL_IDENTIFIER,
    ...
    it('does nothing when the target field already matches the object name', async ()=>{
        ...
        await runCommand(context);
        expect(context.validateBuildAndRunLegacyWorkspaceMigration).not.toHaveBeenCalled();
    });
    it('reports manual repair and runs no migration when the column was already renamed', async ()=>{
        ...
        await runCommand(context);
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('MANUAL REPAIR REQUIRED'));
        expect(context.validateBuildAndRunLegacyWorkspaceMigration).not.toHaveBeenCalled();
    });
    it('skips the workspace when no data source is available to verify columns', async ()=>{
        ...
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('no data source'));
        expect(context.validateBuildAndRunLegacyWorkspaceMigration).not.toHaveBeenCalled();
    });
    ...
    it('fails when the workspace migration is rejected', async ()=>{
        ...
        await expect(runCommand(context)).rejects.toThrow('Failed to repair timeline activity target fields');
    });
});

//# sourceMappingURL=2-35-workspace-command-1787641226000-repair-timeline-activity-target-field-names.command.spec.js.map