...
const _buildtimelineactivitytargetfieldrepairsutil = require("../build-timeline-activity-target-field-repairs.util");
const TIMELINE_ACTIVITY_OBJECT_ID = 'timeline-activity-object-id';
const TARGET_MORPH_ID = _metadata.STANDARD_OBJECTS.timelineActivity.morphIds.targetMorphId.morphId;
const buildArgs = ({ targetObjectNameSingularByKey, fields, indexedFieldNamesByIndexKey = {}, orphanedIndexIds = [], columnNames })=>{
    const indexKeys = Object.keys(indexedFieldNamesByIndexKey);
    const entries = (record)=>Object.entries(record);
    return {
        existingColumnNames: new Set(columnNames ?? fields.map(({ name })=>`${name}Id`)),
        flatObjectMetadataMaps: {
            byUniversalIdentifier: {
                ...
                ...Object.fromEntries(entries(targetObjectNameSingularByKey).map(([key, nameSingular])=>[
                        `object-uid-${key}`,
                        {
                            id: `object-${key}`,
            ...
            universalIdentifierById: {
                [TIMELINE_ACTIVITY_OBJECT_ID]: _metadata.STANDARD_OBJECTS.timelineActivity.universalIdentifier,
                ...Object.fromEntries(Object.keys(targetObjectNameSingularByKey).map((key)=>[
                        `object-${key}`,
                        `object-uid-${key}`
                    ]))
            },
        ...
        flatFieldMetadataMaps: {
            byUniversalIdentifier: Object.fromEntries(fields.map((field)=>[
                    `field-uid-${field.name}`,
                    {
                        ...
                        universalSettings: {
                            relationType: field.relationType ?? _types.RelationType.MANY_TO_ONE,
                            joinColumnName: `${field.name}Id`
                        }
                    }
                ])),
            universalIdentifierById: Object.fromEntries(fields.map(({ name })=>[
                    `field-${name}`,
                    `field-uid-${name}`
                ])),
            universalIdentifiersByApplicationId: {}
        ...
        flatIndexMaps: {
            byUniversalIdentifier: Object.fromEntries(entries(indexedFieldNamesByIndexKey).map(([key, fieldNames])=>[
                    `index-uid-${key}`,
                    {
                        id: `index-${key}`,
            ...
            universalIdentifierById: Object.fromEntries(indexKeys.map((key)=>[
                    `index-${key}`,
                    `index-uid-${key}`
                ])),
            universalIdentifiersByApplicationId: {}
...
describe('buildTimelineActivityTargetFieldRepairs', ()=>{
    it('renames a stale field, its join column and its system label', ()=>{
        const { flatFieldMetadatasToUpdate, unrepairableTargetFields } = (0, _buildtimelineactivitytargetfieldrepairsutil.buildTimelineActivityTargetFieldRepairs)(buildArgs({
            ...staleField,
            fields: [
                {
                    ...staleField.fields[0],
        ...
        expect(flatFieldMetadatasToUpdate[0]).toEqual(expect.objectContaining({
    ...
    it('leaves a curated label untouched when the field is not a system side effect', ()=>{
        const { flatFieldMetadatasToUpdate } = (0, _buildtimelineactivitytargetfieldrepairsutil.buildTimelineActivityTargetFieldRepairs)(buildArgs({
            ...staleField,
            fields: [
                {
                    ...staleField.fields[0],
    ...
    it('does nothing when every target field already matches its object name', ()=>{
        const result = (0, _buildtimelineactivitytargetfieldrepairsutil.buildTimelineActivityTargetFieldRepairs)(buildArgs({
            targetObjectNameSingularByKey: {
                phoneNumber: 'phoneNumber2'
            },
            fields: [
    ...
    it('treats a rename chain identically whatever order the fields arrive in', ()=>{
        const summarise = (args)=>{
            const { flatFieldMetadatasToUpdate, unrepairableTargetFields } = (0, _buildtimelineactivitytargetfieldrepairsutil.buildTimelineActivityTargetFieldRepairs)(buildArgs(args));
            return {
        ...
        expect(forwardOrder.renamed).toEqual([
            'targetPhoneNumber2'
        ]);
        expect(forwardOrder.blocked).toEqual([
            'targetContact'
        ]);
    });
    ...
    it('recomputes an index covering two repaired fields once, from the final names', ()=>{
        const { flatIndexMetadatasToUpdate } = (0, _buildtimelineactivitytargetfieldrepairsutil.buildTimelineActivityTargetFieldRepairs)(buildArgs({
            targetObjectNameSingularByKey: {
                a: 'alpha2',
                b: 'beta2'
            },
        ...
        expect(flatIndexMetadatasToUpdate[0].universalIdentifier).toBe('index-uid-shared');
    ...
    it('refuses to repair when the expected column already exists', ()=>{
        ...
        expect(flatFieldMetadatasToUpdate).toEqual([]);
        expect(unrepairableTargetFields[0].reason).toContain('already exists while metadata still names it');
    });
    it('refuses to repair when both columns exist', ()=>{
        const { unrepairableTargetFields } = (0, _buildtimelineactivitytargetfieldrepairsutil.buildTimelineActivityTargetFieldRepairs)(buildArgs({
            ...staleField,
            columnNames: [
                'targetPhoneNumberId',
                'targetPhoneNumber2Id'
...
//# sourceMappingURL=build-timeline-activity-target-field-repairs.util.spec.js.map