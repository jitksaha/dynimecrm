...
describe('BackfillMessageCalendarTargetsCommand', ()=>{
    it('skips workspaces without a data source', async ()=>{
        const command = new _237workspacecommand1787832413051backfillmessagecalendartargetscommand.BackfillMessageCalendarTargetsCommand({});
        ...
        expect(command['logger'].warn).toHaveBeenCalledWith(`Skipping message and calendar target backfill for workspace ${WORKSPACE_ID}: no workspace data source`);
    });
    it('continues a full candidate batch when conflicts reduce inserted rows', async ()=>{
        ...
        expect(command['logger'].log).toHaveBeenCalledWith(`Created 5000 calendar event person targets for workspace ${WORKSPACE_ID}`);
    });
    it('skips workspaces whose target tables were not provisioned', async ()=>{
        ...
        expect(command['logger'].warn).toHaveBeenCalledWith(`Skipping message and calendar target backfill for workspace ${WORKSPACE_ID}: target tables are not provisioned`);
    });
});

//# sourceMappingURL=2-37-workspace-command-1787832413051-backfill-message-calendar-targets.command.spec.js.map