...
let WorkspaceExportService = class WorkspaceExportService {
    async exportWorkspace({ workspaceId, outputPath, tableFilter }) {
        ...
        for (const searchFieldMetadata of searchFieldMetadatas){
            const objectSearchFieldMetadatas = searchFieldMetadatasByObjectId.get(searchFieldMetadata.objectMetadataId) ?? [];
            objectSearchFieldMetadatas.push(searchFieldMetadata);
            searchFieldMetadatasByObjectId.set(searchFieldMetadata.objectMetadataId, objectSearchFieldMetadatas);
        }
        ...
        try {
            ...
            this.writeWorkspaceSchemaDdl(workspaceId, schemaName, objectMetadatas, fieldsByObjectId, searchFieldMetadatasByObjectId, stream);
    ...
    writeWorkspaceSchemaDdl(workspaceId, schemaName, objectMetadatas, fieldsByObjectId, searchFieldMetadatasByObjectId, stream) {
        this.logger.log('Generating workspace schema DDL from metadata...');
        const ddlStatements = (0, _generateworkspaceschemaddlutil.generateWorkspaceSchemaDdl)(workspaceId, schemaName, objectMetadatas, fieldsByObjectId, searchFieldMetadatasByObjectId);
    ...
    constructor(dataSource, objectMetadataRepository, fieldMetadataRepository, // eslint-disable-next-line twenty/prefer-workspace-scoped-repository -- Ignored
...
WorkspaceExportService = _ts_decorate([
    ...
    _ts_param(3, (0, _typeorm.InjectRepository)(_searchfieldmetadataentity.SearchFieldMetadataEntity)),
