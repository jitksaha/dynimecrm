...
describe('validateOperatorForFieldTypeOrThrow', ()=>{
    ...
    it.each([
        ...
        'lt',
        'lte'
    ])('should not throw when ordering operator "%s" is used on a SELECT field', (operator)=>{
        const fieldMetadata = createFieldMetadata(_types.FieldMetadataType.SELECT);
        expect(()=>(0, _validateoperatorforfieldtypeorthrowutil.validateOperatorForFieldTypeOrThrow)(operator, fieldMetadata, 'severity')).not.toThrow();
    });
});
