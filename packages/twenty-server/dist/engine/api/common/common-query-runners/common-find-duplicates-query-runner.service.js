...
let CommonFindDuplicatesQueryRunnerService = class CommonFindDuplicatesQueryRunnerService extends _commonbasequeryrunnerservice.CommonBaseQueryRunnerService {
    async run(args, queryRunnerContext) {
        const { flatObjectMetadata, flatObjectMetadataMaps, flatFieldMetadataMaps, commonQueryParser, authContext, rolePermissionConfig } = queryRunnerContext;
        const readRepository = this.getReadRepository(queryRunnerContext);
        const existingRecordsQueryBuilder = readRepository.createQueryBuilder(flatObjectMetadata.nameSingular);
        let objectRecords = [];
        ...
        const findDuplicatesOutput = await Promise.all(objectRecords.map(async (record)=>{
            ...
            const duplicateRecordsQueryBuilder = readRepository.createQueryBuilder(flatObjectMetadata.nameSingular);
            ...
            const { startCursor, endCursor } = (0, _getpageinfoutil.getPageInfo)({
                records: duplicates,
                orderBy: [
                    {
