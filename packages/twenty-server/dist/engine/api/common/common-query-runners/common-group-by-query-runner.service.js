...
let CommonGroupByQueryRunnerService = class CommonGroupByQueryRunnerService extends _commonbasequeryrunnerservice.CommonBaseQueryRunnerService {
    async run(args, queryRunnerContext) {
        const { commonQueryParser, flatObjectMetadata, flatObjectMetadataMaps, flatFieldMetadataMaps, authContext } = queryRunnerContext;
    ...
    async addFiltersFromView({ args, flatObjectMetadata, flatFieldMetadataMaps, restrictedFields, appliedFilters, workspaceId }) {
        ...
        const fields = (0, _getflatfieldsforflatobjectmetadatautil.getFlatFieldsFromFlatObjectMetadata)(flatObjectMetadata, flatFieldMetadataMaps).filter((field)=>restrictedFields[field.id]?.canRead !== false).map((field)=>({
                id: field.id,
                name: field.name,
                type: field.type,
                label: field.label,
    ...
    async addFiltersToQueryBuilder({ args, appliedFilters, queryBuilder, flatObjectMetadata, flatFieldMetadataMaps, workspaceId, commonQueryParser }) {
        ...
        if (args.viewId) {
            appliedFilters = await this.addFiltersFromView({
                ...
                flatObjectMetadata,
                flatFieldMetadataMaps,
                restrictedFields: queryBuilder.objectRecordsPermissions[flatObjectMetadata.id]?.restrictedFields ?? {},
                appliedFilters,
                workspaceId
            });
        }
        commonQueryParser.applyFilterToBuilder(queryBuilder, objectAlias, appliedFilters);
