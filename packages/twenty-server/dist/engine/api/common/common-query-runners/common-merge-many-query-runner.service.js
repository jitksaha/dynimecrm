...
let CommonMergeManyQueryRunnerService = class CommonMergeManyQueryRunnerService extends _commonbasequeryrunnerservice.CommonBaseQueryRunnerService {
    ...
    performDeepMerge(recordsToMerge, priorityRecordId, flatObjectMetadata, flatFieldMetadataMaps, isDryRun = false) {
        ...
        recordsToMerge.forEach((record)=>{
            Object.keys(record).forEach((fieldName)=>{
                if (!this.shouldExcludeFieldFromMerge(fieldName, fieldIdByName, flatFieldMetadataMaps)) {
    ...
    shouldExcludeFieldFromMerge(fieldName, fieldIdByName, flatFieldMetadataMaps) {
        const fieldMetadata = (0, _findflatentitybyidinflatentitymapsutil.findFlatEntityByIdInFlatEntityMaps)({
            flatEntityId: fieldIdByName[fieldName],
            flatEntityMaps: flatFieldMetadataMaps
        });
    ...
    getRelationFieldsPointingToCurrentObject(context) {
        ...
        for (const field of Object.values(flatFieldMetadataMaps.byUniversalIdentifier).filter(_utils.isDefined)){
            if (!(0, _ismorphorrelationflatfieldmetadatautil.isMorphOrRelationFlatFieldMetadata)(field) || field.relationTargetObjectMetadataId !== flatObjectMetadata.id || !field.isActive) {
                continue;
            }
    ...
    async executeMergeWithinTransaction({ args, queryRunnerContext, idsToDelete, priorityRecordId, mergedData }) {
        const { flatObjectMetadata, flatObjectMetadataMaps, flatFieldMetadataMaps, rolePermissionConfig } = queryRunnerContext;
        ...
        return this.workspaceOrmManager.runInWorkspaceTransaction(async (transactionScope)=>{
            for (const relationField of this.getRelationFieldsPointingToCurrentObject(queryRunnerContext)){
                const relatedRepository = transactionScope.getRepository(relationField.objectMetadata.nameSingular, rolePermissionConfig);
                await relatedRepository.runMutation({
                    selectQueryBuilder: relatedRepository.createQueryBuilder(relationField.objectMetadata.nameSingular).where({
                        [relationField.joinColumnName]: (0, _typeorm.In)(idsToDelete)
                    }),
                    ...
                    data: {
                        [relationField.joinColumnName]: priorityRecordId
                    }
                });
            }
            const objectRepository = transactionScope.getRepository(alias, rolePermissionConfig);
            await objectRepository.runMutation({
                selectQueryBuilder: objectRepository.createQueryBuilder(alias).where({
                    id: (0, _typeorm.In)(idsToDelete)
                }),
                rowLevelPermissionsApplied: false,
                kind: 'delete',
                columnsToReturn
            ...
            if (!(0, _utils.isDefined)(updatedRecords[0])) {
    ...
    async processNestedRelations({ args, queryRunnerContext, updatedRecords }) {
        const { flatObjectMetadataMaps, flatFieldMetadataMaps, flatObjectMetadata, authContext, rolePermissionConfig } = queryRunnerContext;
    ...
    constructor(...args){
        super(...args), this.operationName = _commonqueryargstype.CommonQueryNames.MERGE_MANY;
    }
};
