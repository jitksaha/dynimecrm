...
const _computegraphqldirectexecutionquerycostutil = require("./utils/compute-graphql-direct-execution-query-cost.util");
...
let DirectExecutionService = class DirectExecutionService {
    ...
    async executeWorkspaceQuery(req, document) {
        try {
            ...
            const data = {};
            const { graphQLResolverNameMap, flatObjectMetadataMaps, flatFieldMetadataMapsOrm: flatFieldMetadataMaps, flatIndexMaps } = await this.workspaceCacheService.getOrRecompute(workspaceId, [
                'graphQLResolverNameMap',
            ...
            await Promise.all(topLevelFields.map(async (field)=>{
                ...
                try {
                    ...
                    const workspaceSchemaBuilderContext = (0, _buildworkspaceschemabuildercontextutil.buildWorkspaceSchemaBuilderContext)(entry, flatObjectMetadataMaps, flatFieldMetadataMaps, flatIndexMaps, objectIdByNameSingular);
    ...
    recordQueryCost({ document, fragmentMap, topLevelFields, variables, graphQLResolverNameMap }) {
        const queryCost = (0, _computegraphqldirectexecutionquerycostutil.computeGraphQLDirectExecutionQueryCost)({
        ...
        _node.getActiveSpan()?.setAttributes({
            'graphql.direct_execution.estimated_result_field_count': queryCost.estimatedResultFieldCount,
            ...(0, _utils.isDefined)(query) && queryCost.estimatedResultFieldCount >= QUERY_FINGERPRINT_MIN_COST && {
                'graphql.direct_execution.query_fingerprint': (0, _crypto.createHash)('sha256').update(query).digest('hex').slice(0, 16)
            },
            'graphql.direct_execution.requested_row_count': queryCost.requestedRowCount,
            'graphql.direct_execution.selected_leaf_field_count': queryCost.selectedLeafFieldCount
        });
    }
    ...
    constructor(workspaceFlatEntityMapsCacheService, workspaceCacheService, workspaceGraphqlSchemaSDLService, twentyConfigService, i18nService, metricsService, findManyResolverFactory, findOneResolverFactory, findDuplicatesResolverFactory, groupByResolverFactory, createOneResolverFactory, createManyResolverFactory, updateOneResolverFactory, updateManyResolverFactory, deleteOneResolverFactory, deleteManyResolverFactory, destroyOneResolverFactory, destroyManyResolverFactory, restoreOneResolverFactory, restoreManyResolverFactory, mergeManyResolverFactory){
        ...
        this.queryCostHistogram = this.metricsService.getMeter().createHistogram('twenty_graphql_direct_execution_estimated_result_field_count', {
            description: 'Estimated number of result field values produced by a directly executed GraphQL request',
            advice: {
                explicitBucketBoundaries: QUERY_COST_BUCKETS
            }
        });
