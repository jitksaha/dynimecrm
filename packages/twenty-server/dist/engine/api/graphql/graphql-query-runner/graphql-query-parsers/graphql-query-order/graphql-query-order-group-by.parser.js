...
let GraphqlQueryOrderGroupByParser = class GraphqlQueryOrderGroupByParser {
    parse({ orderBy, groupByFields }) {
        ...
        for (const orderByArg of orderBy){
            if (this.isAggregateOrderByArg(orderByArg)) {
                const parsedAggregateOrderBy = this.parseAggregateOrderByArg(availableAggregations, orderByArg);
    ...
    constructor(flatObjectMetadata, flatObjectMetadataMaps, flatFieldMetadataMaps){
        ...
        this.parseAggregateOrderByArg = (availableAggregations, orderByArg)=>{
            ...
            const aggregateExpression = _processaggregatehelper.ProcessAggregateHelper.getAggregateExpression(aggregateField, this.objectAlias);
        ...
        this.parseObjectRecordOrderByForScalarField = ({ groupByFields, orderByArg, fieldMetadata })=>{
        ...
        this.parseObjectRecordOrderByForCompositeField = ({ groupByFields, orderByArg, fieldMetadata })=>{
            const fieldName = Object.keys(orderByArg)[0];
            ...
            return (0, _parsecompositefieldfororderutil.parseCompositeFieldForOrder)(fieldMetadata, orderBySubField, this.objectAlias);
        };
        this.parseObjectRecordOrderByWithGroupByDateField = ({ groupByFields, orderByArg, fieldMetadataId })=>{
            ...
            const columnNameWithQuotes = `"${this.objectAlias}"."${(0, _formatcolumnnamesfromcompositefieldandsubfieldutil.formatColumnNamesFromCompositeFieldAndSubfields)(associatedGroupByField.fieldMetadata.name, associatedGroupByField.subFieldName ? [
        ...
        this.parseObjectRecordOrderByForRelationField = ({ groupByFields, orderByArg, fieldMetadata })=>{
            const { associatedGroupByField, nestedFieldMetadata, nestedFieldOrderByValue, isMatchedOnTargetPrimaryKeyGroupBy } = (0, _preparefororderbyrelationfieldparsingutil.prepareForOrderByRelationFieldParsing)({
                orderByArg,
                fieldMetadata,
            ...
            if ((0, _iscompositefieldmetadatatypeutil.isCompositeFieldMetadataType)(nestedFieldMetadata.type)) {
                ...
                if (isMatchedOnTargetPrimaryKeyGroupBy === true) {
                    const groupableSubFields = (0, _getgroupablesubfieldsforcompositetypeutil.getGroupableSubFieldsForCompositeType)(nestedFieldMetadata.type);
                    if (!groupableSubFields?.includes(nestedSubFieldName)) {
                        throw new _graphqlerrorsutil.UserInputError(`Composite subfield "${nestedSubFieldName}" is not orderable for "${nestedFieldMetadata.name}"`);
                    }
                } else if (!(0, _utils.isDefined)(associatedGroupByField.nestedSubFieldName) || associatedGroupByField.nestedSubFieldName !== nestedSubFieldName) {
        ...
        this.objectAlias = (0, _getobjectaliasforgroupbyutil.getObjectAlias)(flatObjectMetadata);
