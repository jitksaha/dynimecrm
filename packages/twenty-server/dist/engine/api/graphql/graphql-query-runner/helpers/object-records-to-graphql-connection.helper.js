...
let ObjectRecordsToGraphqlConnectionHelper = class ObjectRecordsToGraphqlConnectionHelper {
    createConnection({ objectRecords, parentObjectRecord, objectRecordsAggregatedValues = {}, selectedAggregatedFields = [], objectName, take, totalCount, order, hasNextPage, hasPreviousPage, depth = 0, orderByValuesByRecordId }) {
        ...
        const edges = (objectRecords ?? []).map((objectRecord)=>({
                ...
                cursor: (0, _cursorsutil.encodeCursor)({
                    objectRecord,
                    order,
                    flatObjectMetadata,
    ...
    processRecord({ objectRecord, objectName, objectRecordsAggregatedValues = {}, selectedAggregatedFields = [], take, totalCount, order, depth = 0 }) {
        ...
        for (const fieldId of flatObjectMetadata.fieldIds){
            ...
            if ((0, _ismorphorrelationflatfieldmetadatautil.isMorphOrRelationFlatFieldMetadata)(fieldMetadata)) {
                ...
                const isToManyRelation = fieldMetadata.settings?.relationType === _relationtypeinterface.RelationType.ONE_TO_MANY;
                const objectValue = !(0, _utils.isDefined)(objectRecord[fieldMetadata.name]) && isToManyRelation ? [] : objectRecord[fieldMetadata.name];
                if (!(0, _utils.isDefined)(objectValue)) {
                    continue;
                }
    ...
    processCompositeField(fieldMetadata, // oxlint-disable-next-line typescript/no-explicit-any
    fieldValue) {
