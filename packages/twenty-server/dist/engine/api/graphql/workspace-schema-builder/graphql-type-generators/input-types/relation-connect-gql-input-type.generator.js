...
let RelationConnectGqlInputTypeGenerator = class RelationConnectGqlInputTypeGenerator {
    ...
    generateRelationConnectInputType(flatObjectMetadata, fields, context) {
        return new _graphql.GraphQLInputObjectType({
            ...
            fields: ()=>{
                const createInputType = this.gqlTypesStorage.getGqlTypeByKey((0, _computeobjectmetadatainputtypeutil.computeObjectMetadataInputTypeKey)(flatObjectMetadata.nameSingular, _gqlinputtypedefinitionkindenum.GqlInputTypeDefinitionKind.Create));
                if (!(createInputType instanceof _graphql.GraphQLInputObjectType)) {
                    throw new Error(`Create input type not found for ${flatObjectMetadata.nameSingular}`);
                }
                return {
                    ...
                    [_constants.RELATION_NESTED_QUERY_KEYWORDS.CREATE]: {
