...
let CoreEntityCacheService = class CoreEntityCacheService {
    ...
    evictExpiredLocalEntriesIfNeeded() {
        const now = Date.now();
        if ((0, _utils.isDefined)(this.lastLocalCacheExpirationSweepAt) && now - this.lastLocalCacheExpirationSweepAt < LOCAL_CACHE_EXPIRATION_SWEEP_INTERVAL_MS) {
            return;
        }
        this.evictExpiredLocalEntries(now);
        this.lastLocalCacheExpirationSweepAt = now;
    }
    evictExpiredLocalEntries(now) {
