...
let SSOAuthController = class SSOAuthController {
    ...
    async authCallback(req, res) {
        ...
        try {
            if (!workspaceIdentityProvider || workspaceIdentityProvider.status !== _workspacessoidentityproviderentity.SSOIdentityProviderStatus.Active) {
            ...
            if (currentWorkspace.id !== workspaceIdentityProvider.workspaceId) {
                throw new _authexception.AuthException('Identity provider does not belong to this workspace', _authexception.AuthExceptionCode.OAUTH_ACCESS_DENIED);
            }
...
SSOAuthController = _ts_decorate([
    (0, _common.Controller)(_types.ApiPath.Auth),
