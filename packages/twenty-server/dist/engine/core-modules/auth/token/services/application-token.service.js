...
let ApplicationTokenService = class ApplicationTokenService {
    ...
    async generateWorkspaceDeletionApplicationAccessToken({ workspaceId, applicationId, workspaceDeletionRequestTimestamp }) {
        ...
        if (!(0, _isworkspacedeletionrequestpendingutil.isWorkspaceDeletionRequestPending)(workspace, workspaceDeletionRequestTimestamp)) {
    ...
    async generateApplicationTokenPair({ workspaceId, applicationId, userWorkspaceId, userId }) {
        ...
        const [applicationAccessToken, applicationRefreshToken] = await Promise.all([
            ...
            this.signApplicationToken({
                ...
                userWorkspaceId,
                userId,
                tokenType: _jwttokentypeenum.JwtTokenTypeEnum.APPLICATION_REFRESH,
    ...
    async validateApplicationRefreshToken(refreshToken) {
        try {
            ...
            if (payload.type !== _jwttokentypeenum.JwtTokenTypeEnum.APPLICATION_REFRESH) {
    ...
    async signApplicationToken({ workspaceId, applicationId, userWorkspaceId, userId, tokenType, expiresIn, workspaceDeletionRequestTimestamp }) {
