...
let RenewTokenService = class RenewTokenService {
    async generateTokensFromRefreshToken(token) {
        ...
        const targetedTokenType = targetedTokenTypeFromPayload ?? _jwttokentypeenum.JwtTokenTypeEnum.ACCESS;
        const resolvedAuthProvider = authProvider ?? _workspacetype.AuthProviderEnum.Password;
        const accessToken = (0, _utils.isDefined)(authProvider) && targetedTokenType === _jwttokentypeenum.JwtTokenTypeEnum.WORKSPACE_AGNOSTIC && !(0, _utils.isDefined)(workspaceId) ? await this.workspaceAgnosticTokenService.generateWorkspaceAgnosticToken({
            userId: user.id,
            authProvider
