...
const _getorcreatesessionsandboxutil = require("../get-or-create-session-sandbox.util");
...
const buildSandboxApi = (overrides = {})=>{
    const mock = {
...
const buildFakeSandbox = ()=>({
        setTimeout: jest.fn().mockResolvedValue(undefined),
        kill: jest.fn().mockResolvedValue(undefined)
    });
const listedSandbox = (sandboxId, tag = sessionId)=>({
        sandboxId,
        metadata: {
...
describe('getOrCreateSessionSandbox', ()=>{
    it('reuses and extends the existing sandbox for the session', async ()=>{
        ...
        const result = await (0, _getorcreatesessionsandboxutil.getOrCreateSessionSandbox)({
            sandboxApi: api,
            apiKey,
            sessionId,
            timeoutMs,
        ...
        expect(mock.connect).toHaveBeenCalledWith('sbx-1', {
            apiKey
        });
    ...
    it('reaps duplicate sandboxes and reuses a single one', async ()=>{
        ...
        expect(mock.connect).toHaveBeenCalledWith('sbx-keep', {
            apiKey
        });
        expect(mock.kill).toHaveBeenCalledWith('sbx-dup-1', {
            apiKey
        });
        expect(mock.kill).toHaveBeenCalledWith('sbx-dup-2', {
            apiKey
        });
        expect(mock.kill).not.toHaveBeenCalledWith('sbx-keep', {
            apiKey
        });
        expect(mock.create).not.toHaveBeenCalled();
    });
    ...
    it('keeps the first connectable sandbox instead of cold-creating when an earlier one is dead', async ()=>{
        ...
        const { api, mock } = buildSandboxApi({
            ...
            connect: jest.fn((sandboxId)=>sandboxId === 'sbx-live' ? Promise.resolve(live) : Promise.reject(new Error('sandbox not found')))
        });
        const result = await (0, _getorcreatesessionsandboxutil.getOrCreateSessionSandbox)({
            sandboxApi: api,
            apiKey,
        ...
        expect(mock.kill).toHaveBeenCalledWith('sbx-dead', {
            apiKey
        });
        expect(mock.create).not.toHaveBeenCalled();
    });
    ...
    it('kills a connected sandbox and creates fresh when refreshing its timeout fails', async ()=>{
        const stale = {
            setTimeout: jest.fn().mockRejectedValue(new Error('timeout refresh failed')),
            kill: jest.fn().mockResolvedValue(undefined)
        };
        const created = buildFakeSandbox();
        const { api, mock } = buildSandboxApi({
            list: jest.fn(()=>paginatorOf([
                    listedSandbox('sbx-stale')
                ])),
...
//# sourceMappingURL=get-or-create-session-sandbox.util.spec.js.map