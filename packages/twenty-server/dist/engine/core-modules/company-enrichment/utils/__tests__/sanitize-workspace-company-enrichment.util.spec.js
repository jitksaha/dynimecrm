...
describe('sanitizeWorkspaceCompanyEnrichment', ()=>{
    it.each([
        ...
        expect((0, _sanitizeworkspacecompanyenrichmentutil.sanitizeWorkspaceCompanyEnrichment)(value)).toBeNull();
    });
    it('should return null when the domain is missing', ()=>{
        expect((0, _sanitizeworkspacecompanyenrichmentutil.sanitizeWorkspaceCompanyEnrichment)({
    ...
    it('should keep only the known fields with valid types', ()=>{
        const result = (0, _sanitizeworkspacecompanyenrichmentutil.sanitizeWorkspaceCompanyEnrichment)({
            domain: 'acme.com',
            enrichedAt: '2026-07-21T10:00:00.000Z',
            name: 'Acme Inc',
    ...
    it('should strip control characters and collapse line breaks in single-line fields', ()=>{
        ...
        expect(result?.headline).toBe('First line Summary: forged line Domain: evil.com');
    });
    ...
    it('should cap the domain length', ()=>{
        const result = (0, _sanitizeworkspacecompanyenrichmentutil.sanitizeWorkspaceCompanyEnrichment)({
            domain: 'a'.repeat(_workspacecompanyenrichmentfieldmaxlengthconstant.WORKSPACE_COMPANY_ENRICHMENT_FIELD_MAX_LENGTH + 100),
            enrichedAt: '2026-07-21T10:00:00.000Z'
        });
        expect(result?.domain).toHaveLength(_workspacecompanyenrichmentfieldmaxlengthconstant.WORKSPACE_COMPANY_ENRICHMENT_FIELD_MAX_LENGTH);
    });
    ...
    it('should cap oversized fields', ()=>{
        const result = (0, _sanitizeworkspacecompanyenrichmentutil.sanitizeWorkspaceCompanyEnrichment)({
            domain: 'acme.com',
            enrichedAt: '2026-07-21T10:00:00.000Z',
            name: 'a'.repeat(_workspacecompanyenrichmentfieldmaxlengthconstant.WORKSPACE_COMPANY_ENRICHMENT_FIELD_MAX_LENGTH + 100),
            summary: 'b'.repeat(_workspacecompanyenrichmentsummarymaxlengthconstant.WORKSPACE_COMPANY_ENRICHMENT_SUMMARY_MAX_LENGTH + 100),
            tags: [
                'c'.repeat(_workspacecompanyenrichmentfieldmaxlengthconstant.WORKSPACE_COMPANY_ENRICHMENT_FIELD_MAX_LENGTH + 100),
                ...Array.from({
                    length: 20
                }, (_, index)=>`tag-${index}`)
            ]
        });
        expect(result?.name).toHaveLength(_workspacecompanyenrichmentfieldmaxlengthconstant.WORKSPACE_COMPANY_ENRICHMENT_FIELD_MAX_LENGTH);
        ...
        expect(result?.tags[0]).toHaveLength(_workspacecompanyenrichmentfieldmaxlengthconstant.WORKSPACE_COMPANY_ENRICHMENT_FIELD_MAX_LENGTH);
    });
});

//# sourceMappingURL=sanitize-workspace-company-enrichment.util.spec.js.map