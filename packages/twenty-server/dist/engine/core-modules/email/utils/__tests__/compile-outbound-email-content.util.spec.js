...
const _compileoutboundemailcontentutil = require("../compile-outbound-email-content.util");
const compileDocument = async (document)=>(await (0, _compileoutboundemailcontentutil.compileOutboundEmailContent)(document)).html;
describe('compileOutboundEmailContent', ()=>{
    ...
    it('should preserve the rendered email document preamble', async ()=>{
        ...
        expect(html).toContain('<meta content="text/html; charset=UTF-8" http-equiv="Content-Type">');
    ...
    it('should reject unknown structured nodes before rendering', async ()=>{
        await expect((0, _compileoutboundemailcontentutil.compileOutboundEmailContent)({
            type: 'doc',
            content: [
                {
                    type: 'someFutureNode',
    ...
    it('should sanitize structured and legacy HTML with the same policy', async ()=>{
        ...
        const legacy = await (0, _compileoutboundemailcontentutil.compileOutboundEmailContent)('<a href="javascript:alert(1)" onclick="alert(1)">Open</a><script>alert(1)</script>');
        for (const html of [
            structured,
            legacy.html
        ]){
    ...
    it('should derive plain text from the sanitized HTML', async ()=>{
        await expect((0, _compileoutboundemailcontentutil.compileOutboundEmailContent)('<p>Hello <strong>Ada</strong></p>')).resolves.toEqual({
...
//# sourceMappingURL=compile-outbound-email-content.util.spec.js.map