...
const _resolvecampaignaudienceutil = require("../resolve-campaign-audience.util");
const buildRawRecipients = (emails)=>emails.map((email, index)=>({
            personId: `person-${index}`,
            email
        }));
describe('resolveCampaignAudience', ()=>{
    it('should report the sendable count as the number of recipients it returns', ()=>{
        const { sendableRecipients, audience } = (0, _resolvecampaignaudienceutil.resolveCampaignAudience)({
    ...
    it('should report a bounced recipient as hard suppressed rather than unsubscribed', ()=>{
        const { audience } = (0, _resolvecampaignaudienceutil.resolveCampaignAudience)({
            rawRecipients: buildRawRecipients([
                'bounced@example.com',
                'optout@example.com'
            ]),
...
//# sourceMappingURL=resolve-campaign-audience.util.spec.js.map