...
Object.defineProperty(exports, "buildUnsubscribePreferencesPage", {
    enumerable: true,
    get: function() {
...
const buildTopicCheckbox = (topic)=>{
    const label = (0, _escapehtmlutil.escapeHtml)(topic.topicName ?? 'Untitled topic');
    const value = (0, _escapehtmlutil.escapeHtml)(topic.unsubscribeTopicId);
...
const buildUnsubscribePreferencesPage = ({ token, topics, updatePath, unsubscribeAllPath })=>{
    const safeToken = (0, _escapehtmlutil.escapeHtml)(token);
    ...
    const body = topics.length > 0 ? `<p class="subtitle">Confirm your preferences:</p><form method="post" action="${updatePath}">${tokenField}<div class="topics">${topics.map(buildTopicCheckbox).join('')}</div><button type="submit" class="primary">Update</button></form><p class="divider">Or</p><form method="post" action="${unsubscribeAllPath}">${tokenField}<button type="submit" class="secondary">Unsubscribe all</button></form>` : `<p class="subtitle">You will stop receiving these emails.</p><form method="post" action="${unsubscribeAllPath}">${tokenField}<button type="submit" class="primary">Unsubscribe</button></form>`;
...
//# sourceMappingURL=build-unsubscribe-preferences-page.util.js.map