...
describe('CreateEventLogFromInternalEvent', ()=>{
    ...
    const transientError = Object.assign(new Error('Failed to insert 1 objectEvent row(s) into ClickHouse'), {
        cause: new Error('Timeout error.')
    });
    beforeEach(()=>{
        ingest = jest.fn().mockResolvedValue(undefined);
        addToQueue = jest.fn().mockResolvedValue(undefined);
        handler = new _createeventlogfrominternalevent.CreateEventLogFromInternalEvent({
            isEnabled: ()=>true,
            ingest
        }, {
            add: addToQueue
    ...
    it('requeues once on a transient ClickHouse network error instead of failing', async ()=>{
        ...
        expect(addToQueue).toHaveBeenCalledWith(_createeventlogfrominternalevent.CreateEventLogFromInternalEvent.name, expect.objectContaining({
    ...
    it('rethrows non-transient ingestion errors so they fail the job and reach Sentry', async ()=>{
        const error = new Error('Failed to insert 1 objectEvent row(s) into ClickHouse: (total) memory limit exceeded');
...
//# sourceMappingURL=create-event-log-from-internal-event.spec.js.map