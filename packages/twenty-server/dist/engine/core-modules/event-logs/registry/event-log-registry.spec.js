...
const _eventlogregistry = require("./event-log-registry");
...
describe('event-log registry', ()=>{
    it('has a complete definition for every EventLogTable', ()=>{
        for (const table of Object.values(_types.EventLogTable)){
            const definition = _eventlogregistry.EVENT_LOG_TYPES[table];
    ...
    it('exposes the ClickHouse table name', ()=>{
        expect((0, _eventlogregistry.getClickHouseTableName)(_types.EventLogTable.WORKSPACE_EVENT)).toBe('workspaceEvent');
        expect((0, _eventlogregistry.getClickHouseTableName)(_types.EventLogTable.APPLICATION_LOG)).toBe('applicationLog');
    });
    it('gates every table except application logs behind an entitlement', ()=>{
        expect(_eventlogregistry.EVENT_LOG_TYPES[_types.EventLogTable.APPLICATION_LOG].requiresEntitlement).toBeNull();
        expect(_eventlogregistry.EVENT_LOG_TYPES[_types.EventLogTable.WORKSPACE_EVENT].requiresEntitlement).not.toBeNull();
        expect(_eventlogregistry.EVENT_LOG_TYPES[_types.EventLogTable.USAGE_EVENT].requiresEntitlement).not.toBeNull();
    });
    describe('normalize', ()=>{
        const timestamp = '2026-01-01 00:00:00.000';
        it('maps a workspace event row', ()=>{
            const record = _eventlogregistry.EVENT_LOG_TYPES[_types.EventLogTable.WORKSPACE_EVENT].normalize({
                event: 'user.signup',
                timestamp,
                userId: 'user-1',
                properties: {
        ...
        it('parses the ClickHouse timestamp as UTC, not server-local time', ()=>{
            const [record] = (0, _normalizeeventlogrecords.normalizeEventLogRecords)([
                {
                    event: 'user.signup',
                    timestamp
                }
        ...
        it('maps a pageview row from the name column', ()=>{
            const record = _eventlogregistry.EVENT_LOG_TYPES[_types.EventLogTable.PAGEVIEW].normalize({
                name: '/settings',
                timestamp
            });
        ...
        it('maps a usage event row (resourceType, userWorkspaceId, folded metadata)', ()=>{
            const record = _eventlogregistry.EVENT_LOG_TYPES[_types.EventLogTable.USAGE_EVENT].normalize({
        ...
        it('maps an application log row (logicFunctionName, level, message)', ()=>{
            const record = _eventlogregistry.EVENT_LOG_TYPES[_types.EventLogTable.APPLICATION_LOG].normalize({
...
//# sourceMappingURL=event-log-registry.spec.js.map