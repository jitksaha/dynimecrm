...
let ExceptionHandlerSentryDriver = class ExceptionHandlerSentryDriver {
    captureExceptions(// oxlint-disable-next-line typescript/no-explicit-any
    exceptions, options) {
        const eventIds = [];
        _node.withScope((scope)=>{
            ...
            for (const exception of exceptions){
                const isObjectException = (0, _guards.isObject)(exception);
                const errorPath = (isObjectException ? exception.path ?? [] : []).map((v)=>typeof v === 'number' ? '$index' : v).join(' > ');
                if (errorPath) {
                    scope.addBreadcrumb({
