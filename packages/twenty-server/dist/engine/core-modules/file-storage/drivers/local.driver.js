...
let LocalDriver = class LocalDriver {
    ...
    async readFile(params) {
        ...
        try {
            return (0, _fs.createReadStream)(filePath, (0, _utils.isDefined)(params.byteRange) ? {
                start: params.byteRange.startByte,
                end: params.byteRange.endByte
            } : undefined);
        } catch (error) {
    ...
    async resolveWritableRealPathOrThrow(filePath) {
        const resolvedPath = _path.default.resolve(this.options.storagePath, filePath);
        const folderPath = (0, _path.dirname)(resolvedPath);
        await this.createFolder(folderPath);
        const realFolderPath = (0, _fs.realpathSync)(folderPath);
        const realFilePath = _path.default.join(realFolderPath, _path.default.basename(resolvedPath));
    ...
    async writeFile(params) {
        const realFilePath = await this.resolveWritableRealPathOrThrow(params.filePath);
        await _promises.writeFile(realFilePath, params.sourceFile);
    }
    async writeFileStream(params) {
        const realFilePath = await this.resolveWritableRealPathOrThrow(params.filePath);
        try {
            await (0, _promises1.pipeline)(params.stream, (0, _fs.createWriteStream)(realFilePath));
        } catch (error) {
            // Remove the partial file so a failed upload can be retried cleanly
            await _promises.rm(realFilePath, {
                force: true
            });
            throw error;
        }
