...
const _s3clienttimeoutsconstant = require("../constants/s3-client-timeouts.constant");
...
let S3Driver = class S3Driver {
    ...
    async readFile(params) {
        const command = new _clients3.GetObjectCommand({
            ...
            Range: (0, _utils.isDefined)(params.byteRange) ? `bytes=${params.byteRange.startByte}-${params.byteRange.endByte}` : undefined
        });
        try {
    ...
    async getFileMetadata(params) {
        try {
            const head = await this.s3Client.send(new _clients3.HeadObjectCommand({
                Bucket: this.bucketName,
                Key: params.filePath
            }));
            return {
    ...
    constructor(options){
        ...
        const requestHandler = (0, _awsrequesthandlerutil.buildAwsRequestHandlerOptions)({
            requestTimeoutMs: _s3clienttimeoutsconstant.FILE_STORAGE_S3_REQUEST_TIMEOUT_MS,
            connectionTimeoutMs: _s3clienttimeoutsconstant.FILE_STORAGE_S3_CONNECTION_TIMEOUT_MS,
            maxSockets: _s3clienttimeoutsconstant.FILE_STORAGE_S3_MAX_SOCKETS
        });
