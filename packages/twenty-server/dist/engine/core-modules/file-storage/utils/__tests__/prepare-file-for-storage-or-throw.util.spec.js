...
const _preparefileforstorageorthrowutil = require("../prepare-file-for-storage-or-throw.util");
...
describe('prepareFileForStorageOrThrow', ()=>{
    describe('mime resolution', ()=>{
        ...
        ])('returns $expectedMime for $name', async ({ sourceFile, resourcePath, expectedMime })=>{
            const { mimeType } = await (0, _preparefileforstorageorthrowutil.prepareFileForStorageOrThrow)({
                sourceFile,
                resourcePath
            });
        ...
        ])('returns the bytes-derived mime for $name', async ({ sourceFile, resourcePath, expectedMime })=>{
            const { mimeType } = await (0, _preparefileforstorageorthrowutil.prepareFileForStorageOrThrow)({
                sourceFile,
                resourcePath
            });
    ...
    describe('binary-extension backstop', ()=>{
        it('throws when a buffer with a binary extension does not match the magic bytes', async ()=>{
            await expect((0, _preparefileforstorageorthrowutil.prepareFileForStorageOrThrow)({
    ...
    describe('SVG sanitization', ()=>{
        ...
        it('preserves a benign SVG (no destructive sanitization)', async ()=>{
            const { sourceFile } = await (0, _preparefileforstorageorthrowutil.prepareFileForStorageOrThrow)({
...
//# sourceMappingURL=prepare-file-for-storage-or-throw.util.spec.js.map