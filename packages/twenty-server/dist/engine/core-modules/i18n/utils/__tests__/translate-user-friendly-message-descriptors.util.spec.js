...
const _translateuserfriendlymessagedescriptorsutil = require("../translate-user-friendly-message-descriptors.util");
const buildDescriptor = (id)=>({
        id,
        message: id
    });
const i18nMock = {
    _: (descriptor)=>`tr:${descriptor.id}`
};
describe('translateUserFriendlyMessageDescriptors', ()=>{
    it('translates a top-level userFriendlyMessage descriptor to a string', ()=>{
        const result = (0, _translateuserfriendlymessagedescriptorsutil.translateUserFriendlyMessageDescriptors)({
            userFriendlyMessage: buildDescriptor('top-level')
        }, i18nMock);
        expect(result).toEqual({
            userFriendlyMessage: 'tr:top-level'
        });
    });
    ...
    it('does not translate descriptors stored under other keys', ()=>{
        ...
        expect(result).toEqual({
            someOtherKey: {
                id: 'not-user-friendly',
                message: 'not-user-friendly'
            }
        });
    });
    it('translates userFriendlyMessage descriptors nested inside arrays of objects', ()=>{
        const result = (0, _translateuserfriendlymessagedescriptorsutil.translateUserFriendlyMessageDescriptors)({
            errors: [
                {
                    code: 'A',
                    userFriendlyMessage: buildDescriptor('first')
                },
                {
                    code: 'B',
                    userFriendlyMessage: buildDescriptor('second')
                }
            ]
        }, i18nMock);
        expect(result).toEqual({
            errors: [
                {
                    code: 'A',
                    userFriendlyMessage: 'tr:first'
                },
                {
                    code: 'B',
                    userFriendlyMessage: 'tr:second'
                }
            ]
        });
    });
    ...
    it('does not mutate the input payload', ()=>{
        ...
        const input = {
            userFriendlyMessage: descriptor,
            nested: {
                value: 1
            }
        };
        const result = (0, _translateuserfriendlymessagedescriptorsutil.translateUserFriendlyMessageDescriptors)(input, i18nMock);
...
//# sourceMappingURL=translate-user-friendly-message-descriptors.util.spec.js.map