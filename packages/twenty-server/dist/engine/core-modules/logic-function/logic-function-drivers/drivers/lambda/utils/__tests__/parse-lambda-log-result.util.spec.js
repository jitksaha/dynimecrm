...
const _parselambdalogresultutil = require("../parse-lambda-log-result.util");
...
describe('parseLambdaLogResult', ()=>{
    it('returns empty defaults when logResult is undefined', ()=>{
        expect((0, _parselambdalogresultutil.parseLambdaLogResult)(undefined)).toEqual({
            logs: '',
            initDurationMs: null,
    ...
    it('returns empty defaults when logResult is an empty string', ()=>{
        expect((0, _parselambdalogresultutil.parseLambdaLogResult)('')).toEqual({
            logs: '',
            initDurationMs: null,
            billedDurationMs: null,
            reportDurationMs: null,
    ...
    it('extracts Init/Billed/Duration milliseconds from a REPORT line and flags coldStart=true', ()=>{
        ...
        const result = (0, _parselambdalogresultutil.parseLambdaLogResult)(toBase64(raw));
    ...
    it('flags coldStart=false when no Init Duration is present', ()=>{
        const raw = 'REPORT RequestId: 1 Duration: 5 ms Billed Duration: 5 ms Memory Size: 512 MB Max Memory Used: 100 MB';
        const result = (0, _parselambdalogresultutil.parseLambdaLogResult)(toBase64(raw));
    ...
    it('trims leading and trailing whitespace from logs', ()=>{
        ...
        const result = (0, _parselambdalogresultutil.parseLambdaLogResult)(toBase64(raw));
        expect(result.logs.startsWith('2026-06-02T12:00:00.000Z INFO line')).toBe(true);
        expect(result.logs.endsWith('INFO line')).toBe(true);
    });
});

//# sourceMappingURL=parse-lambda-log-result.util.spec.js.map