...
`, 'utf-8')));
        ...
        await this.removeBuildTimeSdkFromDependencies((0, _path.join)(inMemoryFolderPath, 'package.json'));
    }
    ...
    async removeBuildTimeSdkFromDependencies(packageJsonPath) {
        const packageJson = JSON.parse(await _fs.promises.readFile(packageJsonPath, 'utf-8'));
        const dependencies = packageJson.dependencies;
        if (!(0, _utils.isDefined)(dependencies)) {
            return;
        }
        ...
        const packagesToRemoveFromDependencies = packagesToRemove.filter((packageName)=>(0, _utils.isDefined)(dependencies[packageName]));
        ...
        await _fs.promises.writeFile(packageJsonPath, `${JSON.stringify(packageJson, null, 2)}\n`, 'utf-8');
    }
