...
let OpenApiService = class OpenApiService {
    ...
    async generateCoreSchema(request) {
        ...
        schema.paths = flatObjectMetadataArray.reduce((paths, item)=>{
            paths[`/${item.namePlural}`] = (0, _pathutils.computeManyResultPath)(item);
            paths[`/batch/${item.namePlural}`] = (0, _pathutils.computeBatchPath)(item);
            paths[`/${item.namePlural}/{id}`] = (0, _pathutils.computeSingleResultPath)(item);
            paths[`/${item.namePlural}/duplicates`] = (0, _pathutils.computeDuplicatesResultPath)(item);
            paths[`/${item.namePlural}/{id}/restore`] = (0, _pathutils.computeRestoreOneResultPath)(item);
            paths[`/${item.namePlural}/restore`] = (0, _pathutils.computeRestoreManyResultPath)(item);
            paths[`/${item.namePlural}/merge`] = (0, _pathutils.computeMergeManyResultPath)(item);
            paths[`/${item.namePlural}/groupBy`] = (0, _pathutils.computeGroupByResultPath)(item);
            return paths;
        }, schema.paths);
        ...
        schema.webhooks = flatObjectMetadataArray.reduce((paths, item)=>{
            paths[this.createWebhookEventName(_databaseeventaction.DatabaseEventAction.CREATED, item.nameSingular)] = (0, _computeWebhooksutils.computeWebhooks)(_databaseeventaction.DatabaseEventAction.CREATED, item);
            paths[this.createWebhookEventName(_databaseeventaction.DatabaseEventAction.UPDATED, item.nameSingular)] = (0, _computeWebhooksutils.computeWebhooks)(_databaseeventaction.DatabaseEventAction.UPDATED, item);
            paths[this.createWebhookEventName(_databaseeventaction.DatabaseEventAction.DELETED, item.nameSingular)] = (0, _computeWebhooksutils.computeWebhooks)(_databaseeventaction.DatabaseEventAction.DELETED, item);
            return paths;
        }, {});
        schema.components = {
            ...schema.components,
        ...
        schema.paths = metadata.reduce((path, item)=>{
            path[`/${item.namePlural}`] = {
                get: {
                    ...
                    responses: {
                        '200': (0, _responsesutils.getFindManyResponse200)({
                            item,
