...
const getFindManyResponse200 = ({ item, isDirectDataFeatureFlagged = false })=>{
    const schemaRef = `#/components/schemas/${(0, _utils.capitalize)(item.nameSingular)}ForResponse`;
    const directDataSchema = {
        type: 'array',
        items: {
            $ref: schemaRef
        }
                ...
                schema: {
                    ...
                    properties: {
                        ...
                        data: isDirectDataFeatureFlagged ? {
                            description: 'An array when IS_REST_METADATA_API_NEW_FORMAT_DIRECT is enabled for the workspace, otherwise the legacy nested envelope.',
                            oneOf: [
                                directDataSchema,
                                nestedDataSchema
                            ]
