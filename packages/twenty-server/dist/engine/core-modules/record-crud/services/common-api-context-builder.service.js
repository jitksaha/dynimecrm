...
let CommonApiContextBuilderService = class CommonApiContextBuilderService {
    async build({ authContext, objectName, rolePermissionConfig }) {
    ...
    async getObjectsPermissions({ authContext, rolePermissionConfig }) {
        const workspaceId = authContext.workspace.id;
        ...
        if ((0, _utils.isDefined)(rolePermissionConfig)) {
            return (0, _getobjectspermissionsfromrolepermissionconfigutil.getObjectsPermissionsFromRolePermissionConfig)({
                rolesPermissions,
                rolePermissionConfig
            });
        }
