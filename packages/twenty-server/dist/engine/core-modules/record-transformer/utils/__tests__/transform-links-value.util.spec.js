"use strict";
Object.defineProperty(exports, "__esModule", {
    value: true
});
const _transformlinksvalueutil = require("../transform-links-value.util");
describe('transformLinksValue', ()=>{
    it('should handle null/undefined/empty object values', ()=>{
        expect((0, _transformlinksvalueutil.transformLinksValue)({
            input: null
        })).toBeNull();
        expect((0, _transformlinksvalueutil.transformLinksValue)({
            input: undefined
        })).toBeUndefined();
        expect((0, _transformlinksvalueutil.transformLinksValue)({
            input: {}
        })).toEqual({
            primaryLinkLabel: null,
            primaryLinkUrl: null,
            secondaryLinks: null
        });
    });
    describe('primary link', ()=>{
        it('should transform uppercase', ()=>{
            const input = {
                primaryLinkUrl: 'HTTPS://EXAMPLE.COM',
                primaryLinkLabel: 'Example',
                secondaryLinks: null
            };
            const expected = {
                primaryLinkUrl: 'https://example.com',
                primaryLinkLabel: 'Example',
                secondaryLinks: null
            };
            expect((0, _transformlinksvalueutil.transformLinksValue)({
                input
            })).toEqual(expected);
        });
        it('should remove trailing slash', ()=>{
            const input = {
                primaryLinkUrl: 'https://example.com/',
                primaryLinkLabel: 'Example',
                secondaryLinks: null
            };
            const expected = {
                primaryLinkUrl: 'https://example.com',
                primaryLinkLabel: 'Example',
                secondaryLinks: null
            };
            expect((0, _transformlinksvalueutil.transformLinksValue)({
                input
            })).toEqual(expected);
        });
        it('should work fine without protocol', ()=>{
            const input = {
                primaryLinkUrl: 'example.com',
                primaryLinkLabel: 'Example',
                secondaryLinks: null
            };
            const expected = {
                primaryLinkUrl: 'example.com',
                primaryLinkLabel: 'Example',
                secondaryLinks: null
            };
            expect((0, _transformlinksvalueutil.transformLinksValue)({
                input
            })).toEqual(expected);
        });
        it('should work fine with www', ()=>{
            const input = {
                primaryLinkUrl: 'www.example.com',
                primaryLinkLabel: 'Example',
                secondaryLinks: null
            };
            const expected = {
                primaryLinkUrl: 'www.example.com',
                primaryLinkLabel: 'Example',
                secondaryLinks: null
            };
            expect((0, _transformlinksvalueutil.transformLinksValue)({
                input
            })).toEqual(expected);
        });
        it('should preserve percent-encoded payloads when normalizing imported URLs', ()=>{
            const input = {
                primaryLinkUrl: 'https://www.google.com/maps/place/Birdie+-+Eventlocation/data=!4m7!3m6!1s0x479e7674e1702985:0xe482992505cb1ba4!8m2!3d48.1584971!4d11.5538261!16s%2Fg%2F1ptwh8096!19sChIJhSlw4XR2nkcRpBvLBSWZguQ?authuser=0&hl=en&rclk=1',
                primaryLinkLabel: 'Birdie',
                secondaryLinks: JSON.stringify([
                    {
                        url: 'https://example.com/test%2520name',
                        label: 'Encoded secondary link'
                    }
                ])
            };
            const expected = {
                primaryLinkUrl: 'https://www.google.com/maps/place/Birdie+-+Eventlocation/data=!4m7!3m6!1s0x479e7674e1702985:0xe482992505cb1ba4!8m2!3d48.1584971!4d11.5538261!16s%2Fg%2F1ptwh8096!19sChIJhSlw4XR2nkcRpBvLBSWZguQ?authuser=0&hl=en&rclk=1',
                primaryLinkLabel: 'Birdie',
                secondaryLinks: JSON.stringify([
                    {
                        url: 'https://example.com/test%2520name',
                        label: 'Encoded secondary link'
                    }
                ])
            };
            expect((0, _transformlinksvalueutil.transformLinksValue)({
                input
            })).toEqual(expected);
        });
    });
    describe('url-typed field', ()=>{
        it('should keep the whole url, exactly like a field with no variant set', ()=>{
            const input = {
                primaryLinkUrl: 'HTTPS://WWW.EXAMPLE.COM/careers?utm=1',
                primaryLinkLabel: 'Example',
                secondaryLinks: null
            };
            expect((0, _transformlinksvalueutil.transformLinksValue)({
                input,
                settings: {
                    type: 'url'
                }
            })).toEqual((0, _transformlinksvalueutil.transformLinksValue)({
                input
            }));
            expect((0, _transformlinksvalueutil.transformLinksValue)({
                input,
                settings: {
                    type: 'url'
                }
            })?.primaryLinkUrl).toBe('https://www.example.com/careers?utm=1');
        });
    });
    describe('domain-typed field', ()=>{
        const settings = {
            type: 'domain'
        };
        it('should reduce every spelling of a domain to the same value', ()=>{
            const spellings = [
                'example.com',
                'www.example.com',
                'HTTPS://WWW.EXAMPLE.COM/',
                'http://example.com/careers?utm=1'
            ];
            const normalized = spellings.map((primaryLinkUrl)=>(0, _transformlinksvalueutil.transformLinksValue)({
                    input: {
                        primaryLinkUrl,
                        primaryLinkLabel: 'Example'
                    },
                    settings
                })?.primaryLinkUrl);
            expect(normalized).toEqual([
                'example.com',
                'example.com',
                'example.com',
                'example.com'
            ]);
        });
        it('should reduce secondary links to bare domains', ()=>{
            const input = {
                primaryLinkUrl: 'https://example.com',
                primaryLinkLabel: 'Example',
                secondaryLinks: JSON.stringify([
                    {
                        url: 'https://www.example-old.com/about',
                        label: 'Old domain'
                    }
                ])
            };
            expect((0, _transformlinksvalueutil.transformLinksValue)({
                input,
                settings
            })).toEqual({
                primaryLinkUrl: 'example.com',
                primaryLinkLabel: 'Example',
                secondaryLinks: JSON.stringify([
                    {
                        url: 'example-old.com',
                        label: 'Old domain'
                    }
                ])
            });
        });
    });
});

//# sourceMappingURL=transform-links-value.util.spec.js.map