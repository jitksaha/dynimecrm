...
let WorkflowToolProvider = class WorkflowToolProvider {
    ...
    async generateDescriptors(context, options) {
        ...
        const icon = await (0, _resolveobjecticonutil.resolveObjectIcon)(this.flatEntityMapsCacheService, context.workspaceId, _types.CoreObjectNameSingular.Workflow);
        return (0, _toolsettodescriptorsutil.toolSetToDescriptors)(toolSet, _ai.ToolCategory.WORKFLOW, {
    ...
    async executeStaticTool(toolName, args, context) {
        const toolSet = await this.buildToolSet(context);
        if (!toolSet) {
            throw new Error(`Workflow tool service is not available (tool: ${toolName})`);
        }
        return (0, _executetoolfromtoolsetutil.executeToolFromToolSet)(toolSet, toolName, args, _ai.ToolCategory.WORKFLOW);
    }
    async buildToolSet(context) {
        if (!this.workflowToolService) {
            return null;
        }
        return this.workflowToolService.generateWorkflowTools(context.workspaceId, context.rolePermissionConfig);
    }
    constructor(workflowToolService, permissionsService, flatEntityMapsCacheService){
        this.workflowToolService = workflowToolService;
        this.permissionsService = permissionsService;
        this.flatEntityMapsCacheService = flatEntityMapsCacheService;
        this.category = _ai.ToolCategory.WORKFLOW;
    }
};
