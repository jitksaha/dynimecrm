...
describe('createLearnToolsTool', ()=>{
    ...
    it('surfaces "did you mean" suggestions when a tool name is not found', async ()=>{
        ...
        const learnTools = (0, _learntoolstool.createLearnToolsTool)(toolRegistry, context);
        ...
        expect(result.message).toContain('group_by_person (did you mean: group_by_people?)');
    });
    ...
    it('pluralizes the learned-tools count', async ()=>{
        ...
        expect(result.message).toBe('Learned 2 tools: find_many_people, group_by_people.');
    });
    ...
    it('keeps tools inline when the spill service leaves the output untouched', async ()=>{
        const spillToolOutputIfTooLarge = jest.fn().mockImplementation((output)=>Promise.resolve(output));
        const toolRegistry = {
            getToolInfo: jest.fn().mockResolvedValue([
                {
                    name: 'find_many_people',
...
//# sourceMappingURL=learn-tools.tool.spec.js.map