...
const _getmissingdraftemailscopesutil = require("./utils/get-missing-draft-email-scopes.util");
...
let DraftEmailTool = class DraftEmailTool {
    async execute(parameters, context) {
        try {
            ...
            const { data } = result;
            const missingDraftScopes = (0, _getmissingdraftemailscopesutil.getMissingDraftEmailScopes)(data.connectedAccount);
            if (missingDraftScopes.length > 0) {
                return {
                    success: false,
                    message: 'Failed to create draft due to insufficient permissions',
                    error: 'The connected email account does not have permission to create drafts.'
                };
            }
