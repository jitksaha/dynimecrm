...
const _searchoutpututil = require("../search-output.util");
...
describe('searchOutput', ()=>{
    it('returns each occurrence with a character window and offset', ()=>{
        ...
        expect(result.matches[0].charOffset).toBe(multiLineContent.indexOf('error'));
    ...
    it('paginates with offset', ()=>{
        ...
        expect(result.matches[0].charOffset).toBe(multiLineContent.lastIndexOf('error'));
        expect(result.hasMore).toBe(false);
    });
    ...
    it('truncates an overly long single match with a centered ellipsis', ()=>{
        const longMatch = 'z'.repeat(_searchoutputmaxmatchlengthconstant.SEARCH_OUTPUT_MAX_MATCH_LENGTH + 200);
        const result = (0, _searchoutpututil.searchOutput)({
            content: longMatch,
            pattern: 'z+',
            maxMatches: 10,
        ...
        expect(result.matches[0].match.length).toBeLessThanOrEqual(_searchoutputmaxmatchlengthconstant.SEARCH_OUTPUT_MAX_MATCH_LENGTH);
    });
    it('throws for an empty pattern instead of matching everything', ()=>{
        expect(()=>(0, _searchoutpututil.searchOutput)({
                content: multiLineContent,
                pattern: '',
                maxMatches: 10,
                offset: 0,
...
//# sourceMappingURL=search-output.util.spec.js.map