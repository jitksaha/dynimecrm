...
let ConfigStorageService = class ConfigStorageService {
    ...
    isSensitiveStringValue(value, key) {
        const metadata = this.getConfigMetadata(key);
        return typeof value === 'string' && metadata?.isSensitive === true && metadata.type === _configvariabletypeenum.ConfigVariableType.STRING;
    }
    async convertAndDecrypt(dbValue, key) {
        try {
            const convertedValue = this.configValueConverter.convertDbValueToAppValue(dbValue, key);
            if (this.isSensitiveStringValue(convertedValue, key) && (0, _isencryptedstringutil.isEncryptedString)(convertedValue)) {
                return this.secretEncryptionService.decryptVersionedOrThrow(convertedValue);
            }
    ...
    async convertAndEncrypt(appValue, key) {
        try {
            const convertedValue = this.configValueConverter.convertAppValueToDbValue(appValue, key);
            if (this.isSensitiveStringValue(convertedValue, key)) {
                return this.secretEncryptionService.encryptVersioned(convertedValue);
            }
            return convertedValue;
        } catch (error) {
            throw new _twentyconfigexception.ConfigVariableException(`Failed to convert value for key ${key}: ${error.message}`, _twentyconfigexception.ConfigVariableExceptionCode.VALIDATION_FAILED);
