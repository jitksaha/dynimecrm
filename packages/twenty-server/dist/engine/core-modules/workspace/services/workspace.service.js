...
const _workspacefieldsupdatablebeforeactivationconstant = require("../constants/workspace-fields-updatable-before-activation.constant");
const _workspacedeletionapplicationuninstalljob = require("../jobs/workspace-deletion-application-uninstall.job");
...
let WorkspaceService = class WorkspaceService {
    ...
    async activateWorkspace(user, workspace) {
        ...
        if ((activationLockResult.affected ?? 0) === 0) {
            activationLockResult = await this.workspaceRepository.update({
                id: workspace.id,
                activationStatus: _workspace.WorkspaceActivationStatus.ONGOING_CREATION,
                updatedAt: (0, _typeorm1.LessThan)(new Date(Date.now() - WORKSPACE_ACTIVATION_STALE_LOCK_TIMEOUT_MS))
            }, {
                activationStatus: _workspace.WorkspaceActivationStatus.ONGOING_CREATION
            });
        }
        if ((activationLockResult.affected ?? 0) === 0) {
            ...
            if (existingWorkspace?.activationStatus === _workspace.WorkspaceActivationStatus.ACTIVE || existingWorkspace?.activationStatus === _workspace.WorkspaceActivationStatus.CREATED) {
    ...
    async activateAndInitializeUpgradeState({ workspaceId }) {
        ...
        const hasWorkspaceAnySubscription = await this.billingService.hasWorkspaceAnySubscription(workspaceId);
        const queryRunner = this.coreDataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        try {
            await queryRunner.manager.update(_workspaceentity.WorkspaceEntity, workspaceId, {
                activationStatus: hasWorkspaceAnySubscription ? _workspace.WorkspaceActivationStatus.ACTIVE : _workspace.WorkspaceActivationStatus.CREATED
            });
    ...
    async suspendWorkspace(id) {
        ...
        const { affected } = await this.workspaceRepository.update({
            id,
            activationStatus: (0, _typeorm1.Not)(_workspace.WorkspaceActivationStatus.SUSPENDED),
    ...
    async reactivateWorkspace(id) {
        ...
        const hasBeenReactivated = (0, _utils.isDefined)(affected) && affected > 0;
    ...
    async hardDeleteWorkspaceWithApplicationUninstallLock(workspace) {
        const advisoryLockResult = await this.postgresAdvisoryLockService.tryWithLock((0, _getworkspaceapplicationuninstalllocknameutil.getWorkspaceApplicationUninstallLockName)(workspace.id), async ()=>{
            const userWorkspaces = await this.userWorkspaceRepository.find({
                where: {
                    workspaceId: workspace.id
                },
            ...
            for (const userWorkspace of userWorkspaces){
                await this.handleRemoveWorkspaceMember(workspace.id, userWorkspace.userId, false);
            }
            ...
            await this.workspaceSchemaService.deleteWorkspaceDBSchema(workspace.id);
            await this.workspaceCacheStorageService.flush(workspace.id);
            await this.flatEntityMapsCacheService.flushFlatEntityMaps({
                workspaceId: workspace.id
            });
            await this.deleteCascadeMessageQueueService.add(_fileworkspacefolderdeletionjob.FileWorkspaceFolderDeletionJob.name, {
                workspaceId: workspace.id
            });
            const emailingDomains = await this.coreDataSource.getRepository(_emailingdomainentity.EmailingDomainEntity).find({
                where: {
                    workspaceId: workspace.id
                }
            });
            await this.deleteCascadeMessageQueueService.add(_emailingdomainworkspacecleanupjob.EmailingDomainWorkspaceCleanupJob.name, {
                workspaceId: workspace.id,
        ...
        if (!advisoryLockResult.acquired) {
            throw new _workspaceexception.WorkspaceException(`Cannot hard delete workspace ${workspace.id} while application uninstall is running`, _workspaceexception.WorkspaceExceptionCode.APPLICATION_UNINSTALL_IN_PROGRESS);
        }
        return advisoryLockResult.value;
    }
    async enqueueWorkspaceDeletionApplicationUninstall(workspaceId) {
        await this.logicFunctionMessageQueueService.add(_workspacedeletionapplicationuninstalljob.WorkspaceDeletionApplicationUninstallJob.name, {
            workspaceId
        }, {
            id: `${_workspacedeletionapplicationuninstalljob.WorkspaceDeletionApplicationUninstallJob.name}-${workspaceId}`,
    ...
    async runPendingApplicationUninstallHooksBeforeHardDelete(workspace) {
    ...
    async deleteWorkspaceSyncableMetadataEntities(workspace) {
        const fieldMetadataIdChunks = await this.getFieldMetadataIdChunks(workspace.id);
        const queryRunner = this.coreDataSource.createQueryRunner();
        await queryRunner.connect();
        try {
            await queryRunner.startTransaction();
            for (const metadataName of _allmetadatanamessortedatomicallyconstant.ALL_METADATA_NAMES_SORTED_ATOMICALLY){
                if (metadataName === 'fieldMetadata') {
                    const deletedCount = await this.deleteFieldMetadataInChunks(queryRunner, workspace.id, fieldMetadataIdChunks);
                    if (deletedCount > 0) {
    ...
    async getFieldMetadataIdChunks(workspaceId) {
        const CHUNK_SIZE = 50;
    ...
    async deleteFieldMetadataInChunks(queryRunner, workspaceId, fieldMetadataIdChunks) {
        let totalDeleted = 0;
        for (const [index, chunk] of fieldMetadataIdChunks.entries()){
            ...
            this.logger.log(`workspace ${workspaceId}: fieldMetadata chunk ${index + 1}/${fieldMetadataIdChunks.length} - deleted ${deletedInChunk} record(s)`);
        }
        return totalDeleted;
    }
    ...
    async validateWorkspaceUpdatePermissions({ payload, userWorkspaceId, workspaceId, apiKey, workspaceActivationStatus }) {
        ...
        if (workspaceActivationStatus === _workspace.WorkspaceActivationStatus.PENDING_CREATION) {
            const fieldsRequiringActivation = fieldsBeingUpdated.filter((field)=>!(field in _workspacefieldsupdatablebeforeactivationconstant.WORKSPACE_FIELDS_UPDATABLE_BEFORE_ACTIVATION));
            if (fieldsRequiringActivation.length > 0) {
                ...
                throw new _permissionsexception.PermissionsException(_permissionsexception.PermissionsExceptionMessage.PERMISSION_DENIED, _permissionsexception.PermissionsExceptionCode.PERMISSION_DENIED, {
                    userFriendlyMessage: /*i18n*/ {
                        id: "Yi0Q/T",
                        message: "These fields cannot be updated before the workspace is activated: {fieldsList}.",
                        values: {
    ...
    async enqueuePreInstalledAppsInstallation(workspaceId) {
        try {
            await this.preInstalledAppsService.enqueueInstallOnWorkspace(workspaceId);
        } catch (error) {
            this.logger.error(`Non-critical: failed to enqueue pre-installed apps installation for workspace ${workspaceId}`, error);
            this.exceptionHandlerService.captureExceptions([
                error
            ]);
        }
    ...
    constructor(workspaceRepository, userRepository, userWorkspaceRepository, workspaceManagerService, featureFlagService, billingSubscriptionService, billingService, userWorkspaceService, twentyConfigService, exceptionHandlerService, permissionsService, dnsManagerService, flatEntityMapsCacheService, prefillLogicFunctionService, applicationService, preInstalledAppsService, workspaceMigrationValidateBuildAndRunService, workspaceCacheStorageService, subdomainManagerService, workspaceSchemaService, customDomainManagerService, fileCorePictureService, aiModelRegistryService, deleteCascadeMessageQueueService, logicFunctionMessageQueueService, coreDataSource, coreEntityCacheService, upgradeMigrationService, upgradeSequenceReaderService, sdkClientGenerationService, postgresAdvisoryLockService, applicationUninstallService){
        ...
        this.deleteCascadeMessageQueueService = deleteCascadeMessageQueueService;
        this.logicFunctionMessageQueueService = logicFunctionMessageQueueService;
        ...
        this.logger = new _common.Logger(WorkspaceService.name);
        this.WORKSPACE_FIELD_PERMISSIONS = {
            ...
            workspaceDiscoverability: _constants.PermissionFlagType.SECURITY,
...
WorkspaceService = _ts_decorate([
    ...
    _ts_param(24, (0, _messagequeuedecorator.InjectMessageQueue)(_messagequeueconstants.MessageQueue.logicFunctionQueue)),
    _ts_param(25, (0, _typeorm.InjectDataSource)()),
