...
let WorkspaceResolver = class WorkspaceResolver {
    ...
    async activateWorkspace(// Deprecated: the workspace name is set at creation. This argument is kept
    // for backward compatibility (removing it would be a breaking schema change)
    // but is ignored.
    _data, user, workspace) {
    ...
    async billingCustomer(workspace) {
        if (!this.twentyConfigService.isBillingEnabled()) {
            return null;
        }
...
_ts_decorate([
    (0, _graphql.ResolveField)(()=>_billingcustomerentity.BillingCustomerEntity, {
        nullable: true
    }),
    _ts_param(0, (0, _graphql.Parent)()),
    _ts_metadata("design:type", Function),
    ...
    _ts_metadata("design:returntype", Promise)
], WorkspaceResolver.prototype, "billingCustomer", null);
_ts_decorate([
