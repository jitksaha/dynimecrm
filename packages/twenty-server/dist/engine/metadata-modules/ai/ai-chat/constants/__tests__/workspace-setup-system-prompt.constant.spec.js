...
const _workspacesetupsystempromptconstant = require("../workspace-setup-system-prompt.constant");
describe('WORKSPACE_SETUP_SYSTEM_PROMPT', ()=>{
    const prompt = _workspacesetupsystempromptconstant.WORKSPACE_SETUP_SYSTEM_PROMPT;
    ...
    it('should state the two-call reply contract up front', ()=>{
        expect(prompt).toContain('While the setup runs, every reply of yours ends with one of two tool calls, ask_questions or complete_workspace_setup, as the final section below spells out');
    });
    it('should frame the first message as hidden company context', ()=>{
        expect(prompt).toContain('kicking off the setup of this brand-new workspace');
        expect(prompt).toContain('The first message of this conversation is not from the user');
    ...
    it('should absorb the skill-then-tools working loop from the base prompt', ()=>{
        ...
        expect(prompt).toContain('Never call a specialized tool without loading its matching skill first');
    ...
    it('should absorb the database, batching, and retry rules from the base prompt', ()=>{
        ...
        expect(prompt).toContain('http_request is only for external third-party APIs');
        ...
        expect(prompt).toContain('never create another navigation item for an object you just made');
    });
    it('should drop the base rules that fight the setup flow', ()=>{
        expect(prompt).not.toContain('trivial choices that have an obvious default');
        expect(prompt).not.toContain('the general guidance about skipping questions with obvious defaults');
    ...
    it('should instruct a text-first first reply with the mandatory ask_questions call', ()=>{
        ...
        expect(prompt).toContain('reply is unfinished until the ask_questions call is made');
        expect(prompt).toContain('needs no skill and no learn_tools step');
        expect(prompt).toContain('do not call load_skills, learn_tools, execute_tool, or web search');
    ...
    it('should branch the first reply on what the first message contains', ()=>{
        ...
        expect(prompt).toContain('the two most widely used when you know nothing about them');
    ...
    it('should open with the migration-or-scratch question as a tool call', ()=>{
        ...
        expect(prompt).toContain('Then stop writing and make the ask_questions call');
    ...
    it('should ask about the data model with the ask_questions tool', ()=>{
        ...
        expect(prompt).toContain('The turn is unfinished until you call ask_questions asking whether to go ahead and build it');
    ...
    it('should require explicit approval before building and name the metadata tools', ()=>{
        ...
        expect(prompt).toContain('never create, update, or delete anything before approval');
    ...
    it('should route what comes next through a mandatory ask_questions call', ()=>{
        ...
        expect(prompt).toContain('Never stop after that report: the turn is unfinished until you call ask_questions letting them pick what comes next');
    ...
    it('should offer the remaining capabilities plus finishing the setup, never recommending the finish', ()=>{
        expect(prompt).toContain('one line to each capability they do not have yet');
        expect(prompt).toContain('naming the thing in their business it improves');
        expect(prompt).toContain('exactly the capabilities they do not have yet plus finishing the setup');
    ...
    it('should repeat the same question after every build and skip it once nothing is left', ()=>{
        expect(prompt).toContain('then report it and ask again the same way with what is left');
    ...
    it('should treat the pick as the approval and never confirm it', ()=>{
        ...
        expect(prompt).toContain('when they pick finishing it, without asking them to confirm');
        expect(prompt).toContain('When they answer in free text instead, do what they asked, then come back to the question');
    });
    ...
    it('should end the setup with a written close followed by the completion call', ()=>{
        ...
        expect(prompt).toContain('The setup ends the moment they are done, whether they tell you so in their own words or pick the finishing option');
        ...
        expect(prompt).toContain('Only then, as the last thing in the reply, you call complete_workspace_setup');
    ...
    it('should close without pitching what they did not pick', ()=>{
        ...
        expect(prompt).not.toContain('never close while they are still unaware of the rest');
    ...
    it('should carry the ask_questions shape rules the base prompt no longer provides', ()=>{
        ...
        expect(prompt).toContain('a question mark in your text means the call is missing');
        expect(prompt).toContain('never ask it for information you can look up with a tool');
        expect(prompt).toContain('a short header, its question, and 2 to 4 short options');
    ...
    it('should end every setup reply on one of the two calls, after its text', ()=>{
        ...
        expect(prompt).toContain('ends in exactly one of two ways: the ask_questions call, or the complete_workspace_setup call');
        expect(prompt).toContain('The only exception is the CSV upload request');
        expect(prompt).toContain('come after the text of that reply, never instead of it');
        expect(prompt).toContain('while anything is still worth building, it is the ask_questions call');
        expect(prompt).toContain('whether they picked the finishing option or told you so in their own words, it is complete_workspace_setup');
    });
});

//# sourceMappingURL=workspace-setup-system-prompt.constant.spec.js.map