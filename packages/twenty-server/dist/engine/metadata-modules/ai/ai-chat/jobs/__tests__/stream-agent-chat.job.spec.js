...
const _streamagentchatjob = require("../stream-agent-chat.job");
...
const PENDING_QUESTION_RESPONSE_MESSAGE = {
    role: 'assistant',
    parts: [
        {
...
const createFakeChatStream = ({ chunks = TEXT_CHUNKS, responseMessage = RESPONSE_MESSAGE, midStreamError, onFirstChunk, isAborted = false } = {})=>({
        toUIMessageStream: (options)=>new ReadableStream({
...
describe('StreamAgentChatJob', ()=>{
    ...
    const buildJob = ({ workspaceFound = true, chatStream = createFakeChatStream(), streamChatRejection, addMessageRejection, assistantPersistRejection, totalsUpdateAffected = 1, finalPublishRejection } = {})=>{
        ...
        const threadRepository = {
            ...
            update: jest.fn().mockImplementation((_workspaceId, _criteria, values)=>Promise.resolve({
                    affected: values && typeof values.totalInputTokens === 'function' ? totalsUpdateAffected : 1
                }))
        };
        ...
        const agentChatService = {
            addMessage: addMessageRejection ? jest.fn().mockRejectedValue(addMessageRejection) : jest.fn().mockResolvedValue({
                id: 'assistant-message-id'
            }),
            upsertAssistantMessage: assistantPersistRejection ? jest.fn().mockRejectedValue(assistantPersistRejection) : jest.fn().mockResolvedValue(undefined),
        ...
        const chatExecutionService = {
            streamChat: streamChatRejection ? jest.fn().mockRejectedValue(streamChatRejection) : jest.fn().mockResolvedValue({
                stream: chatStream,
                modelConfig: {
        ...
        const eventPublisherService = {
            ...
            publish: jest.fn().mockImplementation(({ event })=>{
                if ((0, _utils.isDefined)(finalPublishRejection) && event.type === 'message-persisted') {
        ...
        const job = new _streamagentchatjob.StreamAgentChatJob(threadRepository, workspaceRepository, agentChatService, chatExecutionService, eventPublisherService, cancelSubscriberService, agentChatStreamingService, streamHeartbeatService, metricsService, aiModelRegistryService);
        const turnCounts = (key)=>metricsService.incrementCounterBy.mock.calls.map(([call])=>call).filter((call)=>call.key === key);
        return {
            job,
            publishedEvents,
            threadRepository,
    ...
    it('publishes all chunks in order with message-persisted last on success', async ()=>{
        const { job, publishedEvents, threadRepository, agentChatService, agentChatStreamingService } = buildJob();
        await job.handle(jobData);
        const chunkEvents = publishedEvents.filter((event)=>event.type === 'stream-chunk');
        expect(chunkEvents).toHaveLength(TEXT_CHUNKS.length);
        expect(publishedEvents[publishedEvents.length - 1]).toMatchObject({
            type: 'message-persisted'
        });
        expect(agentChatService.upsertAssistantMessage).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('never publishes the opaque error chunk to subscribers', async ()=>{
        ...
        const chunkTypes = publishedEvents.filter((event)=>event.type === 'stream-chunk').map((event)=>event.chunk.type);
    ...
    it('rejects, persists the error, and unblocks the thread when the model stream fails mid-stream', async ()=>{
        ...
        expect(publishedEvents.map((event)=>event.type)).not.toContain('message-persisted');
        expect(threadRepository.update).toHaveBeenCalledWith('workspace-id', {
            id: 'thread-id'
        }, {
    ...
    it('rejects promptly when execution setup throws instead of hanging until the queue lock expires', async ()=>{
        ...
        await expect(job.handle(jobData)).rejects.toThrow('model resolution failed');
        expect(publishedEvents[publishedEvents.length - 2]).toMatchObject({
            type: 'stream-error',
    ...
    it('persists the error and unblocks the thread when the workspace is missing', async ()=>{
        ...
        await expect(job.handle(jobData)).rejects.toMatchObject({
            code: _aiexception.AiExceptionCode.WORKSPACE_NOT_FOUND
        });
        expect(publishedEvents[publishedEvents.length - 2]).toMatchObject({
            type: 'stream-error',
            code: _aiexception.AiExceptionCode.WORKSPACE_NOT_FOUND
    ...
    it('bails out without streaming when the thread no longer holds the claim for this stream', async ()=>{
        const { job, publishedEvents, threadRepository, eventPublisherService, agentChatStreamingService } = buildJob();
    ...
    it('bails out without streaming when the thread was deleted', async ()=>{
        const { job, publishedEvents, threadRepository, eventPublisherService } = buildJob();
    ...
    it('persists the interrupted error and publishes the terminal sequence when aborted by a worker shutdown', async ()=>{
        let triggerShutdown;
        const { job, publishedEvents, threadRepository, agentChatStreamingService } = buildJob({
            chatStream: createFakeChatStream({
        ...
        await expect(job.handle(jobData, {
    ...
    it('keeps user-cancel semantics when a shutdown signal is wired but never aborted', async ()=>{
        let triggerUserCancel;
        const { job, publishedEvents, agentChatStreamingService, cancelCallbacks } = buildJob({
            chatStream: createFakeChatStream({
        ...
        expect(publishedEvents.map((event)=>event.type)).not.toContain('stream-error');
        expect(agentChatStreamingService.flushNextQueuedMessage).not.toHaveBeenCalled();
    });
    it('resolves without flushing the queue when the stream is cancelled', async ()=>{
        let triggerCancel;
        const { job, publishedEvents, agentChatService, agentChatStreamingService, cancelCallbacks } = buildJob({
            chatStream: createFakeChatStream({
    ...
    it('labels turn-started with the resolved model, not the auto-select id', async ()=>{
        ...
        expect(aiModelRegistryService.getEffectiveModelConfig).toHaveBeenCalledWith('default-fast-model');
    ...
    it('falls back to the workspace default model when the turn did not pick one', async ()=>{
        ...
        await job.handle(jobData);
        expect(aiModelRegistryService.getEffectiveModelConfig).toHaveBeenCalledWith('default-smart-model');
    });
    ...
    it('records exactly one outcome for every started turn', async ()=>{
        ...
        for (const scenario of scenarios){
            ...
            const terminal = scenario.turnCounts('ai-chat/turn-completed').length + scenario.turnCounts('ai-chat/turn-cancelled').length + scenario.turnCounts('ai-chat/turn-failed').length;
            expect({
                started,
                terminal
            }).toEqual({
...
//# sourceMappingURL=stream-agent-chat.job.spec.js.map