...
describe('AgentChatStreamingService claim & reap', ()=>{
    ...
    const buildService = ({ thread = idleThread, claimAffected = 1, queuedMessages = [], heartbeatAlive = true } = {})=>{
        ...
        const agentChatService = {
            ...
            hasQueuedMessages: jest.fn().mockImplementation(()=>Promise.resolve(queuedMessages.length > 0)),
        ...
        }, messageQueueService, agentChatService, eventPublisherService, {
    ...
    describe('streamAgentChat', ()=>{
        it('claims the thread conditionally before enqueueing', async ()=>{
            const { service, threadRepository, streamHeartbeatService } = buildService();
            ...
            expect(streamHeartbeatService.markClaimed.mock.invocationCallOrder[0]).toBeLessThan(threadRepository.update.mock.invocationCallOrder[0]);
        });
        ...
        it('queues behind a halted backlog and kicks the drain from the front', async ()=>{
            ...
            expect(result.queued).toBe(true);
            expect(agentChatService.queueMessage).toHaveBeenCalled();
            expect(agentChatService.promoteQueuedMessage).toHaveBeenCalledWith(expect.objectContaining({
                messageId: 'older-queued-id'
            }));
        });
        ...
        it('releases the claim when enqueueing the job fails', async ()=>{
            const { service, threadRepository, messageQueueService, streamHeartbeatService } = buildService();
            messageQueueService.add.mockRejectedValue(new Error('redis down'));
            await expect(service.streamAgentChat(sendArguments)).rejects.toThrow('redis down');
    ...
    describe('reapDeadStream', ()=>{
        ...
        it('converts a heartbeat-less claim into a retryable interrupted error', async ()=>{
            const { service, threadRepository, eventPublisherService, publishedEvents } = buildService({
            ...
            expect(eventPublisherService.resetStreamState).toHaveBeenCalledWith('thread-id');
...
//# sourceMappingURL=agent-chat-streaming.service.claim.spec.js.map