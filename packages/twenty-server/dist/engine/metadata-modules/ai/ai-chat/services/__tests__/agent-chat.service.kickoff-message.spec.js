...
const buildService = ({ existingHiddenMessage = null } = {})=>{
    ...
    const service = new _agentchatservice.AgentChatService(threadRepository, turnRepository, messageRepository, messagePartRepository, {}, {}, {}, {});
    return {
        service,
        messageRepository,
        turnRepository,
...
const ensureKickoff = (service)=>service.ensureHiddenKickoffMessage({
        threadId: THREAD_ID,
...
describe('AgentChatService ensureHiddenKickoffMessage', ()=>{
    ...
    it('should insert a hidden kickoff message with its own turn when none exists', async ()=>{
        const { service, messageRepository, turnRepository, messagePartRepository } = buildService();
        ...
        expect(messageRepository.insert).toHaveBeenCalledWith(WORKSPACE_ID, expect.objectContaining({
            threadId: THREAD_ID,
...
describe('AgentChatService hasConversationMessages', ()=>{
    ...
    it('should return true when a visible message exists', async ()=>{
        ...
        await expect(service.hasConversationMessages({
            threadId: THREAD_ID,
            workspaceId: WORKSPACE_ID
        })).resolves.toBe(true);
        expect(messageRepository.findOne).toHaveBeenCalledWith(WORKSPACE_ID, {
...
//# sourceMappingURL=agent-chat.service.kickoff-message.spec.js.map