...
const _buildworkspacesetupkickoffmessagetextutil = require("../build-workspace-setup-kickoff-message-text.util");
...
const buildMessage = (overrides = {})=>(0, _buildworkspacesetupkickoffmessagetextutil.buildWorkspaceSetupKickoffMessageText)({
        companyEnrichment,
...
describe('buildWorkspaceSetupKickoffMessageText', ()=>{
    it('should carry the company context when a full enrichment is provided', ()=>{
        ...
        expect(result).toContain((0, _buildcompanycontextmessagetextutil.buildCompanyContextMessageText)(companyEnrichment));
    ...
    it('should state that no company information is available when the enrichment is null', ()=>{
        ...
        expect(result).toContain('No information about the company that owns this workspace is available.');
    ...
    it('should carry the person context when a person enrichment is provided', ()=>{
        ...
        expect(result).toContain((0, _buildpersoncontextmessagetextutil.buildPersonContextMessageText)(personEnrichment));
    ...
    it('should state that no person information is available when the person enrichment is null', ()=>{
        const result = buildMessage();
        expect(result).toContain('No third-party information about the person setting up this workspace is available.');
    });
    it('should always carry the workspace context line', ()=>{
        const result = buildMessage();
        expect(result).toContain('This workspace is named "Acme" (subdomain: acme). The admin setting it up signed up with admin@acme.com.');
    });
    ...
    ])('should end with the locale instruction when the locale is %s', (locale, languageName)=>{
        const result = buildMessage({
            locale
        });
        expect(result.endsWith(`The user locale is ${languageName}, please continue the discussion in that language.`)).toBe(true);
    });
    it('should fall back to the raw locale when it is not a structurally valid language tag', ()=>{
        const result = buildMessage({
            locale: '!'
        });
        expect(result).toContain('The user locale is !, please continue the discussion in that language.');
    });
});

//# sourceMappingURL=build-workspace-setup-kickoff-message-text.util.spec.js.map