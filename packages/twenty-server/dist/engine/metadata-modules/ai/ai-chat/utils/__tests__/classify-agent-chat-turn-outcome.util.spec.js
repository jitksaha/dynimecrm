...
const _classifyagentchatturnoutcomeutil = require("../classify-agent-chat-turn-outcome.util");
const baseTurn = {
    hasText: true,
    isAborted: false,
...
describe('classifyAgentChatTurnOutcome', ()=>{
    it('counts a turn that replied with text as answered', ()=>{
        expect((0, _classifyagentchatturnoutcomeutil.classifyAgentChatTurnOutcome)(baseTurn)).toEqual({
            kind: 'completed',
            outcome: 'answered'
        });
    });
    it('counts a turn that ended on a question as completed, not abandoned', ()=>{
        expect((0, _classifyagentchatturnoutcomeutil.classifyAgentChatTurnOutcome)({
            ...baseTurn,
            hasText: false,
...
describe('resolveSupersededTurnOutcome', ()=>{
    it('keeps a credits failure rather than filing it as a cancellation', ()=>{
        expect((0, _classifyagentchatturnoutcomeutil.resolveSupersededTurnOutcome)({
            kind: 'failed',
            failurePhase: 'credits_exhausted'
        })).toEqual({
            kind: 'failed',
...
//# sourceMappingURL=classify-agent-chat-turn-outcome.util.spec.js.map