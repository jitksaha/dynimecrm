...
const addFlatEntityToFlatEntityAndRelatedEntityMapsThroughMutationOrThrow = ({ metadataName, flatEntity, flatEntityAndRelatedMapsToMutate, skipMissingRelatedEntities = false })=>{
    ...
    for (const relationPropertyName of Object.keys(manyToOneRelations)){
        ...
        if (skipMissingRelatedEntities) {
            const maybeRelatedFlatEntity = (0, _findflatentitybyidinflatentitymapsutil.findFlatEntityByIdInFlatEntityMaps)({
                flatEntityId: flatEntityRelatedEntityForeignKeyValue,
                flatEntityMaps: relatedFlatEntityMetadataMaps
            });
            if (!(0, _utils.isDefined)(maybeRelatedFlatEntity)) {
                continue;
            }
        }
        ...
        if (!Object.prototype.hasOwnProperty.call(relatedFlatEntity, flatEntityForeignKeyAggregator)) {
            throw new _flatentitymapsexception.FlatEntityMapsException(`Should never occur, invalid flat entity typing. flat ${relatedMetadataName} should contain ${String(flatEntityForeignKeyAggregator)} (metadataName: ${metadataName}, universalIdentifier: ${flatEntity.universalIdentifier})`, _flatentitymapsexception.FlatEntityMapsExceptionCode.ENTITY_MALFORMED, {
                context: {
                    id: flatEntity.id,
