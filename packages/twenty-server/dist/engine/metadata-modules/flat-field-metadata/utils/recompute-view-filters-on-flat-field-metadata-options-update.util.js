...
const _getinvalidselectfilteroptionvaluesutil = require("./get-invalid-select-filter-option-values.util");
const recomputeViewFiltersOnFlatFieldMetadataOptionsUpdate = ({ flatViewFilterMaps, fromFlatFieldMetadata, toOptions })=>{
    ...
    for (const viewFilter of flatViewFilters){
        ...
        if (!(0, _utils.isDefined)(rawViewFilterValue) || !(0, _utils.isRecordFilterOperandExpectingValue)(viewFilter.operand) || (0, _guards.isNonEmptyString)(viewFilter.subFieldName)) {
            continue;
        }
        const viewFilterValue = (0, _getinvalidselectfilteroptionvaluesutil.normalizeSelectFilterValues)(rawViewFilterValue);
