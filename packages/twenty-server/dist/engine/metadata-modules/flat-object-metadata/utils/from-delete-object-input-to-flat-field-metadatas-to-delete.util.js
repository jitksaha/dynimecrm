...
const fromDeleteObjectInputToFlatFieldMetadatasToDelete = ({ deleteObjectInput: rawDeleteObjectInput, flatFieldMetadataMaps, flatObjectMetadataMaps, flatIndexMaps })=>{
    ...
    const flatIndexMetadataToDelete = Object.values(flatIndexMaps.byUniversalIdentifier).filter((flatIndex)=>(0, _utils.isDefined)(flatIndex) && !(flatIndex.isSystemSideEffect === true && flatIndex.objectMetadataId === flatObjectMetadataToDelete.id) && (flatIndex.objectMetadataId === flatObjectMetadataToDelete.id || flatIndex.flatIndexFieldMetadatas.some((flatIndexField)=>fieldIdsToDelete.has(flatIndexField.fieldMetadataId))));
    return {
