...
const recomputeViewFieldIdentifierAfterFlatObjectIdentifierUpdate = ({ existingFlatObjectMetadata, flatViewFieldMaps, flatViewMaps, flatFieldMetadataMaps, updatedLabelIdentifierFieldMetadataId })=>{
    ...
    }).filter((flatView)=>flatView.type !== _types.ViewType.FIELDS_WIDGET);
    ...
    for (const flatView of flatViews){
        ...
        } else if (labelMetadataIdentifierViewField.position > lowestViewFieldPosition || labelMetadataIdentifierViewField.isVisible === false) {
            const updatedFlatViewField = {
                ...labelMetadataIdentifierViewField,
                position: labelMetadataIdentifierViewField.position > lowestViewFieldPosition ? lowestViewFieldPosition - 1 : labelMetadataIdentifierViewField.position,
                isVisible: true
            };
