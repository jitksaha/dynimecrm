...
const _eventlogliveservice = require("../../core-modules/event-logs/live/event-log-live.service");
...
let LogicFunctionResolver = class LogicFunctionResolver {
    ...
    async executeOneLogicFunction({ id, payload }, { id: workspaceId }, { id: userId }, userWorkspaceId) {
        try {
    ...
    async logicFunctionLogs(_, workspace) {
        // Register CLI presence (refreshed by the heartbeat) so the executor only publishes when watched.
        await this.eventLogLiveService.markWatched(workspace.id, _subscriptionchannelenum.SubscriptionChannel.LOGIC_FUNCTION_LOGS_CHANNEL);
    ...
    constructor(logicFunctionFromSourceService, flatEntityMapsCacheService, subscriptionService, eventLogLiveService){
