...
let MessageFolderMetadataService = class MessageFolderMetadataService {
    ...
    async setSyncStatus({ ids, workspaceId, data }) {
        await this.repository.manager.transaction(async (manager)=>{
            if (!data.isSynced) {
                await manager.update(_messagefolderentity.MessageFolderEntity, {
                ...
                await manager.update(_messagefolderentity.MessageFolderEntity, {
                    id: (0, _typeorm1.In)(ids),
                    workspaceId,
                    pendingSyncAction: _types.MessageFolderPendingSyncAction.FOLDER_IMPORT
                }, {
                    pendingSyncAction: _types.MessageFolderPendingSyncAction.NONE
                });
                return;
            }
            const folderIdsToBackfill = (await manager.find(_messagefolderentity.MessageFolderEntity, {
                where: {
                    id: (0, _typeorm1.In)(ids),
                    workspaceId,
                    isSynced: false,
            ...
            if (folderIdsToBackfill.length > 0) {
                await manager.update(_messagefolderentity.MessageFolderEntity, {
                    id: (0, _typeorm1.In)(folderIdsToBackfill),
                    workspaceId
                }, {
                    pendingSyncAction: _types.MessageFolderPendingSyncAction.FOLDER_IMPORT
                });
