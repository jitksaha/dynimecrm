...
const toUniversalFlatFieldPermissionToDelete = (fieldPermission)=>({
        universalIdentifier: fieldPermission.universalIdentifier,
        applicationUniversalIdentifier: fieldPermission.applicationUniversalIdentifier,
        roleUniversalIdentifier: fieldPermission.roleUniversalIdentifier,
        objectMetadataUniversalIdentifier: fieldPermission.objectMetadataUniversalIdentifier,
        fieldMetadataUniversalIdentifier: fieldPermission.fieldMetadataUniversalIdentifier,
...
let FieldPermissionService = class FieldPermissionService {
    async upsertFieldPermissions({ workspaceId, input }) {
        ...
        for (const [key, desired] of desiredMap){
            ...
            if (!(0, _utils.isDefined)(current)) {
                ...
                if (!(0, _utils.isDefined)(canReadFieldValue) && !(0, _utils.isDefined)(canUpdateFieldValue)) {
                    continue;
                }
                ...
                if (!(0, _utils.isDefined)(effectiveCanRead) && !(0, _utils.isDefined)(effectiveCanUpdate)) {
                    ...
                    const isForeignMirroredRow = mirroredFieldKeys.has(key) && !inputFieldKeys.has(key) && current.applicationUniversalIdentifier !== workspaceCustomFlatApplication.universalIdentifier;
                    if (!isForeignMirroredRow) {
                        flatEntityToDelete.push(toUniversalFlatFieldPermissionToDelete(current));
                    }
                    continue;
                }
    ...
    addRelatedFieldPermissionsToDesired({ desiredMap, inputFieldPermissions, flatFieldMetadataMaps }) {
        ...
        for (const fieldPermission of inputFieldPermissions){
            ...
            const sourceIsCleared = !(0, _utils.isDefined)(fieldPermission.canReadFieldValue) && !(0, _utils.isDefined)(fieldPermission.canUpdateFieldValue);
            desiredMap.set(targetKey, {
                objectMetadataId: targetObjectId,
                fieldMetadataId: targetFieldId,
                canReadFieldValue: sourceIsCleared ? null : fieldPermission.canReadFieldValue,
                canUpdateFieldValue: sourceIsCleared ? null : fieldPermission.canUpdateFieldValue
            });
