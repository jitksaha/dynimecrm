...
let PageLayoutTabService = class PageLayoutTabService {
    async findByPageLayoutId({ workspaceId, pageLayoutId }) {
        ...
        return Object.values(flatPageLayoutTabMaps.byUniversalIdentifier).filter(_utils.isDefined).filter((tab)=>tab.pageLayoutId === pageLayoutId && !(0, _utils.isDefined)(tab.deletedAt)).sort((a, b)=>(a.position ?? 0) - (b.position ?? 0) || a.id.localeCompare(b.id)).map((tab)=>(0, _fromflatpagelayouttabwithwidgetstopagelayouttabdtoutil.fromFlatPageLayoutTabWithWidgetsToPageLayoutTabDto)((0, _reconstructflatpagelayouttabwithwidgetsutil.reconstructFlatPageLayoutTabWithWidgets)({
                tab,
