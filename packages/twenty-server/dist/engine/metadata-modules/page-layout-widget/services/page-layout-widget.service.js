...
const _validatefieldconfigurationnestedrelationutil = require("../utils/validate-field-configuration-nested-relation.util");
...
let PageLayoutWidgetService = class PageLayoutWidgetService {
    ...
    async validateConfigurationFieldReferences({ configuration, objectMetadataId, widgetTitle, workspaceId }) {
    ...
    async findByPageLayoutTabId({ workspaceId, pageLayoutTabId }) {
        ...
        return Object.values(flatPageLayoutWidgetMaps.byUniversalIdentifier).filter(_utils.isDefined).filter((widget)=>(0, _resolveoverridableentitypropertyutil.resolveOverridableEntityProperty)(widget, 'pageLayoutTabId') === pageLayoutTabId && !(0, _utils.isDefined)(widget.deletedAt)).sort((widgetA, widgetB)=>{
            const createdAtComparison = new Date(widgetA.createdAt).getTime() - new Date(widgetB.createdAt).getTime();
            return createdAtComparison || widgetA.id.localeCompare(widgetB.id);
        }).map(_fromflatpagelayoutwidgettopagelayoutwidgetdtoutil.fromFlatPageLayoutWidgetToPageLayoutWidgetDto);
    }
    ...
    async update({ id, workspaceId, updateData }) {
        ...
        const shouldValidateConfigurationFields = isConfigurationBeingUpdated || Object.prototype.hasOwnProperty.call(updateData, 'objectMetadataId') || Object.prototype.hasOwnProperty.call(updateData, 'type');
