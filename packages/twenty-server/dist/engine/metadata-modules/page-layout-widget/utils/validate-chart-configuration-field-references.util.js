...
const _getinvalidselectfilteroptionvaluesutil = require("../../flat-field-metadata/utils/get-invalid-select-filter-option-values.util");
...
const validateSelectFilterOptionsOrThrow = ({ recordFilter, filterField, widgetTitle })=>{
    if (!(0, _utils.isDefined)(recordFilter.value)) {
        return;
    }
    ...
    const invalidValuesText = invalidValues.map((value)=>`"${value}"`).join(', ');
    const allowedValuesText = filterField.options?.map((option)=>option.value).map((optionValue)=>`"${optionValue}"`).join(', ');
    throw buildChartFieldValidationException(`Filter on "${filterField.label}" uses option(s) ${invalidValuesText} that do not exist. Allowed values: ${allowedValuesText}.`, widgetTitle);
};
const validateChartConfigurationFieldReferencesOrThrow = ({ widgetConfiguration, widgetObjectMetadataId, widgetTitle, flatFieldMetadataMaps, flatObjectMetadataMaps })=>{
    ...
    if ((0, _utils.isDefined)(widgetConfiguration.filter?.recordFilters)) {
        for (const recordFilter of widgetConfiguration.filter.recordFilters){
            ...
            if ((0, _isflatfieldmetadataoftypeutil.isFlatFieldMetadataOfType)(filterField, _types.FieldMetadataType.SELECT) || (0, _isflatfieldmetadataoftypeutil.isFlatFieldMetadataOfType)(filterField, _types.FieldMetadataType.MULTI_SELECT)) {
