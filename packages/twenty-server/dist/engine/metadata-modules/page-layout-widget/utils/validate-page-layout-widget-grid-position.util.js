    ...
    for (const [property, minimum] of [
        ...
        if (!Number.isInteger(position[property]) || position[property] < minimum) {
            errors.push({
                ...
                message: (0, _pagelayoutwidgetexception.generatePageLayoutWidgetExceptionMessage)(_pagelayoutwidgetexception.PageLayoutWidgetExceptionMessageKey.INVALID_WIDGET_POSITION, widgetTitle, undefined, `${property} must be an integer greater than or equal to ${minimum}`),
