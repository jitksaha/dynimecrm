...
const _compareflatpagelayoutsbycreationutil = require("../utils/compare-flat-page-layouts-by-creation.util");
...
let PageLayoutService = class PageLayoutService {
    async findByWorkspaceId(workspaceId) {
        ...
        const activeLayouts = Object.values(flatPageLayoutMaps.byUniversalIdentifier).filter(_utils.isDefined).filter((layout)=>!(0, _utils.isDefined)(layout.deletedAt)).sort(_compareflatpagelayoutsbycreationutil.compareFlatPageLayoutsByCreation);
    ...
    async findBy({ workspaceId, filter: { objectMetadataId, pageLayoutType } }) {
        const { flatObjectMetadataMaps, flatPageLayoutMaps, flatPageLayoutTabMaps, flatPageLayoutWidgetMaps } = await this.getPageLayoutFlatEntityMaps(workspaceId);
    ...
    async findManyPaginated({ workspaceId, objectMetadataId, pageLayoutType, pagination }) {
        const { flatObjectMetadataMaps, flatPageLayoutMaps, flatPageLayoutTabMaps, flatPageLayoutWidgetMaps } = await this.getPageLayoutFlatEntityMaps(workspaceId);
        const activeLayouts = this.findActiveFlatPageLayouts({
            objectMetadataId,
            pageLayoutType,
        ...
        return {
            ...page,
            items: page.items.map((layout)=>(0, _fromflatpagelayoutwithtabsandwidgetstopagelayoutdtoutil.fromFlatPageLayoutWithTabsAndWidgetsToPageLayoutDto)((0, _reconstructflatpagelayoutwithtabsandwidgetsutil.reconstructFlatPageLayoutWithTabsAndWidgets)({
                    layout,
                    flatPageLayoutTabMaps,
                    flatPageLayoutWidgetMaps
                }))),
    ...
    findActiveFlatPageLayouts({ objectMetadataId, pageLayoutType, flatObjectMetadataMaps, flatPageLayoutMaps }) {
        const candidateFlatPageLayouts = (0, _guards.isNonEmptyString)(objectMetadataId) ? this.findFlatPageLayoutsByObjectMetadataId({
            objectMetadataId,
            flatObjectMetadataMaps,
            flatPageLayoutMaps
        }) : Object.values(flatPageLayoutMaps.byUniversalIdentifier).filter(_utils.isDefined);
        return candidateFlatPageLayouts.filter((layout)=>!(0, _utils.isDefined)(layout.deletedAt) && (!(0, _utils.isDefined)(pageLayoutType) || layout.type === pageLayoutType)).sort(_compareflatpagelayoutsbycreationutil.compareFlatPageLayoutsByCreation);
    }
    findFlatPageLayoutsByObjectMetadataId({ objectMetadataId, flatObjectMetadataMaps, flatPageLayoutMaps }) {
    ...
    async destroy({ id, workspaceId, isLinkedDashboardAlreadyDestroyed = false }) {
        ...
        if (flatPageLayoutToDestroy.type === _types.PageLayoutType.DASHBOARD && !isLinkedDashboardAlreadyDestroyed) {
    ...
    constructor(workspaceOrmManager, workspaceMigrationValidateBuildAndRunService, workspaceManyOrAllFlatEntityMapsCacheService, applicationService, dashboardSyncService){
