...
const _applymetadatafiltertoquerybuilderutil = require("../apply-metadata-filter-to-query-builder.util");
let FakeWhereBuilder = class FakeWhereBuilder {
    ...
    render() {
        return this.conditions.map((condition, index)=>index === 0 ? condition.replace(/^(AND|OR) /, '') : condition).join(' ');
    }
    append(operator, condition, parameters) {
        if (condition instanceof _typeorm.Brackets) {
...
const applyFilter = (filter, columnByFilterField = {
    id: {
        column: 'id',
        type: 'uuid'
    },
...
describe('applyMetadataFilterToQueryBuilder', ()=>{
    it('translates eq comparisons into parameterized conditions', ()=>{
        ...
        expect(whereBuilder.render()).toBe('"entity"."id" = :metadataFilterParameter0');
    ...
    it('combines multiple comparisons on one field with AND', ()=>{
        ...
        expect(whereBuilder.render()).toBe('"entity"."id" > :metadataFilterParameter0 AND "entity"."id" < :metadataFilterParameter1');
    });
    it('translates in comparisons and renders an empty in as a false condition', ()=>{
        ...
        }).render()).toBe('"entity"."id" IN (:...metadataFilterParameter0)');
        expect(applyFilter({
            id: {
                in: []
            }
    ...
    it('translates is and isNot including explicit null', ()=>{
        ...
        }).render()).toBe('"entity"."isActive" IS TRUE');
        expect(applyFilter({
            isActive: {
                is: null
            }
        }).render()).toBe('"entity"."isActive" IS NULL');
        expect(applyFilter({
            isActive: {
                isNot: false
            }
        }).render()).toBe('"entity"."isActive" IS NOT FALSE');
    });
    it('inverts boolean values for columns flagged with invertBooleanValues', ()=>{
        ...
        }, columnByFilterField).render()).toBe('"entity"."isUIEditable" IS FALSE');
        expect(applyFilter({
            isUIReadOnly: {
                isNot: false
            }
        }, columnByFilterField).render()).toBe('"entity"."isUIEditable" IS NOT TRUE');
        expect(applyFilter({
            isUIReadOnly: {
                is: null
            }
        }, columnByFilterField).render()).toBe('"entity"."isUIEditable" IS NULL');
    });
    it('nests and groups with AND and or groups with OR', ()=>{
        ...
        expect(whereBuilder.render()).toBe('("entity"."isActive" IS TRUE) AND (("entity"."id" = :metadataFilterParameter0) OR ("entity"."id" = :metadataFilterParameter1))');
    });
...
//# sourceMappingURL=apply-metadata-filter-to-query-builder.util.spec.js.map