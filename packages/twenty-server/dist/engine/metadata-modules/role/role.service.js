...
const _validaterolemutationdoesnotlockoutactorutil = require("./utils/validate-role-mutation-does-not-lock-out-actor.util");
...
let RoleService = class RoleService {
    async getWorkspaceRoles(workspaceId) {
        ...
        return this.findManyWithRelationsFromCache(Object.values(flatRoleMaps.byUniversalIdentifier).filter(_utils.isDefined), workspaceId);
    }
    async findManyWithRelationsFromCache(flatRoles, workspaceId) {
        const { flatRolePermissionFlagMaps, flatPermissionFlagMaps, flatObjectPermissionMaps, flatFieldPermissionMaps } = await this.flatEntityMapsCacheService.getOrRecomputeManyOrAllFlatEntityMaps({
            workspaceId,
            flatMapsKeys: [
        ...
        return flatRoles.map((flatRole)=>{
            const roleDto = (0, _fromFlatRoleToRoleDtoutil.fromFlatRoleToRoleDto)(flatRole);
            roleDto.permissionFlags = (0, _findmanyflatentitybyidinflatentitymapsutil.findManyFlatEntityByIdInFlatEntityMaps)({
                flatEntityIds: flatRole.rolePermissionFlagIds,
                flatEntityMaps: flatRolePermissionFlagMaps
            }).map((flatRolePermissionFlag)=>(0, _fromflatrolepermissionflagtorolepermissionflagdtoutil.fromFlatRolePermissionFlagToRolePermissionFlagDto)(flatRolePermissionFlag, flatPermissionFlagMaps));
            roleDto.objectPermissions = (0, _findmanyflatentitybyidinflatentitymapsutil.findManyFlatEntityByIdInFlatEntityMaps)({
                flatEntityIds: flatRole.objectPermissionIds,
                flatEntityMaps: flatObjectPermissionMaps
            }).map(_fromflatobjectpermissiontoobjectpermissiondtoutil.fromFlatObjectPermissionToObjectPermissionDto);
            roleDto.fieldPermissions = (0, _findmanyflatentitybyidinflatentitymapsutil.findManyFlatEntityByIdInFlatEntityMaps)({
                flatEntityIds: flatRole.fieldPermissionIds,
                flatEntityMaps: flatFieldPermissionMaps
            }).map(_fromflatfieldpermissiontofieldpermissiondtoutil.fromFlatFieldPermissionToFieldPermissionDto);
            return roleDto;
        });
    }
    ...
    async updateRole({ input, workspaceId, ownerFlatApplication, actingRoleIds }) {
        ...
        const { flatRoleMaps: existingFlatRoleMaps, flatRolePermissionFlagMaps } = await this.flatEntityMapsCacheService.getOrRecomputeManyOrAllFlatEntityMaps({
            workspaceId,
            flatMapsKeys: [
                'flatRoleMaps',
                'flatRolePermissionFlagMaps'
        ...
        if ((0, _utils.isDefined)(existingFlatRole)) {
            (0, _validaterolemutationdoesnotlockoutactorutil.validateRoleUpdateDoesNotLockOutActorOrThrow)({
    ...
    async deleteRole({ roleId, workspaceId, ownerFlatApplication, actingRoleIds }) {
    ...
    async deleteManyRoles({ ids, workspaceId, isSystemBuild = false, ownerFlatApplication, actingRoleIds }) {
        if (ids.length === 0) {
            return [];
        }
        const resolvedOwnerFlatApplication = ownerFlatApplication ?? (await this.applicationService.findWorkspaceTwentyStandardAndCustomApplicationOrThrow({
        ...
        for (const roleId of ids){
            ...
            (0, _validaterolemutationdoesnotlockoutactorutil.validateRoleDeletionDoesNotLockOutActorOrThrow)({
