...
const _rolevalidationservice = require("../role-validation/services/role-validation.service");
...
let UserRoleService = class UserRoleService {
    ...
    async assignRoleToWorkspaceMember({ workspaceId, workspaceMemberId, roleId, actingUserWorkspaceId }) {
        ...
        await this.roleValidationService.validateRoleAssignableToUsersOrThrow(roleId, workspaceId);
    ...
    async assignRoleToManyUserWorkspace({ workspaceId, userWorkspaceIds, roleId, actingUserWorkspaceId }) {
    ...
    validateNotSelfAssignmentOrThrow({ userWorkspaceIds, actingUserWorkspaceId }) {
        if ((0, _utils.isDefined)(actingUserWorkspaceId) && userWorkspaceIds.includes(actingUserWorkspaceId)) {
    ...
    async getWorkspaceMemberByIdOrThrow({ workspaceMemberId, workspaceId }) {
        const authContext = (0, _buildsystemauthcontextutil.buildSystemAuthContext)(workspaceId);
        return this.workspaceOrmManager.executeInWorkspaceContext(async ()=>{
            const workspaceMemberRepository = this.workspaceOrmManager.getRepository('workspaceMember', {
                shouldBypassPermissionChecks: true
    ...
    constructor(roleTargetRepository, userWorkspaceRepository, workspaceOrmManager, roleTargetService, workspaceCacheService, roleValidationService){
