...
let ViewResolver = class ViewResolver {
    async name(view, context, workspace) {
        ...
        return (0, _resolveviewnameutil.resolveViewName)({
            view,
            objectLabelPlaceholderValues: await this.resolveObjectLabelPlaceholderValues({
                view,
                context,
                workspace
            }),
    ...
    // standard object.
    async resolveObjectLabelPlaceholderValues({ view, context, workspace }) {
        if (!(0, _i18n.hasObjectMetadataLabelPlaceholder)(view.name)) {
            return undefined;
        }
        ...
        const objectI18nContext = await this.applicationTranslationCatalogService.buildEffectiveEntityI18nContext({
            applicationId: objectMetadata.applicationId,
            loaders: context.loaders,
            locale: context.req.locale,
            workspaceId: workspace.id
        });
        return (0, _buildviewnameobjectlabelsutil.buildViewNameObjectLabels)({
            viewName: view.name,
            objectMetadata,
    ...
    constructor(viewService, viewWidgetUpsertService, applicationTranslationCatalogService){
