        ...
        const module = await _testing.Test.createTestingModule({
            providers: [
                ...
                },
                {
                    ...
                    useValue: {
                        extractJwtFromRequest: ()=>(request)=>/^Bearer\s+(\S+)/i.exec(request.headers.authorization ?? '')?.[1]
                    }
                }
            ]
        }).compile();
        middleware = module.get(_cookiesessioncsrfmiddleware.CookieSessionCsrfMiddleware);
        next = jest.fn();
    });
...
//# sourceMappingURL=cookie-session-csrf.middleware.spec.js.map