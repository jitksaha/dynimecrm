...
const _utils = require("@graphql-tools/utils");
const _utils1 = require("twenty-shared/utils");
...
const _applicationkeepaliveintervalmsconstant = require("./constants/application-keepalive-interval-ms.constant");
...
const _metadataeventresolutionservice = require("./metadata-event/services/metadata-event-resolution.service");
...
let EventStreamResolver = class EventStreamResolver {
    async onEventSubscription(eventStreamId, locale, workspace, user, userWorkspaceId, apiKey, application) {
        ...
        if ((0, _utils1.isDefined)(existingStreamData)) {
        ...
        try {
            ...
            iterator = (0, _utils.mapAsyncIterator)(rawIterator, async (payload)=>({
                    ...payload,
                    metadataEvents: await this.metadataEventResolutionService.resolveMetadataEvents({
        ...
        return (0, _wrapasynciteratorwithlifecycle.wrapAsyncIteratorWithLifecycle)(iterator, {
            ...
            onHeartbeat: async ()=>{
                const now = Date.now();
                if (now - lastTtlRefreshAt > _eventstreamttlconstant.EVENT_STREAM_TTL_MS / 5) {
            ...
            onCleanupError: (error)=>this.exceptionHandlerService.captureExceptions([
                    error
                ], {
                    workspace: {
                        id: workspace.id
    ...
    async addQueryToEventStream(input, workspace, user, userWorkspaceId, apiKey, application) {
        ...
        if (!(0, _utils1.isDefined)(streamData)) {
            return false;
        }
        const isAuthorized = await this.eventStreamService.isAuthorized({
            streamData,
    ...
    async removeQueryFromEventStream(input, workspace, user, userWorkspaceId, apiKey, application) {
        const eventStreamChannelId = (0, _getchannelidfromeventstreamid.eventStreamIdToChannelId)(input.eventStreamId);
        const streamData = await this.eventStreamService.getStreamData(workspace.id, eventStreamChannelId);
        if (!(0, _utils1.isDefined)(streamData)) {
            return false;
    ...
    constructor(subscriptionService, eventStreamService, exceptionHandlerService, metadataEventResolutionService){
        this.subscriptionService = subscriptionService;
        this.eventStreamService = eventStreamService;
        this.exceptionHandlerService = exceptionHandlerService;
        this.metadataEventResolutionService = metadataEventResolutionService;
    }
};
_ts_decorate([
    ...
    _ts_param(5, (0, _authapikeydecorator.AuthApiKey)()),
    _ts_param(6, (0, _authapplicationdecorator.AuthApplication)({
        allowUndefined: true
    })),
...
_ts_decorate([
    ...
    _ts_param(5, (0, _authapplicationdecorator.AuthApplication)({
        allowUndefined: true
    })),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", [
...
EventStreamResolver = _ts_decorate([
    ...
    _ts_metadata("design:paramtypes", [
        ...
        typeof _metadataeventresolutionservice.MetadataEventResolutionService === "undefined" ? Object : _metadataeventresolutionservice.MetadataEventResolutionService
    ])
], EventStreamResolver);
