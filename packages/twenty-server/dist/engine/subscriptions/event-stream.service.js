...
let EventStreamService = class EventStreamService {
    ...
    async getTotalActiveStreamCount() {
        const now = Date.now();
        const isStale = now - this.activeStreamCountRefreshedAt >= ACTIVE_STREAM_COUNT_REFRESH_MS;
        if (isStale) {
            this.activeStreamCount = await this.cacheStorageService.sortedSetRemoveByScoreAndCount(ACTIVE_STREAM_EXPIRATIONS_KEY, 0, now);
    ...
    async createEventStream({ workspaceId, eventStreamChannelId, authContext }) {
        ...
        await this.cacheStorageService.setAdd(this.getActiveStreamsKey(workspaceId), [
    ...
    async destroyEventStream({ workspaceId, eventStreamChannelId }) {
        ...
        await this.cacheStorageService.setRemove(this.getActiveStreamsKey(workspaceId), [
            eventStreamChannelId
        ]);
        await this.untrackActiveStreams(workspaceId, [
            eventStreamChannelId
        ]);
    }
    ...
    async removeFromActiveStreams(workspaceId, streamIdsToRemove) {
        ...
        await this.cacheStorageService.setRemove(this.getActiveStreamsKey(workspaceId), streamIdsToRemove);
    ...
    async isAuthorized({ authContext, streamData }) {
        if ((0, _utils.isDefined)(authContext.userWorkspaceId)) {
            return streamData.authContext.userWorkspaceId === authContext.userWorkspaceId && (streamData.authContext.applicationId ?? null) === (authContext.applicationId ?? null);
        }
    ...
    async trackActiveStream({ workspaceId, eventStreamChannelId }) {
        try {
            await this.cacheStorageService.sortedSetAdd(ACTIVE_STREAM_EXPIRATIONS_KEY, [
                {
                    score: Date.now() + _eventstreamttlconstant.EVENT_STREAM_TTL_MS,
                    value: this.getActiveStreamExpirationMember(workspaceId, eventStreamChannelId)
                }
            ]);
        } catch (error) {
    ...
    async untrackActiveStreams(workspaceId, eventStreamChannelIds) {
        try {
            await this.cacheStorageService.sortedSetRemove(ACTIVE_STREAM_EXPIRATIONS_KEY, eventStreamChannelIds.map((eventStreamChannelId)=>this.getActiveStreamExpirationMember(workspaceId, eventStreamChannelId)));
        } catch (error) {
            this.logger.warn(`Failed to untrack active event streams: ${error}`);
        }
    }
    getActiveStreamExpirationMember(workspaceId, eventStreamChannelId) {
    ...
    constructor(cacheStorageService, metricsService){
        ...
        this.activeStreamCountRefreshedAt = 0;
    }
};
_ts_decorate([
