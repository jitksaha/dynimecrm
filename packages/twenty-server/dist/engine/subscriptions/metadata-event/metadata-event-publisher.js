...
let MetadataEventPublisher = class MetadataEventPublisher {
        ...
        await this.workspaceEventBroadcaster.broadcast({
            ...
            events: enrichedBatch.events.map((event)=>{
                ...
                return {
                    ...
                    recipientUserWorkspaceIds: (0, _guards.isNonEmptyString)(ownerUserWorkspaceId) ? [
    ...
    resolveOwnerUserWorkspaceId(event) {
        if (event.metadataName !== 'navigationMenuItem') {
            return undefined;
        }
        const record = event.type === 'deleted' ? event.properties.before : event.properties.after;
    ...
    async enrichFieldMetadataEventsWithRelations(metadataEventBatch) {
        ...
        const enrichedEvents = metadataEventBatch.events.map((event)=>{
            ...
            if ('after' in enrichedProperties && (0, _utils.isDefined)(enrichedProperties.after)) {
                enrichedProperties.after = (0, _enrichfieldmetadataeventwithrelationsutil.enrichFieldMetadataEventWithRelations)({
    ...
    constructor(workspaceEventBroadcaster, workspaceManyOrAllFlatEntityMapsCacheService, navigationMenuItemRecordIdentifierService){
