...
describe('ObjectRecordEventPublisher', ()=>{
    ...
    const companyNameField = (0, _getflatfieldmetadatamock.getFlatFieldMetadataMock)({
        objectMetadataId: _companyflatobjectmock.COMPANY_FLAT_OBJECT_MOCK.id,
    ...
    describe('publish', ()=>{
        ...
        it('should publish update events when only the BEFORE state matches the filter (record leaving the view)', async ()=>{
            isRecordMatchingRLSRowLevelPermissionPredicate.mockImplementation(({ record })=>record.name === 'Open Company');
            const streamDataWithFilter = {
                ...mockStreamData,
                queries: {
                    'query-1': {
            ...
            await service.publish(eventBatch);
            expect(mockSubscriptionService.publishToEventStream).toHaveBeenCalledTimes(1);
            const publishCall = mockSubscriptionService.publishToEventStream.mock.calls[0][0];
            expect(publishCall.payload.objectRecordEventsWithQueryIds).toHaveLength(1);
            expect(publishCall.payload.objectRecordEventsWithQueryIds[0].queryIds).toEqual([
                'query-1'
            ]);
        });
        it('should not publish update events when the delivered state fails the RLS filter, even if the before state matched', async ()=>{
            ...
            isRecordMatchingRLSRowLevelPermissionPredicate.mockImplementation(({ record, filter })=>'status' in filter ? record.status === 'active' : true);
            const eventBatch = {
                name: 'company.updated',
                workspaceId,
                objectMetadata: companyObjectMetadata,
        ...
        describe('stream opened by an application acting for a user', ()=>{
            ...
            const mockApplicationStream = (overrides = {})=>{
                mockEventStreamService.getStreamsData.mockResolvedValue(new Map([
                    [
                        streamChannelId,
                        {
            ...
            const buildRolePermissions = (canReadObjectRecords)=>({
                    [companyObjectMetadata.id]: {
