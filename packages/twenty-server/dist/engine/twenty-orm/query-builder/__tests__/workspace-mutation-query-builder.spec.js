...
const buildBuilders = ({ rows = [] } = {})=>{
    const executedStatements = [];
    const selectQueryBuilder = new _workspaceselectquerybuilder.WorkspaceSelectQueryBuilder('person', {
...
describe('WorkspaceMutationQueryBuilder', ()=>{
    it('should build a soft delete that stamps deletedAt and updatedAt', ()=>{
        ...
        const mutationQueryBuilder = selectQueryBuilder.where('"person"."id" IN (:...ids)', {
            ids: [
                'id-1',
                'id-2'
            ]
        }).softDelete().returning([
            'id',
            'nameFirstName'
        ]);
        expect(mutationQueryBuilder.getQuery()).toBe(`UPDATE "${SCHEMA_NAME}"."person" AS "person" ` + 'SET "deletedAt" = CURRENT_TIMESTAMP, "updatedAt" = CURRENT_TIMESTAMP ' + 'WHERE ("person"."id" IN (:...ids)) ' + 'RETURNING "person"."id" AS "person_id", "person"."nameFirstName" AS "person_nameFirstName"');
    });
    it('should build a restore that clears deletedAt and stamps updatedAt', ()=>{
        ...
        expect(mutationQueryBuilder.getQuery()).toBe(`UPDATE "${SCHEMA_NAME}"."person" AS "person" ` + 'SET "deletedAt" = NULL, "updatedAt" = CURRENT_TIMESTAMP ' + 'WHERE ("person"."id" IN (:...ids)) ' + 'RETURNING "person"."id" AS "person_id"');
    });
    it('should build a hard delete with no SET clause', ()=>{
        ...
        expect(mutationQueryBuilder.getQuery()).toBe(`DELETE FROM "${SCHEMA_NAME}"."person" AS "person" ` + 'WHERE ("person"."id" IN (:...ids)) ' + 'RETURNING "person"."id" AS "person_id"');
    });
    it('should build an update that binds set values and appends updatedAt maintenance', ()=>{
        const { selectQueryBuilder } = buildBuilders();
        const [sql, values] = selectQueryBuilder.where('"person"."id" IN (:...ids)', {
            ids: [
                'id-1'
            ]
        }).update().set({
        ...
        expect(sql).toBe(`UPDATE "${SCHEMA_NAME}"."person" AS "person" ` + 'SET "jobTitle" = $1, "updatedAt" = CURRENT_TIMESTAMP ' + 'WHERE ("person"."id" IN ($2)) ' + 'RETURNING "person"."id" AS "person_id", "person"."jobTitle" AS "person_jobTitle"');
    ...
    it('should not append updatedAt maintenance when the caller sets updatedAt itself', ()=>{
        const { selectQueryBuilder } = buildBuilders();
        const [sql, values] = selectQueryBuilder.where('"person"."id" = :id', {
            id: 'id-1'
        }).update().set({
            jobTitle: 'Tech Lead',
            updatedAt: null
        ...
        expect(sql).toBe(`UPDATE "${SCHEMA_NAME}"."person" AS "person" ` + 'SET "jobTitle" = $1, "updatedAt" = $2 ' + 'WHERE ("person"."id" = $3) ' + 'RETURNING "person"."id" AS "person_id"');
        expect(values).toEqual([
            'Tech Lead',
            null,
            'id-1'
    ...
    it('should return formatted rows from execute', async ()=>{
        ...
        const result = await selectQueryBuilder.where('"person"."id" IN (:...ids)', {
            ids: [
                'id-1',
                'id-2'
            ]
    ...
    it('should not overwrite a where parameter that collides with a set parameter name', ()=>{
        const { selectQueryBuilder } = buildBuilders();
        const [, values] = selectQueryBuilder.where('"person"."id" = :ormSet_0', {
            ormSet_0: 'id-1'
        }).update().set({
            jobTitle: 'Tech Lead'
        }).returning([
    ...
    it('should reject a soft delete on a table without a deletedAt column', ()=>{
        ...
        expect(()=>selectQueryBuilder.where('"person"."id" = :id', {
                id: 'id-1'
            }).softDelete().returning([
                'id'
            ]).getQuery()).toThrow(_twentyormexception.TwentyOrmException);
    });
    ...
    it('should bump only updatedAt for an update carrying no columns', ()=>{
        ...
        expect(selectQueryBuilder.where('"person"."id" = :id', {
            ...
            'id'
        ]).getQuery()).toBe(`UPDATE "${SCHEMA_NAME}"."person" AS "person" ` + 'SET "updatedAt" = CURRENT_TIMESTAMP ' + 'WHERE ("person"."id" = :id) ' + 'RETURNING "person"."id" AS "person_id"');
    });
    it('should reject an update that would emit no SET clause', ()=>{
        const shapeWithoutUpdatedAt = {
            ...personTableShape,
            columnShapeByColumnName: {
                id: buildColumn('id'),
                deletedAt: buildColumn('deletedAt')
    ...
    it('should refuse to morph a query that carries a relation join', ()=>{
        ...
        expect(()=>selectQueryBuilder.softDelete()).toThrow(_twentyormexception.TwentyOrmException);
    });
});

//# sourceMappingURL=workspace-mutation-query-builder.spec.js.map