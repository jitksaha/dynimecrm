...
describe('WorkspaceSelectQueryBuilder joined hydration', ()=>{
    it('should project every joined column for a leftJoinAndSelect', ()=>{
        ...
        expect(sql).toContain('"company"."id" AS "company_id", "company"."name" AS "company_name", ' + '"company"."deletedAt" AS "company_deletedAt"');
        expect(sql).toContain(`LEFT JOIN "${_workspaceselectquerybuildertestshapesutil.SCHEMA_NAME}"."company" AS "company" ` + 'ON ("person"."companyId" = "company"."id")');
    });
    it('should render an inner join for an innerJoinAndSelect', ()=>{
        ...
        expect(queryBuilder.getQuery()).toContain(`INNER JOIN "${_workspaceselectquerybuildertestshapesutil.SCHEMA_NAME}"."company" AS "company"`);
    });
    ...
    it('should hydrate a partial relation column selected without an output alias', async ()=>{
        ...
        expect(queryBuilder.getQuery()).toContain('"company"."name" AS "company_name"');
        await expect(queryBuilder.getMany()).resolves.toEqual([
            {
                id: 'person-1',
                company: {
    ...
    it('should keep an aliased relation column selection as raw output', async ()=>{
        ...
        expect(queryBuilder.getQuery()).toContain('"company"."name" AS "companyName"');
        await expect(queryBuilder.getMany()).resolves.toEqual([
            {
                id: 'person-1'
            }
    ...
    it('should render an added join condition on a selected inner join', ()=>{
        ...
        expect(sql.indexOf('"company"."name" = :rlsName')).toBeLessThan(sql.indexOf('WHERE'));
    });
});

//# sourceMappingURL=workspace-select-query-builder-hydration.spec.js.map