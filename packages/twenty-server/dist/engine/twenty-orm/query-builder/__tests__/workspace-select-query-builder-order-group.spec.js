...
describe('WorkspaceSelectQueryBuilder order, group and distinct', ()=>{
    it('should render order by with direction and nulls handling', ()=>{
        ...
        expect(queryBuilder.getQuery()).toContain('ORDER BY "person"."nameFirstName" DESC NULLS LAST, "company"."name" ASC');
    });
    it('should render a DISTINCT ON clause with normalised columns', ()=>{
        ...
        expect(queryBuilder.getQuery()).toContain('SELECT DISTINCT ON ("company"."name", "person"."nameFirstName") ');
    });
    ...
    it('should report joined-alias columns referenced only in order by', ()=>{
        ...
        expect(columnNamesByAlias['person']).toEqual([
            'id'
        ]);
    ...
    it('should emit a GROUP BY with aggregate and grouped columns', ()=>{
        ...
        expect(queryBuilder.getQuery()).toBe('SELECT COUNT(*) AS "totalCount", "person"."companyId" AS "companyId" ' + `FROM "workspace_1wgvd1injqtife6y4rvfbu3h5"."person" AS "person" ` + 'WHERE "person"."deletedAt" IS NULL ' + 'GROUP BY "person"."companyId"');
    });
    it('should append additional GROUP BY expressions with addGroupBy', ()=>{
        const { queryBuilder } = (0, _workspaceselectquerybuildertestshapesutil.buildQueryBuilder)();
        queryBuilder.select([]).addSelect('COUNT(*)', 'totalCount').groupBy('"person"."companyId"').addGroupBy('"person"."nameFirstName"');
        expect(queryBuilder.getQuery()).toContain('GROUP BY "person"."companyId", "person"."nameFirstName"');
    });
});

//# sourceMappingURL=workspace-select-query-builder-order-group.spec.js.map