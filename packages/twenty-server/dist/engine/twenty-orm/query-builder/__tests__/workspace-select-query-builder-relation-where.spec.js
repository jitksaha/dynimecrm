...
const _applyfindoptionsutil = require("../utils/apply-find-options.util");
...
describe('WorkspaceSelectQueryBuilder relation-keyed where', ()=>{
    it('should filter a to-many relation with a correlated EXISTS instead of a join', ()=>{
        ...
        queryBuilder.setFindOptions({
            ...
            people: {
                name: (0, _typeorm.Equal)('Twenty')
            }
        });
        const [text, values] = queryBuilder.getQueryAndParameters();
        expect(text).toContain('EXISTS (SELECT 1 FROM "workspace_1wgvd1injqtife6y4rvfbu3h5"."company" AS "person_people_filter" WHERE "person_people_filter"."personId" = "person"."id" AND ("person_people_filter"."name" = $1) AND "person_people_filter"."deletedAt" IS NULL)');
    ...
    it('should correlate a to-one relation on the parent join column', ()=>{
        ...
        expect(queryBuilder.getQuery()).toContain('EXISTS (SELECT 1 FROM "workspace_1wgvd1injqtife6y4rvfbu3h5"."company" AS "person_company_filter" WHERE "person"."companyId" = "person_company_filter"."id"');
    });
    it('should combine a relation filter with sibling column conditions', ()=>{
        ...
        queryBuilder.setFindOptions({
            ...
            nameFirstName: (0, _typeorm.Equal)('Ada'),
            people: {
                name: (0, _typeorm.Equal)('Twenty')
            }
        });
    ...
    it('should keep a relation filter inside its OR group', ()=>{
        const { queryBuilder } = (0, _workspaceselectquerybuildertestshapesutil.buildQueryBuilder)();
        (0, _applyfindoptionsutil.applyFindOptionsToQueryBuilder)(queryBuilder, {
            select: {
                id: true
            },
            where: [
    ...
    it('should nest a relation filter reached through another relation', ()=>{
        ...
        expect(text).toContain('"person_people_filter"."personId" = "person_people_filter_person_filter"."id"');
        expect(text).not.toContain('__ormExistsFilter');
    });
    it('should drop the soft-delete predicate inside the subquery when deleted rows are included', ()=>{
        ...
        expect(queryBuilder.getQuery()).not.toContain('"person_people_filter"."deletedAt" IS NULL');
    });
    it('should apply a row-level permission predicate inside the subquery', ()=>{
        ...
        queryBuilder.addJoinCondition('person_people_filter', '"person_people_filter"."name" <> \'hidden\'');
        expect(queryBuilder.getQuery()).toContain('AND "person_people_filter"."name" <> \'hidden\'');
    });
    it('should expose the filtered relation as a join attribute so permissions resolve it', ()=>{
        ...
        expect(queryBuilder.getJoinAliases().map((joinAlias)=>joinAlias.name)).toContain('person_people_filter');
        expect(queryBuilder.getJoinedTableShape('person_people_filter')?.nameSingular).toBe('company');
    });
    ...
    it('should carry relation filters into a builder that copies the where clauses', ()=>{
        const { queryBuilder } = (0, _workspaceselectquerybuildertestshapesutil.buildQueryBuilder)();
        const { queryBuilder: snapshotQueryBuilder } = (0, _workspaceselectquerybuildertestshapesutil.buildQueryBuilder)();
        queryBuilder.setFindOptions({
            select: {
                id: true
            }
        ...
        snapshotQueryBuilder.setFindOptions({
            select: {
                id: true
            }
    ...
    it('should reject a relation filter on a mutation', ()=>{
        ...
        expect(()=>queryBuilder.delete()).toThrow(_twentyormexception.TwentyOrmException);
    });
});

//# sourceMappingURL=workspace-select-query-builder-relation-where.spec.js.map