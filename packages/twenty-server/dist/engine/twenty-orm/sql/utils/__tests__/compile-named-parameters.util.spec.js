...
describe('compileNamedParameters', ()=>{
    it('should turn a named parameter into a positional placeholder', ()=>{
        const compiled = (0, _compilenamedparametersutil.compileNamedParameters)('"person"."name" = :name1a2b', {
            name1a2b: 'acme'
        });
    ...
    it('should expand a spread parameter into one placeholder per item', ()=>{
        const compiled = (0, _compilenamedparametersutil.compileNamedParameters)('"id" IN (:...ids)', {
            ids: [
                'a',
                'b',
                'c'
    ...
    it('should bind a parameter referenced twice to a single placeholder', ()=>{
        const compiled = (0, _compilenamedparametersutil.compileNamedParameters)('"a" = :value OR "b" = :value', {
            value: 42
        });
    ...
    it('should not treat a cast as a parameter', ()=>{
        const compiled = (0, _compilenamedparametersutil.compileNamedParameters)('"createdAt" < :cutoff::timestamptz + interval \'1 millisecond\'', {
            cutoff: '2026-01-01'
        });
        expect(compiled.text).toBe('"createdAt" < $1::timestamptz + interval \'1 millisecond\'');
        expect(compiled.values).toEqual([
            '2026-01-01'
        ]);
    });
    it('should not treat a colon inside a string literal as a parameter', ()=>{
        const compiled = (0, _compilenamedparametersutil.compileNamedParameters)(`"label" = 'a:b' AND "id" = :id`, {
            id: 7
        });
    ...
    it('should not treat a colon inside a quoted identifier as a parameter', ()=>{
        const compiled = (0, _compilenamedparametersutil.compileNamedParameters)('"weird:column" = :id', {
            id: 1
        });
    ...
    it('should handle doubled quotes inside a string literal', ()=>{
        const compiled = (0, _compilenamedparametersutil.compileNamedParameters)(`"note" = 'it''s here' AND "id" = :id`, {
            id: 1
        });
    ...
    it('should throw when a referenced parameter was not provided', ()=>{
        expect(()=>(0, _compilenamedparametersutil.compileNamedParameters)('"id" = :missing', {})).toThrow(_twentyormexception.TwentyOrmException);
    });
    ...
    it('should not let a spread item key collide with a caller parameter', ()=>{
        const compiled = (0, _compilenamedparametersutil.compileNamedParameters)('"a" IN (:...ids) OR "z" = :ids__0', {
            ids: [
                'a',
                'b'
            ],
    ...
    it('should throw when a spread parameter is not an array', ()=>{
        expect(()=>(0, _compilenamedparametersutil.compileNamedParameters)('"id" IN (:...ids)', {
                ids: 'a'
            })).toThrow(_twentyormexception.TwentyOrmException);
    });
    it('should throw on an unterminated string literal', ()=>{
        expect(()=>(0, _compilenamedparametersutil.compileNamedParameters)(`"a" = 'unterminated`, {})).toThrow(_twentyormexception.TwentyOrmException);
    });
    it('should leave a value containing a placeholder-like string untouched', ()=>{
        const compiled = (0, _compilenamedparametersutil.compileNamedParameters)('"a" = :value', {
            value: ':notAParameter'
        });
...
//# sourceMappingURL=compile-named-parameters.util.spec.js.map