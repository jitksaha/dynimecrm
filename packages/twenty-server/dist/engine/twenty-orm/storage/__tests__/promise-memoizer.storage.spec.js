    ...
    describe('memoizePromiseAndExecute', ()=>{
        ...
        it('should re-execute factory after TTL expires even under continuous reads', async ()=>{
            ...
            for (const elapsedTime of [
                ...
                jest.spyOn(global.Date, 'now').mockImplementation(()=>startTime + elapsedTime + 1);
                await memoizer.memoizePromiseAndExecute('test-key-1', mockFactory);
            }
        ...
        it('should not cache a stale promise after its key is cleared', async ()=>{
            let markFirstFactoryStarted;
            const firstFactoryStarted = new Promise((resolve)=>{
                markFirstFactoryStarted = resolve;
            });
            let resolveFirstFactory;
            let resolveSecondFactory;
            const firstFactoryPromise = new Promise((resolve)=>{
                resolveFirstFactory = resolve;
            });
            const secondFactoryPromise = new Promise((resolve)=>{
            ...
            const firstResultPromise = memoizer.memoizePromiseAndExecute('test-key-1', mockFactory);
            await firstFactoryStarted;
            await memoizer.clearKey('test-key-1');
            const secondResultPromise = memoizer.memoizePromiseAndExecute('test-key-1', mockFactory);
            ...
            await expect(memoizer.memoizePromiseAndExecute('test-key-1', mockFactory)).resolves.toBe('fresh-value');
            expect(mockFactory).toHaveBeenCalledTimes(2);
        });
    });
    describe('clearKey', ()=>{
