...
const buildRecordFilterForRole = ({ flatRowLevelPermissionPredicateMaps, flatRowLevelPermissionPredicateGroupMaps, flatFieldMetadataMaps, objectMetadata, roleId, workspaceMember })=>{
...
const buildRowLevelPermissionRecordFilter = ({ roleIds, ...buildRecordFilterForRoleArgs })=>{
    const recordFilters = roleIds.map((roleId)=>buildRecordFilterForRole({
            ...buildRecordFilterForRoleArgs,
            roleId
        })).filter(_utils.isDefined).filter((recordFilter)=>Object.keys(recordFilter).length > 0);
