...
const intersectRowLevelPermissionPredicates = (rowLevelPermissionPredicatesPerRole)=>{
    const [firstRolePredicates = [], ...otherRolesPredicates] = rowLevelPermissionPredicatesPerRole;
    const constrainedFieldMetadataIdsPerOtherRole = otherRolesPredicates.map((predicates)=>new Set(predicates.map((predicate)=>predicate.fieldMetadataId)));
    return firstRolePredicates.filter((predicate)=>constrainedFieldMetadataIdsPerOtherRole.every((fieldMetadataIds)=>fieldMetadataIds.has(predicate.fieldMetadataId)));
};
const computePermissionIntersection = (permissionsArray)=>{
    ...
    for (const objectMetadataId of allObjectMetadataIds){
        ...
        const rowLevelPermissionPredicatesPerRole = [];
        for (const permissions of permissionsArray){
            ...
            rowLevelPermissionPredicatesPerRole.push(objPerm.rowLevelPermissionPredicates);
        ...
        result[objectMetadataId] = {
            ...
            restrictedFields,
            rowLevelPermissionPredicates: intersectRowLevelPermissionPredicates(rowLevelPermissionPredicatesPerRole),
