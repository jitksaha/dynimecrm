...
let WorkspaceSchemaColumnManagerService = class WorkspaceSchemaColumnManagerService {
    ...
    async alterColumnNullable({ queryRunner, schemaName, tableName, columnName, isNullable, backfillValue }) {
        const tableRef = `${(0, _removesqlinjectionutil.escapeIdentifier)(schemaName)}.${(0, _removesqlinjectionutil.escapeIdentifier)(tableName)}`;
        const columnRef = (0, _removesqlinjectionutil.escapeIdentifier)(columnName);
        if (!isNullable && backfillValue !== undefined && backfillValue !== 'NULL') {
            await queryRunner.query(`UPDATE ${tableRef} SET ${columnRef} = ${backfillValue} WHERE ${columnRef} IS NULL`);
        }
        await queryRunner.query(`ALTER TABLE ${tableRef} ALTER COLUMN ${columnRef} ${isNullable ? 'DROP NOT NULL' : 'SET NOT NULL'}`);
    }
};
