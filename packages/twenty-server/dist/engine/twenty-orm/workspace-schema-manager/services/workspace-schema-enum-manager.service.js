...
const buildTemporaryIdentifier = (baseName, suffix)=>{
    const maxBaseLength = POSTGRES_MAX_IDENTIFIER_LENGTH - suffix.length;
    const truncatedBase = baseName.length <= maxBaseLength ? baseName : baseName.slice(0, maxBaseLength);
...
let WorkspaceSchemaEnumManagerService = class WorkspaceSchemaEnumManagerService {
    ...
    async upsertEnumValue({ queryRunner, schemaName, enumName, value }) {
        const sql = `ALTER TYPE ${(0, _removesqlinjectionutil.escapeIdentifier)(schemaName)}.${(0, _removesqlinjectionutil.escapeIdentifier)(enumName)} ADD VALUE IF NOT EXISTS ${(0, _removesqlinjectionutil.escapeLiteral)(value)}`;
        await queryRunner.query(sql);
    }
