    ...
    return item.fields.reduce((node, field)=>{
        ...
        switch(field.type){
            ...
            case _types.FieldMetadataType.LINKS:
                {
                    const linksSettings = (0, _isfieldmetadatasettingsoftypeutil.isFieldMetadataSettingsOfType)(field.settings, _types.FieldMetadataType.LINKS) ? field.settings : null;
                    const secondaryLinkUrlProperty = linksSettings?.type === 'domain' ? {
                        type: 'string'
                    } : {
                        type: 'string',
                        format: 'uri'
