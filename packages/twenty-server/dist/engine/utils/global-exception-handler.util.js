...
const graphQLErrorCodesToFilter = [
    ...
    _graphqlerrorsutil.ErrorCode.RATE_LIMITED,
    _graphqlerrorsutil.ErrorCode.QUOTA_EXHAUSTED
];
...
const shouldCaptureException = (exception, statusCode)=>{
    if (exception instanceof _logicfunction.RetryableLogicFunctionError) {
        return false;
    }
    if (exception instanceof _customexception.CustomException && (0, _utils.isDefined)(exception.statusCode) && exception.statusCode < 500) {
        return false;
    }
...
const handleException = ({ exception, exceptionHandlerService, user, workspace, statusCode, shouldBeCapturedBySentry = true })=>{
