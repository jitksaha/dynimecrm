...
_export(exports, {
    get HASH_KEYED_WORKSPACE_CACHE_KEYS () {
        return HASH_KEYED_WORKSPACE_CACHE_KEYS;
    },
...
const HASH_KEYED_WORKSPACE_CACHE_KEYS = {
...
let WorkspaceCacheStorageService = class WorkspaceCacheStorageService {
    setMetadataVersion(workspaceId, metadataVersion) {
        return this.cacheStorageService.set(`${WORKSPACE_CACHE_KEYS.MetadataVersion}:${workspaceId}`, metadataVersion, TTL_ONE_WEEK);
    }
    getMetadataVersion(workspaceId) {
        return this.cacheStorageService.get(`${WORKSPACE_CACHE_KEYS.MetadataVersion}:${workspaceId}`);
    }
    setGraphQLTypeDefs(workspaceId, metadataCacheHash, typeDefs, applicationId) {
        const applicationSuffix = applicationId ? `:${applicationId}` : '';
        return this.cacheStorageService.set(`${HASH_KEYED_WORKSPACE_CACHE_KEYS.GraphQLTypeDefs}:${workspaceId}:${metadataCacheHash}${applicationSuffix}`, typeDefs, TTL_ONE_WEEK);
    }
    getGraphQLTypeDefs(workspaceId, metadataCacheHash, applicationId) {
        const applicationSuffix = applicationId ? `:${applicationId}` : '';
        return this.cacheStorageService.get(`${HASH_KEYED_WORKSPACE_CACHE_KEYS.GraphQLTypeDefs}:${workspaceId}:${metadataCacheHash}${applicationSuffix}`);
    }
    setGraphQLUsedScalarNames(workspaceId, metadataCacheHash, usedScalarNames, applicationId) {
        const applicationSuffix = applicationId ? `:${applicationId}` : '';
        return this.cacheStorageService.set(`${HASH_KEYED_WORKSPACE_CACHE_KEYS.GraphQLUsedScalarNames}:${workspaceId}:${metadataCacheHash}${applicationSuffix}`, usedScalarNames, TTL_ONE_WEEK);
    }
    getGraphQLUsedScalarNames(workspaceId, metadataCacheHash, applicationId) {
        const applicationSuffix = applicationId ? `:${applicationId}` : '';
        return this.cacheStorageService.get(`${HASH_KEYED_WORKSPACE_CACHE_KEYS.GraphQLUsedScalarNames}:${workspaceId}:${metadataCacheHash}${applicationSuffix}`);
    }
    ...
    async flushHashKeyedWorkspaceCache(workspaceId) {
        await Promise.all(Object.values(HASH_KEYED_WORKSPACE_CACHE_KEYS).map((key)=>this.cacheStorageService.flushByPattern(`${key}:${workspaceId}:*`)));
    }
