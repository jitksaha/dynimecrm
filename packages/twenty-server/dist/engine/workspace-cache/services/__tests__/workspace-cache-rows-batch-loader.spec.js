...
describe('WorkspaceCacheRowsBatchLoader', ()=>{
    const setup = ()=>{
        const findMocksByEntity = new Map([
            [
        ...
        return {
            rowsBatchLoader: new _workspacecacherowsbatchloader.WorkspaceCacheRowsBatchLoader(dataSource, WORKSPACE_ID),
            findMocksByEntity
        };
    };
    it('fetches all workspace rows including soft-deleted ones', async ()=>{
        ...
        expect(rowsBatchLoader.readRows({
            objectMetadata: true
        }).objectMetadata).toEqual([
            {
                id: 'object-row',
                workspaceId: WORKSPACE_ID
            }
        ]);
        expect(findMocksByEntity.get(_objectmetadataentity.ObjectMetadataEntity)).toHaveBeenCalledWith({
            where: {
                workspaceId: WORKSPACE_ID
            },
            withDeleted: true
    ...
    it('merges requirements into one query per entity with the union of declared columns', async ()=>{
        ...
        const objectFindMock = findMocksByEntity.get(_objectmetadataentity.ObjectMetadataEntity);
        expect(objectFindMock).toHaveBeenCalledTimes(1);
        expect(objectFindMock.mock.calls[0][0].select).toHaveLength(2);
        expect(objectFindMock.mock.calls[0][0].select).toEqual(expect.arrayContaining([
            'id',
            'nameSingular'
        ]));
        expect(findMocksByEntity.get(_applicationentity.ApplicationEntity)).toHaveBeenCalledTimes(1);
    });
    it('drops the column selection once any requirement needs full rows', async ()=>{
        ...
        const findMock = findMocksByEntity.get(_objectmetadataentity.ObjectMetadataEntity);
    ...
    it('throws when loadRows is called more than once', async ()=>{
        ...
        await expect(rowsBatchLoader.loadRows([
            {
                objectMetadata: [
                    'nameSingular'
                ]
    ...
    it('widens the fetched columns with the declared groupBy keys', async ()=>{
        ...
        const findMock = findMocksByEntity.get(_viewfieldentity.ViewFieldEntity);
        expect(findMock).toHaveBeenCalledTimes(1);
        expect(findMock.mock.calls[0][0].select).toHaveLength(2);
        expect(findMock.mock.calls[0][0].select).toEqual(expect.arrayContaining([
            'id',
            'fieldMetadataId'
        ]));
    });
    it('returns rows and grouped maps for a groupBy declaration, skipping null foreign keys', async ()=>{
        ...
        findMocksByEntity.get(_viewfieldentity.ViewFieldEntity).mockResolvedValue(rows);
        ...
        expect([
            ...viewField.byFieldMetadataId.keys()
        ]).toEqual([
            'field-1'
        ]);
    ...
    it('memoizes grouped maps across readRows calls for the same entity and foreign key', async ()=>{
        const { rowsBatchLoader, findMocksByEntity } = setup();
        findMocksByEntity.get(_viewfieldentity.ViewFieldEntity).mockResolvedValue([
            {
                id: 'view-field-a',
                fieldMetadataId: 'field-1',
                workspaceId: WORKSPACE_ID
        ...
        expect(secondRead.viewField.byFieldMetadataId).toBe(firstRead.viewField.byFieldMetadataId);
    });
        ...
        await rowsBatchLoader.loadRows([
            {
                viewField: {
                    ...
                    where: {
                        isVisible: true,
                        fieldMetadataId: (0, _typeorm.Not)((0, _typeorm.IsNull)())
                    }
                }
            },
            {
...
//# sourceMappingURL=workspace-cache-rows-batch-loader.spec.js.map