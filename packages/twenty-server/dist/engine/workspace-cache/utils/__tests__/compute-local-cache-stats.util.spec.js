...
const _computelocalcachestatsutil = require("../compute-local-cache-stats.util");
const entry = (versionCount, state = 'live')=>({
        versions: new Map(Array.from({
            length: versionCount
        }, (_, index)=>[
                `hash-${index}`,
                state === 'live' ? {
...
describe('computeLocalCacheStats', ()=>{
    it('returns zeros for an empty cache', ()=>{
        const stats = (0, _computelocalcachestatsutil.computeLocalCacheStats)(new Map());
    ...
    it('counts entries, distinct workspaces, and total versions', ()=>{
        const stats = (0, _computelocalcachestatsutil.computeLocalCacheStats)(new Map([
            [
                'ORMEntityMetadatas:ws-a',
                entry(1)
            ],
            [
                'flatFieldMetadataMaps:ws-a',
                entry(2)
            ],
            [
                'ORMEntityMetadatas:ws-b',
                entry(1)
            ]
        ]));
...
//# sourceMappingURL=compute-local-cache-stats.util.spec.js.map