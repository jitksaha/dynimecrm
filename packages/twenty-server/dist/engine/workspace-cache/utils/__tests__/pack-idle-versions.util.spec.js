...
const _packidleversionsutil = require("../pack-idle-versions.util");
...
const run = (localCache, ponderationBudget, overrides)=>(0, _packidleversionsutil.packIdleVersions)({
        localCache,
        minIdleMs: IDLE_MS,
        ponderationBudget,
...
const stateOf = (localCache, localKey)=>localCache.get(localKey)?.versions.get('hash-1')?.state;
describe('packIdleVersions', ()=>{
    ...
    it('should pack no more than one run allows, coldest first', ()=>{
        const localCache = new Map([
            [
                `${FIELD_METADATA}:ws-coldest`,
                liveEntry(IDLE - 2_000)
            ],
            [
                `${FIELD_METADATA}:ws-colder`,
                liveEntry(IDLE - 1_000)
            ],
            [
                `${ORM}:ws-warm`,
                liveEntry(IDLE)
    ...
    it('should converge across successive runs', ()=>{
        const localCache = new Map(Array.from({
            length: 5
        }, (_, index)=>[
        ...
        expect([
            ...localCache.values()
        ].every((entry)=>entry.versions.get('hash-1')?.state === 'packed')).toBe(true);
    });
    it('should preserve lastReadAt so a packed version keeps its place in the eviction order', ()=>{
        ...
        run(localCache, 2);
        expect(localCache.get(`${FIELD_METADATA}:ws-a`)?.versions.get('hash-1')).toMatchObject({
            lastReadAt: IDLE
        });
    });
    ...
    it('should never pack a provider heavier than the whole budget', ()=>{
        ...
        const result = run(localCache, 8, {
            ponderationOf: (localKey)=>localKey.startsWith(FIELD_METADATA) ? 64 : 1
        });
        expect(result.packed).toBe(1);
        expect(result.pending).toBe(0);
    ...
    it('should skip a candidate that does not fit and pack a lighter one behind it', ()=>{
        ...
        const localCache = new Map([
            ...
            [
                `${FIELD_METADATA}:ws-light`,
                liveEntry(IDLE - 1_000)
            ]
        ]);
        const result = run(localCache, 4, {
            ponderationOf: (localKey)=>ponderationBySuffix[localKey.split(':')[1]] ?? 1
        });
        expect(result.packed).toBe(2);
        expect(result.pending).toBe(1);
    ...
    it('should treat a non-finite or below-one ponderation as one', ()=>{
        const localCache = new Map([
            [
                `${FIELD_METADATA}:ws-nan`,
                liveEntry(IDLE - 2_000)
            ],
            [
                `${FIELD_METADATA}:ws-zero`,
                liveEntry(IDLE - 1_000)
            ]
        ]);
        const result = run(localCache, 4, {
...
//# sourceMappingURL=pack-idle-versions.util.spec.js.map