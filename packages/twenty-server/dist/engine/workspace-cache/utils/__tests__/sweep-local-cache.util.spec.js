...
const _sweeplocalcacheutil = require("../sweep-local-cache.util");
const TTL_MS = 30 * 60 * 1000;
const ORM_KEY_NAME = 'ORMEntityMetadatas';
const entry = (lastReadAt)=>({
        versions: new Map([
            [
                'h',
                {
...
describe('sweepLocalCache', ()=>{
    it('drops entries idle past the TTL and keeps recent ones', ()=>{
        const cache = new Map([
            [
                `${ORM_KEY_NAME}:ws-idle`,
                entry(0)
            ],
            [
                `${ORM_KEY_NAME}:ws-recent`,
                entry(TTL_MS)
            ]
        ]);
        const evicted = (0, _sweeplocalcacheutil.sweepLocalCache)(cache, TTL_MS + 1, config());
    ...
    it('trims a provider over its cap, evicting the least-recently-read', ()=>{
        ...
        const evicted = (0, _sweeplocalcacheutil.sweepLocalCache)(cache, 100, config({
            maxEntriesByKeyName: new Map([
                [
                    ORM_KEY_NAME,
                    2
    ...
    it('leaves a provider without a cap untouched', ()=>{
        const cache = new Map(Array.from({
            length: 5
        }, (_, index)=>[
    ...
    it('expires first so the per-provider cap only counts survivors', ()=>{
        ...
        const evicted = (0, _sweeplocalcacheutil.sweepLocalCache)(cache, TTL_MS + 10, config({
            maxEntriesByKeyName: new Map([
                [
                    ORM_KEY_NAME,
                    2
...
//# sourceMappingURL=sweep-local-cache.util.spec.js.map