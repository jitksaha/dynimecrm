...
const _rolepermissionflagservice = require("../../../../metadata-modules/role-permission-flag/role-permission-flag.service");
...
let DevSeederPermissionsService = class DevSeederPermissionsService {
    async initPermissions({ twentyStandardFlatApplication, workspaceCustomFlatApplication, workspaceId, light = false }) {
        ...
        if (workspaceId === _seederworkspacesconstant.SEED_APPLE_WORKSPACE_ID) {
            if (light) {
                ...
                const impersonateOnlyRole = await this.createImpersonateOnlyRoleForSeedWorkspace({
                    workspaceId,
                    ownerFlatApplication: workspaceCustomFlatApplication
                });
                await this.userRoleService.assignRoleToManyUserWorkspace({
                    workspaceId,
                    userWorkspaceIds: [
                        _seeduserworkspacesutil.USER_WORKSPACE_DATA_SEED_IDS.SCOTT
                    ],
    ...
    async createImpersonateOnlyRoleForSeedWorkspace({ ownerFlatApplication, workspaceId }) {
    ...
    async createLimitedRoleForSeedWorkspace({ ownerFlatApplication, workspaceId }) {
        ...
        const personJobTitleFieldMetadata = personObjectMetadata.fields.find((field)=>field.name === 'jobTitle');
    ...
    constructor(roleService, userRoleService, objectPermissionService, objectMetadataRepository, roleRepository, fieldPermissionService, roleTargetService, rolePermissionFlagService, coreDataSource){
...
DevSeederPermissionsService = _ts_decorate([
    ...
    _ts_param(4, (0, _injectworkspacescopedrepositorydecorator.InjectWorkspaceScopedRepository)(_roleentity.RoleEntity)),
