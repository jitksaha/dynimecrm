...
const _seedemailingdomainsutil = require("./seed-emailing-domains.util");
...
const seedConnectedAccounts = async ({ queryRunner, schemaName, workspaceId })=>{
    ...
    const emailGroupDomains = (0, _seedemailingdomainsutil.getSeededEmailGroupDomains)(workspaceId);
...
const seedUnsubscribeTopics = async ({ queryRunner, schemaName, workspaceId })=>{
    ...
    const unsubscribeTopics = [
        ...
        },
        {
            ...
            visibility: _unsubscribetopicvisibilitytype.UnsubscribeTopicVisibility.PRIVATE,
            workspaceId
        }
    ];
    await queryRunner.manager.createQueryBuilder().insert().into(`${schemaName}.unsubscribeTopic`, [
        'id',
        'name',
        'description',
        'visibility',
        'workspaceId'
    ]).orIgnore().values(unsubscribeTopics).execute();
};
