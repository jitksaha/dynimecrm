...
const _timelineactivitytypecacheservice = require("../../../../../modules/timeline/services/timeline-activity-type-cache.service");
const _timelineexception = require("../../../../../modules/timeline/exceptions/timeline.exception");
...
let TimelineActivitySeederService = class TimelineActivitySeederService {
    async seedTimelineActivities({ entityManager, schemaName, workspaceId }) {
        ...
        const resolveTimelineActivityType = await this.timelineActivityTypeCacheService.getTimelineActivityTypeResolver(workspaceId);
        const resolveRequiredTimelineActivityType = (args)=>{
            const timelineActivityType = resolveTimelineActivityType(args);
            if (!(0, _utils.isDefined)(timelineActivityType)) {
                throw new _timelineexception.TimelineException(`Timeline activity type ${args.action} is required to seed workspace ${workspaceId}`);
            }
    ...
    async insertTimelineActivities(entityManager, schemaName, timelineActivities) {
        ...
        for (const batch of timelineActivityBatches){
            await entityManager.createQueryBuilder().insert().into(`${schemaName}.timelineActivity`, [
                'id',
    ...
    createTimelineActivity({ entityType, recordSeed, index, resolveTimelineActivityTypeId }) {
        ...
        const timelineActivity = {
            ...
            timelineActivityTypeSnapshot: JSON.stringify(timelineActivityType.snapshot),
    ...
    computeLinkedTimelineActivityRecords({ activityType, recordSeed, index, activityIndex, linkedObjectMetadataId, calendarEventParticipants, messageParticipants, resolveTimelineActivityTypeId }) {
    ...
    computeLinkedActivityRecord({ activityType, recordSeed, index, activityIndex, linkedObjectMetadataId, targetInfo, resolveTimelineActivityTypeId }) {
        ...
        const timelineActivityType = resolveTimelineActivityTypeId({
            action: 'linked',
            objectUniversalIdentifier: _metadata.STANDARD_OBJECTS[activityType].universalIdentifier
        });
    ...
    constructor(objectMetadataService, timelineActivityTypeCacheService){
