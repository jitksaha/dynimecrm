...
const createStandardViewFieldFlatMetadata = ({ workspaceId, objectName, context: { viewName, viewFieldName, fieldName, position, isVisible, size, aggregateOperation = null, viewFieldGroupName = null }, standardObjectMetadataRelatedEntityIds, dependencyFlatEntityMaps, twentyStandardApplicationId, now })=>{
    ...
    const parentView = dependencyFlatEntityMaps.flatViewMaps.byUniversalIdentifier[viewDefinition.universalIdentifier];
    if (!(0, _utils.isDefined)(parentView)) {
        throw new Error(`Missing parent view ${objectName} ${viewName.toString()} for view field ${viewFieldName}`);
    }
