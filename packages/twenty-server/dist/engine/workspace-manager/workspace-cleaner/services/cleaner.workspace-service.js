...
let CleanerWorkspaceService = class CleanerWorkspaceService {
    ...
    async sendWarningEmail(workspaceMember, workspaceDisplayName, billingSettingsUrl, daysSinceInactive) {
        const emailData = {
            daysSinceInactive,
        ...
        const workspaceDeletionMsg = /*i18n*/ {
            id: "DmYwVp",
            message: "Your workspace is paused — reactivate to keep your data"
        };
    ...
    async warnWorkspaceMembers(workspace, daysSinceInactive, dryRun) {
        ...
        if (!dryRun) {
            for (const workspaceMember of workspaceMembers){
                await this.sendWarningEmail(workspaceMember, workspace.displayName, billingSettingsUrl, daysSinceInactive);
    ...
    constructor(workspaceService, twentyConfigService, userVarsService, userService, emailService, workspaceRepository, billingSubscriptionRepository, billingSubscriptionService, userWorkspaceRepository, i18nService, metricsService, workspaceDomainsService){
