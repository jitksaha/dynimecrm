...
const addUniversalFlatEntityToUniversalFlatEntityAndRelatedEntityMapsThroughMutationOrThrow = ({ metadataName, universalFlatEntity, universalFlatEntityAndRelatedMapsToMutate, skipMissingRelatedEntities = false })=>{
    ...
    for (const relationPropertyName of Object.keys(manyToOneRelations)){
        ...
        if (skipMissingRelatedEntities) {
            const maybeRelatedUniversalFlatEntity = (0, _findflatentitybyuniversalidentifierutil.findFlatEntityByUniversalIdentifier)({
                universalIdentifier: universalForeignKeyValue,
                flatEntityMaps: relatedUniversalFlatEntityMaps
            });
            if (!(0, _utils.isDefined)(maybeRelatedUniversalFlatEntity)) {
                continue;
            }
        }
        ...
        if (!Object.prototype.hasOwnProperty.call(relatedUniversalFlatEntity, universalFlatEntityForeignKeyAggregator)) {
            throw new _flatentitymapsexception.FlatEntityMapsException(`Should never occur, invalid flat entity typing. flat ${relatedMetadataName} should contain ${universalFlatEntityForeignKeyAggregator} (metadataName: ${metadataName}, universalIdentifier: ${universalFlatEntity.universalIdentifier})`, _flatentitymapsexception.FlatEntityMapsExceptionCode.ENTITY_MALFORMED, {
                context: {
