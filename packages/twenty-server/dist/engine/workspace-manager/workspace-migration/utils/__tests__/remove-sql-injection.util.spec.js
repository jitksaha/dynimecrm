...
describe('assertSafeTsVectorExpression', ()=>{
    it('should accept a real server-generated tsvector expression', ()=>{
        ...
        expect(()=>(0, _removesqlinjectionutil.assertSafeTsVectorExpression)(generated)).not.toThrow();
    });
    it('should reject expressions containing a statement terminator', ()=>{
        expect(()=>(0, _removesqlinjectionutil.assertSafeTsVectorExpression)(`to_tsvector('simple', coalesce("name", ''))) STORED; CREATE TABLE core."x" (a text); --`)).toThrow('Unsafe tsvector expression detected');
    });
    it('should reject expressions containing a line comment', ()=>{
        expect(()=>(0, _removesqlinjectionutil.assertSafeTsVectorExpression)(`to_tsvector('simple', '') -- comment`)).toThrow('Unsafe tsvector expression detected');
    });
    it('should reject expressions containing a block comment', ()=>{
        expect(()=>(0, _removesqlinjectionutil.assertSafeTsVectorExpression)(`to_tsvector('simple', '') /* comment */`)).toThrow('Unsafe tsvector expression detected');
    });
    it('should reject null bytes', ()=>{
        expect(()=>(0, _removesqlinjectionutil.assertSafeTsVectorExpression)(`to_tsvector('simple', '\0')`)).toThrow('Unsafe tsvector expression detected');
    });
    it('should reject a parenthesis-balanced clause-injection that uses no forbidden token', ()=>{
        ...
        expect(()=>(0, _removesqlinjectionutil.assertSafeTsVectorExpression)(`to_tsvector('simple', coalesce("x",''))) STORED, ADD COLUMN "evil" text GENERATED ALWAYS AS (to_tsvector('simple', '')`)).toThrow('Unsafe tsvector expression detected');
    });
    it('should reject dollar-quoting used to smuggle a breakout parenthesis', ()=>{
        ...
        expect(()=>(0, _removesqlinjectionutil.assertSafeTsVectorExpression)(`to_tsvector('simple', $$'$$ ) STORED, ADD COLUMN "evil" text $$'$$`)).toThrow('Unsafe tsvector expression detected');
    });
    it('should reject a double-quoted-identifier desync that hides a breakout parenthesis', ()=>{
        ...
        expect(()=>(0, _removesqlinjectionutil.assertSafeTsVectorExpression)(`"'")"'"`)).toThrow('Unsafe tsvector expression detected');
    });
});
describe('isSafeTsVectorExpression', ()=>{
    it('should return true for a safe generated expression', ()=>{
        expect((0, _removesqlinjectionutil.isSafeTsVectorExpression)(`to_tsvector('simple', COALESCE(public.unaccent_immutable("name"), ''))`)).toBe(true);
    });
    it('should accept balanced parentheses that appear inside string literals', ()=>{
        expect((0, _removesqlinjectionutil.isSafeTsVectorExpression)(`to_tsvector('simple', regexp_replace("x"::text, '"(a|b)"\\s*:\\s*', '', 'g'))`)).toBe(true);
        expect((0, _removesqlinjectionutil.isSafeTsVectorExpression)(`COALESCE("x", ')')`)).toBe(true);
    });
    it('should accept parentheses that appear inside a double-quoted identifier', ()=>{
        expect((0, _removesqlinjectionutil.isSafeTsVectorExpression)(`COALESCE("weird)name", '')`)).toBe(true);
    });
    it('should reject expressions with a forbidden token', ()=>{
        expect((0, _removesqlinjectionutil.isSafeTsVectorExpression)(`to_tsvector('simple', '') ; DROP`)).toBe(false);
    });
    it('should reject any expression containing a dollar sign', ()=>{
        expect((0, _removesqlinjectionutil.isSafeTsVectorExpression)(`to_tsvector('simple', $$x$$)`)).toBe(false);
    });
    it('should reject expressions with unbalanced parentheses', ()=>{
        expect((0, _removesqlinjectionutil.isSafeTsVectorExpression)(`coalesce("x", '')) STORED`)).toBe(false);
    });
});
