...
const _validateflatviewfiltercreationutil = require("../../validators/utils/validate-flat-view-filter-creation.util");
const _validateflatviewfilterdeletionutil = require("../../validators/utils/validate-flat-view-filter-deletion.util");
const _validateflatviewfilterupdateutil = require("../../validators/utils/validate-flat-view-filter-update.util");
...
let WorkspaceMigrationViewFilterActionsBuilderService = class WorkspaceMigrationViewFilterActionsBuilderService extends _workspaceentitymigrationbuilderservice.WorkspaceEntityMigrationBuilderService {
    validateFlatEntityCreation(args) {
        const validationResult = (0, _validateflatviewfiltercreationutil.validateFlatViewFilterCreation)(args);
    ...
    validateFlatEntityDeletion(args) {
        const validationResult = (0, _validateflatviewfilterdeletionutil.validateFlatViewFilterDeletion)(args);
        if (validationResult.errors.length > 0) {
            return {
                status: 'fail',
                ...validationResult
    ...
    validateFlatEntityUpdate(args) {
        const validationResult = (0, _validateflatviewfilterupdateutil.validateFlatViewFilterUpdate)(args);
        if (validationResult.errors.length > 0) {
            return {
                status: 'fail',
                ...validationResult
    ...
    constructor(){
        super(_metadata.ALL_METADATA_NAME.viewFilter);
    }
};
