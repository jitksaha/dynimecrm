...
const _deleteflatentityforeignkeyaggregatorsutil = require("../../universal-flat-entity/utils/delete-flat-entity-foreign-key-aggregators.util");
...
let WorkspaceEntityMigrationBuilderService = class WorkspaceEntityMigrationBuilderService {
    async validateAndBuild({ buildOptions, dependencyOptimisticFlatEntityMaps: optimisticFlatEntityMapsAndRelatedFlatEntityMaps, from: fromFlatEntityMaps, to: toFlatEntityMaps, additionalCacheDataMaps, workspaceId }) {
        this.logger.perfTime(`EntityBuilder ${this.metadataName}`, 'validateAndBuild');
        this.logger.perfTime(`EntityBuilder ${this.metadataName}`, 'matrix computation');
        ...
        this.logger.perfTimeEnd(`EntityBuilder ${this.metadataName}`, 'matrix computation');
        this.logger.perfTime(`EntityBuilder ${this.metadataName}`, 'entity processing');
        ...
        this.logger.perfTime(`EntityBuilder ${this.metadataName}`, 'deletion validation');
        ...
        for (const universalIdentifierToDelete of universalIdentifiersToDelete){
            ...
            const universalFlatEntityToDeletePayload = (0, _deleteflatentityforeignkeyaggregatorsutil.deleteFlatEntityForeignKeyAggregators)({
                metadataName: this.metadataName,
                universalFlatEntity: universalFlatEntityToDelete
            });
        ...
        this.logger.perfTimeEnd(`EntityBuilder ${this.metadataName}`, 'deletion validation');
        this.logger.perfTime(`EntityBuilder ${this.metadataName}`, 'creation validation');
        this.recordBuildEntityPhaseMetric({
        ...
        this.logger.perfTimeEnd(`EntityBuilder ${this.metadataName}`, 'creation validation');
        this.logger.perfTime(`EntityBuilder ${this.metadataName}`, 'update validation');
        this.recordBuildEntityPhaseMetric({
        ...
        for(const flatEntityToUpdateUniversalIdentifier in updatedFlatEntityMaps.byUniversalIdentifier){
            ...
            const diff = Object.fromEntries(Object.entries(flatEntityUpdate.update).map(([key, after])=>[
                    key,
                    {
                        before: existingFlatEntity[key],
                        after
                    }
                ]));
        ...
        this.logger.perfTimeEnd(`EntityBuilder ${this.metadataName}`, 'update validation');
        this.logger.perfTimeEnd(`EntityBuilder ${this.metadataName}`, 'entity processing');
        this.recordBuildEntityPhaseMetric({
        ...
        this.logger.perfTimeEnd(`EntityBuilder ${this.metadataName}`, 'validateAndBuild');
        return {
            status: 'success',
            actions: actionsResult
        };
    }
    recordBuildEntityDurationMetric({ status, startedAt }) {
        this.metricsService.recordHistogram({
            key: _metricskeystype.MetricsKeys.WorkspaceMigrationBuildEntityDurationMs,
    ...
    recordBuildEntityPhaseMetric({ phase, startedAt }) {
        this.metricsService.recordHistogram({
            key: _metricskeystype.MetricsKeys.WorkspaceMigrationBuildEntityPhaseDurationMs,
            value: performance.now() - startedAt,
            unit: 'ms',
            attributes: {
                metadataName: this.metadataName,
...
_ts_decorate([
    (0, _common.Inject)(_metricsservice.MetricsService),
    _ts_metadata("design:type", typeof _metricsservice.MetricsService === "undefined" ? Object : _metricsservice.MetricsService)
], WorkspaceEntityMigrationBuilderService.prototype, "metricsService", void 0);
