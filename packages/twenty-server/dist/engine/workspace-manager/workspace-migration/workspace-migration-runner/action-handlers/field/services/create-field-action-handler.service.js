...
let CreateFieldActionHandlerService = class CreateFieldActionHandlerService extends (0, _workspacemigrationrunneractionhandlerserviceinterface.WorkspaceMigrationRunnerActionHandler)('create', 'fieldMetadata') {
    async transpileUniversalActionToFlatAction(context) {
        ...
        const allFieldIdToBeCreatedInActionByUniversalIdentifierMap = new Map(Object.entries(action.fieldIdByUniversalIdentifier ?? {}));
        if (!allFieldIdToBeCreatedInActionByUniversalIdentifierMap.has(action.flatEntity.universalIdentifier)) {
            allFieldIdToBeCreatedInActionByUniversalIdentifierMap.set(action.flatEntity.universalIdentifier, action.id ?? (0, _uuid.v4)());
        }
        if ((0, _utils.isDefined)(action.relatedUniversalFlatFieldMetadata) && !allFieldIdToBeCreatedInActionByUniversalIdentifierMap.has(action.relatedUniversalFlatFieldMetadata.universalIdentifier)) {
    ...
    async executeForWorkspaceSchema(context) {
        const { flatAction, queryRunner, allFlatEntityMaps: { flatObjectMetadataMaps, flatFieldMetadataMaps, flatSearchFieldMetadataMaps }, getSearchFieldMetadatasByTsVectorFieldId, workspaceId } = context;
        const { flatEntity, relatedFlatFieldMetadata } = flatAction;
        ...
        for (const [objectMetadataId, createdFlatFieldMetadatas] of fieldsByObjectMetadataId){
            ...
            const objectFlatFieldMetadatas = [
                ...(0, _findmanyflatentitybyidinflatentitymapsutil.findManyFlatEntityByIdInFlatEntityMaps)({
    ...
    async executeSingleFieldMetadataWorkspaceSchema({ flatFieldMetadata, flatObjectMetadata, flatObjectMetadataMaps, objectFlatFieldMetadatas, flatSearchFieldMetadataMaps, getSearchFieldMetadatasByTsVectorFieldId, queryRunner, schemaName, tableName, workspaceId }) {
        ...
        const columnDefinitions = (0, _generatecolumndefinitionsutil.generateColumnDefinitions)({
            ...
            flatObjectMetadata,
            workspaceId,
            searchVectorAsExpression: (0, _isflatfieldmetadataoftypeutil.isFlatFieldMetadataOfType)(flatFieldMetadata, _types.FieldMetadataType.TS_VECTOR) ? (0, _resolvesearchvectorasexpressionfortsvectorfieldutil.resolveSearchVectorAsExpressionForTsVectorField)({
