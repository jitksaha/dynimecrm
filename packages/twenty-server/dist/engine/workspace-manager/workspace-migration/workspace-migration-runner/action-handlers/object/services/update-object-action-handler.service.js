...
let UpdateObjectActionHandlerService = class UpdateObjectActionHandlerService extends (0, _workspacemigrationrunneractionhandlerserviceinterface.WorkspaceMigrationRunnerActionHandler)('update', 'objectMetadata') {
    async transpileUniversalActionToFlatAction(context) {
        ...
        if ((0, _utils.isDefined)(labelIdentifierFieldMetadataUniversalIdentifier)) {
            transpiledUpdate.labelIdentifierFieldMetadataId = this.resolveFieldMetadataIdOrThrow({
                context,
        ...
        if ((0, _utils.isDefined)(imageIdentifierFieldMetadataUniversalIdentifier)) {
            transpiledUpdate.imageIdentifierFieldMetadataId = this.resolveFieldMetadataIdOrThrow({
                context,
    ...
    resolveFieldMetadataIdOrThrow({ context, universalIdentifier, foreignKeyName }) {
        const { allFlatEntityMaps, preallocatedIdByUniversalIdentifierByMetadataName } = context;
        const preallocatedFieldMetadataId = preallocatedIdByUniversalIdentifierByMetadataName?.fieldMetadata?.[universalIdentifier];
        if ((0, _utils.isDefined)(preallocatedFieldMetadataId)) {
        ...
        if (!(0, _utils.isDefined)(flatFieldMetadata)) {
            throw new _flatentitymapsexception.FlatEntityMapsException(`Could not resolve ${foreignKeyName}: no fieldMetadata found for universal identifier ${universalIdentifier}`, _flatentitymapsexception.FlatEntityMapsExceptionCode.ENTITY_NOT_FOUND);
        }
