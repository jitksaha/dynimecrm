...
const _fromuniversaloverridestoviewoverridesutil = require("./utils/from-universal-overrides-to-view-overrides.util");
...
let CreateViewActionHandlerService = class CreateViewActionHandlerService extends (0, _workspacemigrationrunneractionhandlerserviceinterface.WorkspaceMigrationRunnerActionHandler)('create', 'view') {
    async transpileUniversalActionToFlatAction({ action, allFlatEntityMaps, flatApplication, workspaceId }) {
        const { calendarFieldMetadataId, calendarEndFieldMetadataId, kanbanAggregateOperationFieldMetadataId, mainGroupByFieldMetadataId, objectMetadataId } = (0, _resolveuniversalrelationidentifierstoidsutil.resolveUniversalRelationIdentifiersToIds)({
            flatEntityMaps: allFlatEntityMaps,
            metadataName: action.metadataName,
            universalForeignKeyValues: action.flatEntity
        });
        const overrides = (0, _utils.isDefined)(action.flatEntity.universalOverrides) ? (0, _fromuniversaloverridestoviewoverridesutil.fromUniversalOverridesToViewOverrides)({
