import{N as o,z as f}from"./types-BVPyJxl2-BGV2ej5I.js";import{t as a}from"./isDefined-Dtu5EYqP-_d6Dqdoe.js";import{G as y,c as $,tn as R}from"./utils-wA24uq8M-QF6HonJV.js";import{Jt as E}from"./graphql-Bmj7vfTz.js";import{t as d}from"./isUndefinedOrNull-BgCnd9uY.js";import{n as l}from"./useObjectPermissions-DHmZToEp.js";import{t as C}from"./isNonCompositeField-CUzJdyYn.js";var _=t=>t.type===o.MORPH_RELATION?(t.morphRelations??[]).map(e=>$({fieldName:t.name,relationType:e.type,targetObjectMetadataNameSingular:e.targetObjectMetadata.nameSingular,targetObjectMetadataNamePlural:e.targetObjectMetadata.namePlural})):[R({name:t.name})],b=({objectMetadataItems:t,gqlField:e,fieldMetadata:n,relationRecordGqlFields:O,computeReferences:u=!1,objectPermissionsByObjectMetadataId:m})=>{const s=n.type;if(C(s))return e;if(s===o.MORPH_RELATION&&(n.settings?.relationType===f.ONE_TO_MANY||n.settings?.relationType===f.MANY_TO_ONE)){let p="";for(const r of n.morphRelations??[]){const i=y({fieldName:n.name,relationType:n.settings?.relationType,targetObjectMetadataNameSingular:r.targetObjectMetadata.nameSingular,targetObjectMetadataNamePlural:r.targetObjectMetadata.namePlural}),N=t.find(L=>L.id===r.targetObjectMetadata.id);if(a(N)){if(a(m)&&a(N.id)){if(!a(r.targetObjectMetadata.id))throw new Error(`Target object metadata id not found with field metadata ${n.name}`);if(!l(m,r.targetObjectMetadata.id).canReadObjectRecords)continue}if(n.settings?.relationType===f.ONE_TO_MANY){if(e!==i)continue;p+=`${i}
{
  edges {
