...
const _parseicaleventutil = require("../parse-ical-event.util");
...
const buildVEvent = (lines)=>buildVCalendar([
        lines
    ]);
describe('parseICalEvents', ()=>{
    it('returns an empty array when the payload contains no VEVENT', ()=>{
        ...
        expect((0, _parseicaleventutil.parseICalEvents)(ics, HREF)).toEqual([]);
    });
    it('extracts the canonical VEVENT fields', ()=>{
        ...
        const [event] = (0, _parseicaleventutil.parseICalEvents)(ics, HREF);
    ...
    it('falls back to "Untitled Event" when SUMMARY is missing', ()=>{
        ...
        expect((0, _parseicaleventutil.parseICalEvents)(ics, HREF)[0].title).toBe('Untitled Event');
    });
    it('flags isFullDay when DTSTART carries VALUE=DATE', ()=>{
        ...
        expect((0, _parseicaleventutil.parseICalEvents)(ics, HREF)[0].isFullDay).toBe(true);
    });
    it('classifies isFullDay per-event when a full-day master has a timed recurrence override', ()=>{
        ...
        const [master, override] = (0, _parseicaleventutil.parseICalEvents)(ics, HREF);
    ...
    it('attaches recurringEventExternalId in ISO format when RECURRENCE-ID is present', ()=>{
        ...
        expect((0, _parseicaleventutil.parseICalEvents)(ics, HREF)[0].recurringEventExternalId).toBe('2026-06-01T10:00:00.000Z');
    });
    it('returns master + override events as separate entries with unique ids (RFC 5545 §3.8.4.4)', ()=>{
        ...
        const events = (0, _parseicaleventutil.parseICalEvents)(ics, HREF);
    ...
    it('handles bare ORGANIZER (plain mailto, no CN) — node-ical returns a string', ()=>{
        ...
        expect((0, _parseicaleventutil.parseICalEvents)(ics, HREF)[0].participants[0]).toMatchObject({
    ...
    it('handles bare ATTENDEE (plain mailto, no params) — node-ical returns a string', ()=>{
        ...
        const attendee = (0, _parseicaleventutil.parseICalEvents)(ics, HREF)[0].participants.find((p)=>!p.isOrganizer);
    ...
    it('maps each ATTENDEE PARTSTAT to the matching response status', ()=>{
        ...
        const [event] = (0, _parseicaleventutil.parseICalEvents)(ics, HREF);
        const byHandle = Object.fromEntries(event.participants.map((p)=>[
                p.handle,
                p.responseStatus
            ]));
        expect(byHandle).toEqual({
            'bob@example.com': _calendareventparticipantworkspaceentity.CalendarEventParticipantResponseStatus.ACCEPTED,
            'carol@example.com': _calendareventparticipantworkspaceentity.CalendarEventParticipantResponseStatus.DECLINED,
            'dan@example.com': _calendareventparticipantworkspaceentity.CalendarEventParticipantResponseStatus.TENTATIVE,
            'eve@example.com': _calendareventparticipantworkspaceentity.CalendarEventParticipantResponseStatus.NEEDS_ACTION,
            'frank@example.com': _calendareventparticipantworkspaceentity.CalendarEventParticipantResponseStatus.NEEDS_ACTION
        });
    });
    it('returns an empty array and does not throw when iCal data is malformed', ()=>{
        expect((0, _parseicaleventutil.parseICalEvents)('not a calendar', HREF)).toEqual([]);
    });
    it('skips a VEVENT missing DTSTART without dropping its siblings', ()=>{
        ...
        const events = (0, _parseicaleventutil.parseICalEvents)(ics, HREF);
        expect(events.map((event)=>event.iCalUid)).toEqual([
            'healthy'
        ]);
    });
});

//# sourceMappingURL=parse-ical-event.util.spec.js.map