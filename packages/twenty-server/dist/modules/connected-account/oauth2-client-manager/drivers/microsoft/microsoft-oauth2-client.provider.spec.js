...
const _microsoftoauth2clientprovider = require("./microsoft-oauth2-client.provider");
...
describe('MicrosoftOAuth2ClientProvider', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        provider = module.get(_microsoftoauth2clientprovider.MicrosoftOAuth2ClientProvider);
    });
    afterEach(()=>{
        jest.clearAllMocks();
    });
    describe('getClient', ()=>{
        ...
        it('should throw when resolveTokens returns no access token', async ()=>{
            ...
            await expect(provider.getClient(mockConnectedAccountId)).rejects.toThrow(_connectedaccountrefreshtokensexception.ConnectedAccountRefreshAccessTokenException);
            await expect(provider.getClient(mockConnectedAccountId)).rejects.toMatchObject({
                code: _connectedaccountrefreshtokensexception.ConnectedAccountRefreshAccessTokenExceptionCode.ACCESS_TOKEN_NOT_FOUND
            });
        });
    });
});

//# sourceMappingURL=microsoft-oauth2-client.provider.spec.js.map