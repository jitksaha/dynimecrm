"use strict";
Object.defineProperty(exports, "__esModule", {
    value: true
});
const _isthrottled = require("../is-throttled");
describe('isThrottled', ()=>{
    it('should not throttle when no sync stage is active', ()=>{
        expect((0, _isthrottled.isThrottled)(null, 3)).toBe(false);
    });
    it('should throttle until retryAfter even when no sync stage is active', ()=>{
        const fiveMinutesFromNow = new Date(Date.now() + 5 * 60 * 1000).toISOString();
        expect((0, _isthrottled.isThrottled)(null, 3, fiveMinutesFromNow)).toBe(true);
    });
    it('should not throttle when there have been no failures', ()=>{
        expect((0, _isthrottled.isThrottled)(new Date().toISOString(), 0)).toBe(false);
    });
    it('should throttle until retryAfter even when there have been no failures', ()=>{
        const fiveMinutesFromNow = new Date(Date.now() + 5 * 60 * 1000).toISOString();
        expect((0, _isthrottled.isThrottled)(new Date().toISOString(), 0, fiveMinutesFromNow)).toBe(true);
    });
    it('should keep throttling when retryAfter is in the future even though exponential backoff has expired', ()=>{
        const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000).toISOString();
        const fiveMinutesFromNow = new Date(Date.now() + 5 * 60 * 1000).toISOString();
        expect((0, _isthrottled.isThrottled)(tenMinutesAgo, 1, fiveMinutesFromNow)).toBe(true);
    });
    it('should fall back to exponential backoff when retryAfter is not provided', ()=>{
        const justNow = new Date().toISOString();
        expect((0, _isthrottled.isThrottled)(justNow, 1)).toBe(true);
    });
    it('should fall back to exponential backoff when retryAfter is in the past', ()=>{
        const justNow = new Date().toISOString();
        const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000).toISOString();
        expect((0, _isthrottled.isThrottled)(justNow, 1, fiveMinutesAgo)).toBe(true);
    });
    it('should fall back to exponential backoff when retryAfter is explicitly null', ()=>{
        const justNow = new Date().toISOString();
        expect((0, _isthrottled.isThrottled)(justNow, 1, null)).toBe(true);
    });
    it('should fall back to exponential backoff when retryAfter is an invalid date string', ()=>{
        const justNow = new Date().toISOString();
        expect((0, _isthrottled.isThrottled)(justNow, 1, 'not-a-date')).toBe(true);
    });
    it('should use exponential backoff when both backoff is active and retryAfter is in the past', ()=>{
        const justNow = new Date().toISOString();
        const oneMinuteAgo = new Date(Date.now() - 60 * 1000).toISOString();
        expect((0, _isthrottled.isThrottled)(justNow, 1, oneMinuteAgo)).toBe(true);
    });
});

//# sourceMappingURL=is-throttled.spec.js.map