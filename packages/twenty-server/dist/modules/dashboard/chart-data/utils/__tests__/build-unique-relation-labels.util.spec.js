...
const _builduniquerelationlabelsutil = require("../build-unique-relation-labels.util");
describe('buildUniqueRelationLabels', ()=>{
    it('should keep unique labels untouched', ()=>{
        const { labelByRecordId, unresolvedRecordIds } = (0, _builduniquerelationlabelsutil.buildUniqueRelationLabels)({
    ...
    it('should suffix every member of a colliding label group', ()=>{
        const { labelByRecordId } = (0, _builduniquerelationlabelsutil.buildUniqueRelationLabels)({
            rawLabelByRecordId: new Map([
                [
                    'id-1',
                    'John Smith'
    ...
    it('should assign ordinals by ascending record id regardless of input order', ()=>{
        const resolutionFromShuffledInput = (0, _builduniquerelationlabelsutil.buildUniqueRelationLabels)({
            rawLabelByRecordId: new Map([
                [
                    'id-b',
                    'John Smith'
        ...
        expect(resolutionFromShuffledInput.labelByRecordId.get('id-a')).toBe('John Smith (1)');
        expect(resolutionFromShuffledInput.labelByRecordId.get('id-b')).toBe('John Smith (2)');
    });
    it('should classify missing labels as unresolved without assigning a label', ()=>{
        ...
        expect(unresolvedRecordIds).toEqual(new Set([
            'id-2'
        ]));
    });
    ...
    it('should not reuse a label already taken by a suffixed record', ()=>{
        ...
        const assignedLabels = [
            ...labelByRecordId.values()
        ];
        expect(new Set(assignedLabels).size).toBe(assignedLabels.length);
    });
});

//# sourceMappingURL=build-unique-relation-labels.util.spec.js.map