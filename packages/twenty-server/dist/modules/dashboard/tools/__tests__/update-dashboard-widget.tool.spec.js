...
const createTool = (deps)=>(0, _updatedashboardwidgettool.createUpdateDashboardWidgetTool)(deps, {
...
describe('update_dashboard_widget tool', ()=>{
    it('resolves filter field names using the existing widget object when the object is not changed', async ()=>{
        ...
        const updateData = deps.pageLayoutWidgetService.update.mock.calls[0][0].updateData;
        expect(updateData.configuration.aggregateFieldMetadataId).toBe(AMOUNT_FIELD_ID);
        expect(updateData.configuration.filter.recordFilters[0]).toMatchObject({
            fieldMetadataId: STAGE_FIELD_ID,
            operand: _types.ViewFilterOperand.IS
        });
        expect(updateData.configuration.filter.recordFilters[0]).not.toHaveProperty('fieldName');
    });
    ...
    it('does not resolve identifiers when only non-field properties change', async ()=>{
        ...
        expect(result.success).toBe(true);
        expect(deps.flatEntityMapsCacheService.getOrRecomputeManyOrAllFlatEntityMaps).not.toHaveBeenCalled();
        expect(deps.pageLayoutWidgetService.findByIdOrThrow).not.toHaveBeenCalled();
        const updateData = deps.pageLayoutWidgetService.update.mock.calls[0][0].updateData;
...
//# sourceMappingURL=update-dashboard-widget.tool.spec.js.map