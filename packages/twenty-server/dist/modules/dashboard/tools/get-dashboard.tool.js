...
const createGetDashboardTool = (deps, context)=>({
        ...
        execute: async (parameters)=>{
            try {
                ...
                const dashboard = await deps.workspaceOrmManager.executeInWorkspaceContext(async ()=>{
                ...
                const tabs = pageLayout.tabs?.map((tab)=>({
                        ...
                        widgets: tab.widgets?.map((w)=>{
                            if (w.type !== _types.WidgetType.GRAPH || !(0, _ischartreferencingfieldinconfigurationutil.isChartReferencingFieldInConfiguration)(w.configuration)) {
                                return {
                                    id: w.id,
                                    title: w.title,
                                    type: w.type,
