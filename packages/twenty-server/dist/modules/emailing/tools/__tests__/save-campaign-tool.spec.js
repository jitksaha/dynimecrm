...
const _totooljsonschemautil = require("../../../../engine/core-modules/record-crud/utils/to-tool-json-schema.util");
const _messagecampaigndraftservice = require("../../services/message-campaign-draft.service");
const _savecampaigntool = require("../save-campaign-tool");
...
describe('SaveCampaignTool', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        const module = await _testing.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: _messagecampaigndraftservice.MessageCampaignDraftService,
                    useValue: {
                        saveDraft
                    }
                }
        ...
        tool = module.get(_savecampaigntool.SaveCampaignTool);
    });
    afterEach(()=>{
        jest.clearAllMocks();
    });
    ...
    it('should expose an input schema convertible to JSON schema for the LLM', ()=>{
        const jsonSchema = (0, _totooljsonschemautil.toToolJsonSchema)(_savecampaigntoolschema.SaveCampaignToolInputZodSchema);
    ...
    it('should surface domain errors as tool output', async ()=>{
        saveDraft.mockRejectedValue(new _emailingdomainexception.EmailingDomainException('Campaign is SENT; only draft campaigns can be edited', _emailingdomainexception.EmailingDomainExceptionCode.MESSAGE_CAMPAIGN_NOT_SENDABLE));
        const output = await tool.execute({
            campaignId,
            subject: 'New subject'
        }, {
...
//# sourceMappingURL=save-campaign-tool.spec.js.map