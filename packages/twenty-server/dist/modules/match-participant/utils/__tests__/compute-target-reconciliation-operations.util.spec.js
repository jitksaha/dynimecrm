...
const _computetargetreconciliationoperationsutil = require("../compute-target-reconciliation-operations.util");
const personTarget = {
    parentId: 'parent-1',
...
describe('computeTargetReconciliationOperations', ()=>{
    it('creates missing automatic targets', ()=>{
        expect((0, _computetargetreconciliationoperationsutil.computeTargetReconciliationOperations)({
            desiredTargets: [
                personTarget
            ],
            existingTargets: []
    ...
    it('adds automatic provenance to an existing manual target', ()=>{
        ...
        }).targetsToMarkAutomatic).toEqual([
            'target-1'
        ]);
    });
    it('keeps a manual target when its automatic reason disappears', ()=>{
        ...
        }).targetsToMarkNotAutomatic).toEqual([
            'target-1'
        ]);
    });
    ...
    it('deduplicates desired targets', ()=>{
        expect((0, _computetargetreconciliationoperationsutil.computeTargetReconciliationOperations)({
            ...
            existingTargets: []
        }).targetsToCreate).toEqual([
            personTarget
        ]);
    });
});

//# sourceMappingURL=compute-target-reconciliation-operations.util.spec.js.map