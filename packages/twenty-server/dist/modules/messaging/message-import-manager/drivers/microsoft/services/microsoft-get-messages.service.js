...
let MicrosoftGetMessagesService = class MicrosoftGetMessagesService {
    ...
    formatBatchResponseAsMessages(batchResponse, connectedAccount) {
        ...
        const messages = parsedResponses.map((response)=>{
            ...
            const safeParseReplyTo = response?.replyTo?.filter(_utils.isDefined).map((recipient)=>(0, _safeparseemailaddressutil.safeParseEmailAddress)(recipient.emailAddress));
            ...
            const participants = [
                ...
                ...(0, _buildreplytoparticipantsutil.buildReplyToParticipants)(safeParseReplyTo, safeParseFrom[0]),
            ...
            const text = (0, _extractmessagebodytextutil.extractMessageBodyText)(response.body?.contentType === 'text' ? {
