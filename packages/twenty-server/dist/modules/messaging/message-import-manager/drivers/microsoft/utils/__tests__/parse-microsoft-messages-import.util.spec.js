...
const _parsemicrosoftmessagesimportutil = require("../parse-microsoft-messages-import.util");
describe('parseMicrosoftMessagesImportError', ()=>{
    it('should be temporary when the access token is expired so the next attempt can refresh it', ()=>{
        const exception = (0, _parsemicrosoftmessagesimportutil.parseMicrosoftMessagesImportError)({
            statusCode: 401,
    ...
    it('should keep the Microsoft error code and message so the failure stays diagnosable', ()=>{
        ...
        expect(exception.message).toContain('Lifetime validation failed, the token is expired.');
    });
    ...
    it('should be not found for a 404 that is not a mailbox error', ()=>{
        ...
        expect(exception.code).toBe(_messageimportdriverexception.MessageImportDriverExceptionCode.NOT_FOUND);
    });
...
//# sourceMappingURL=parse-microsoft-messages-import.util.spec.js.map