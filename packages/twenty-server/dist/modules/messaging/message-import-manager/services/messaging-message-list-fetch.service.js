...
let MessagingMessageListFetchService = class MessagingMessageListFetchService {
    async processMessageListFetch(messageChannel, workspaceId) {
        ...
        await this.workspaceOrmManager.executeInWorkspaceContext(async ()=>{
            try {
                ...
                const processedfolderActionsResult = await this.processPendingFolderActions(messageChannel, workspaceId);
                await this.messageChannelSyncStatusService.markAsMessagesListFetchOngoing([
                    messageChannel.id
                ], workspaceId);
                this.logger.log(`WorkspaceId: ${workspaceId}, MessageChannelId: ${messageChannel.id} - Processing message list fetch`);
                const freshMessageChannel = pendingGroupEmailActionsProcessed || (0, _utils.isDefined)(processedfolderActionsResult) ? await this.messageChannelRepository.findOne({
                    where: {
                ...
                const messageExternalIds = [
                    ...messageLists.flatMap((messageList)=>messageList.messageExternalIds),
                ...
                const fullSyncMessageChannelMessageAssociationsToDelete = isFullSync ? await this.computeFullSyncMessageChannelMessageAssociationsToDelete(freshMessageChannel, messageExternalIds) : [];
    ...
    async processPendingFolderActions(messageChannel, workspaceId) {
        ...
        return this.messagingProcessFolderActionsService.processFolderActions(messageChannel, foldersWithPendingActions, workspaceId);
    }
    async computeFullSyncMessageChannelMessageAssociationsToDelete(messageChannel, messageExternalIds) {
        const messageChannelMessageAssociationRepository = this.workspaceOrmManager.getRepository('messageChannelMessageAssociation');
        ...
        while((0, _utils.isDefined)(nextFirstBatchMessageChannelMessageAssociationId)){
            ...
            const messageChannelMessageAssociationsToDelete = existingMessageChannelMessageAssociations.filter((existingMessageChannelMessageAssociation)=>(0, _utils.isDefined)(existingMessageChannelMessageAssociation.messageExternalId) && !messageExternalIdSet.has(existingMessageChannelMessageAssociation.messageExternalId));
