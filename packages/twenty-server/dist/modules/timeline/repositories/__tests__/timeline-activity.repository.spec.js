...
const _timelineactivityrepository = require("../timeline-activity.repository");
...
const TIMELINE_ACTIVITY_TYPE_SNAPSHOT = {
...
describe('TimelineActivityRepository', ()=>{
    it('merges and stamps a recent row written without a snapshot', async ()=>{
        ...
        const workspaceOrmManager = {
            ...
            runInWorkspaceTransaction: jest.fn(async (callback)=>callback({
                    getRepository: ()=>workspaceRepository,
                    executeRawQuery: jest.fn().mockResolvedValue([])
                }))
        };
        const repository = new _timelineactivityrepository.TimelineActivityRepository(workspaceOrmManager);
        ...
        expect(update).toHaveBeenCalledWith('20202020-0000-4000-8000-000000000006', {
            properties: {
                diff: {
                    name: {
                        before: 'Before',
   ...
   ORDER BY "ordinality"`;
        ...
        expect(workspaceRepository.find.mock.invocationCallOrder[0]).toBeGreaterThan(executeRawQuery.mock.invocationCallOrder[0]);
    });
});

//# sourceMappingURL=timeline-activity.repository.spec.js.map