...
const _resolvetimelineactivitytypeutil = require("../resolve-timeline-activity-type.util");
...
const NOTE_TARGETS_UNIVERSAL_IDENTIFIER = '00000000-0000-4000-8000-000000000105';
const FRONT_COMPONENT_UNIVERSAL_IDENTIFIER = '00000000-0000-4000-8000-000000000020';
...
describe('buildTimelineActivityTypeResolution', ()=>{
    const { resolveTimelineActivityType } = (0, _resolvetimelineactivitytypeutil.buildTimelineActivityTypeResolution)(flatTimelineActivityTypeMaps);
    it('returns the object-bound type and its immutable render snapshot', ()=>{
        expect(resolveTimelineActivityType({
            action: 'linked',
            objectUniversalIdentifier: NOTE_UNIVERSAL_IDENTIFIER
        })).toEqual({
            id: NOTE_LINKED_ID,
    ...
    it('falls back to a shared type', ()=>{
        ...
        })?.id).toBe(SHARED_LINKED_ID);
    });
    it('does not fall back when the matching object type is inactive', ()=>{
        const { resolveTimelineActivityType: resolver } = (0, _resolvetimelineactivitytypeutil.buildTimelineActivityTypeResolution)({
    ...
    it('does not treat a routed type as a source record resolver', ()=>{
        const { resolveTimelineActivityType: resolver, effectiveTimelineActivityTypes } = (0, _resolvetimelineactivitytypeutil.buildTimelineActivityTypeResolution)({
        ...
        expect(resolver({
            action: 'linked',
            objectUniversalIdentifier: NOTE_UNIVERSAL_IDENTIFIER
        })?.id).toBe(NOTE_LINKED_ID);
        expect(effectiveTimelineActivityTypes.map(({ id })=>id)).toContain('00000000-0000-4000-8000-000000000004');
    });
    it('isolates a routed emit conflict from the source record resolver', ()=>{
        ...
        const { resolveTimelineActivityType: resolver, routingConflicts, effectiveTimelineActivityTypes } = (0, _resolvetimelineactivitytypeutil.buildTimelineActivityTypeResolution)({
            objectMetadataByUniversalIdentifier: flatTimelineActivityTypeMaps.objectMetadataByUniversalIdentifier,
            byUniversalIdentifier: {
                ...flatTimelineActivityTypeMaps.byUniversalIdentifier,
        ...
        expect(effectiveTimelineActivityTypes.map(({ id })=>id)).not.toContain(routedTimelineActivityType.id);
        expect(resolver({
            action: 'linked',
            objectUniversalIdentifier: NOTE_UNIVERSAL_IDENTIFIER
        })?.id).toBe(NOTE_LINKED_ID);
    ...
    it('isolates an ambiguous object contract without falling back', ()=>{
        const { resolveTimelineActivityType: resolver, routingConflicts, resolverConflicts } = (0, _resolvetimelineactivitytypeutil.buildTimelineActivityTypeResolution)({
            objectMetadataByUniversalIdentifier: flatTimelineActivityTypeMaps.objectMetadataByUniversalIdentifier,
            byUniversalIdentifier: {
                ...flatTimelineActivityTypeMaps.byUniversalIdentifier,
                duplicate: {
    ...
    it('selects an explicit override deterministically', ()=>{
        ...
        const { resolveTimelineActivityType: resolver, resolverConflicts } = (0, _resolvetimelineactivitytypeutil.buildTimelineActivityTypeResolution)({
            objectMetadataByUniversalIdentifier: flatTimelineActivityTypeMaps.objectMetadataByUniversalIdentifier,
            byUniversalIdentifier: {
                ...flatTimelineActivityTypeMaps.byUniversalIdentifier,
                override: {
                    ...noteLinkedTimelineActivityType,
                    id: '00000000-0000-4000-8000-000000000003',
                    applicationUniversalIdentifier: '00000000-0000-4000-8000-000000000032',
                    universalIdentifier: overrideUniversalIdentifier,
                    replacesTimelineActivityTypeUniversalIdentifier: noteLinkedTimelineActivityType.universalIdentifier
                }
            }
        });
        expect(resolver({
            action: 'linked',
            objectUniversalIdentifier: NOTE_UNIVERSAL_IDENTIFIER
        })?.snapshot.universalIdentifier).toBe(overrideUniversalIdentifier);
        expect(resolverConflicts).toEqual([]);
    });
    it('ignores and reports an implicit foreign-object takeover', ()=>{
        const { resolveTimelineActivityType: resolver, invalidContracts } = (0, _resolvetimelineactivitytypeutil.buildTimelineActivityTypeResolution)({
            objectMetadataByUniversalIdentifier: flatTimelineActivityTypeMaps.objectMetadataByUniversalIdentifier,
            byUniversalIdentifier: {
                ...flatTimelineActivityTypeMaps.byUniversalIdentifier,
                takeover: {
...
//# sourceMappingURL=resolve-timeline-activity-type.util.spec.js.map