...
describe('removeStep', ()=>{
    ...
    it('should replace with EMPTY node when deleting a leaf step on an If branch', ()=>{
        ...
        expect(replacementNode?.type).toBe(_workflow.WorkflowActionType.EMPTY);
    ...
    it('should not replace with EMPTY node when deleted step has children that get re-linked', ()=>{
        ...
        const hasEmptyNode = result.updatedSteps?.some((step)=>step.type === _workflow.WorkflowActionType.EMPTY);
