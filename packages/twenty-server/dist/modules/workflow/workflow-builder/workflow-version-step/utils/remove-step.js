...
const removeStep = ({ existingTrigger, existingSteps, stepIdToDelete, stepToDeleteChildrenIds })=>{
    ...
    const updatedSteps = existingSteps?.filter((step)=>!allRemovedStepIds.includes(step.id)).map((step)=>{
        if (step.type === _workflow.WorkflowActionType.IF_ELSE) {
        ...
        if (step.type === _workflow.WorkflowActionType.ITERATOR && (0, _utils.isDefined)(step.settings.input.initialLoopStepIds) && step.settings.input.initialLoopStepIds.includes(stepIdToDelete)) {
