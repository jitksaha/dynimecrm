...
function evaluateTextAndArrayFilter(filter, filterType, compositeFieldSubFieldName) {
    ...
    switch(filter.operand){
        ...
        case _types.ViewFilterOperand.IS:
            return (0, _lodashisequal.default)(filter.leftOperand, filter.rightOperand);
...
function parseDateOperandToInstant(value) {
    try {
        if (value instanceof Date && !Number.isNaN(value.getTime())) {
            return _temporalpolyfill.Temporal.Instant.fromEpochMilliseconds(value.getTime());
        }
        return (0, _utils.parseToInstantOrThrow)(String(value));
    } catch  {
        return null;
    }
}
function toUtcPlainDate(instant) {
    return instant.toZonedDateTimeISO('UTC').toPlainDate();
}
function evaluateDateFilter(filter) {
    if (filter.operand === _types.ViewFilterOperand.IS_EMPTY) {
        return !(0, _utils.isDefined)(filter.leftOperand) || filter.leftOperand === '';
    }
    if (filter.operand === _types.ViewFilterOperand.IS_NOT_EMPTY) {
        return (0, _utils.isDefined)(filter.leftOperand) && filter.leftOperand !== '';
    }
    const leftInstant = parseDateOperandToInstant(filter.leftOperand);
    if (!(0, _utils.isDefined)(leftInstant)) {
        return false;
    }
    switch(filter.operand){
        case _types.ViewFilterOperand.IS:
            {
                ...
                return (0, _utils.isDefined)(rightInstant) && (0, _utils.isSamePlainDate)(toUtcPlainDate(leftInstant), toUtcPlainDate(rightInstant));
            }
        case _types.ViewFilterOperand.IS_IN_PAST:
            return _temporalpolyfill.Temporal.Instant.compare(leftInstant, _temporalpolyfill.Temporal.Now.instant()) < 0;
        case _types.ViewFilterOperand.IS_IN_FUTURE:
            return _temporalpolyfill.Temporal.Instant.compare(leftInstant, _temporalpolyfill.Temporal.Now.instant()) > 0;
        case _types.ViewFilterOperand.IS_TODAY:
            return (0, _utils.isSamePlainDate)(toUtcPlainDate(leftInstant), _temporalpolyfill.Temporal.Now.zonedDateTimeISO('UTC').toPlainDate());
        case _types.ViewFilterOperand.IS_BEFORE:
            {
                const rightInstant = parseDateOperandToInstant(filter.rightOperand);
                return (0, _utils.isDefined)(rightInstant) && _temporalpolyfill.Temporal.Instant.compare(leftInstant, rightInstant) < 0;
            }
