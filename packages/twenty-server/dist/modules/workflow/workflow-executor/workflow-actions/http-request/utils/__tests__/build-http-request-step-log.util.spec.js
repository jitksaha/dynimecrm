...
const _buildhttprequeststeplogutil = require("../build-http-request-step-log.util");
const baseInput = {
...
describe('buildHttpRequestStepLog', ()=>{
    it('redacts sensitive request and response headers', ()=>{
        const stepLog = (0, _buildhttprequeststeplogutil.buildHttpRequestStepLog)({
            input: baseInput,
            output: baseOutput,
            durationMs: 42
        });
        ...
        expect(stepLog.details.request.headers['content-type']).toBe('application/json');
        expect(stepLog.details.response?.headers['set-cookie']).toBe('[redacted]');
        expect(stepLog.details.response?.headers['content-type']).toBe('application/json');
    });
    ...
    it('truncates non-ASCII bodies by UTF-8 bytes, not UTF-16 code units', ()=>{
        ...
        const truncatedByteLength = Buffer.byteLength(truncatedBody.replace('…[truncated]', ''), 'utf8');
    ...
    it('leaves the URL untouched when there are no sensitive params', ()=>{
        ...
        expect(stepLog.details.request.url).toBe('https://api.example.com/widgets?page=2');
    });
    it('redacts sensitive query params even when the URL is unparseable (regression)', ()=>{
        const unparseableUrl = 'https://api example.com/data?page=1&api_key=AKIA-leaked&token=oauth-leaked&safe=ok';
...
//# sourceMappingURL=build-http-request-step-log.util.spec.js.map