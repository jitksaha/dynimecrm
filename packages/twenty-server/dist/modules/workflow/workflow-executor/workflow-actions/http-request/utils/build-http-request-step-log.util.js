...
Object.defineProperty(exports, "buildHttpRequestStepLog", {
    enumerable: true,
    get: function() {
        return buildHttpRequestStepLog;
    }
});
...
const SENSITIVE_BODY_KEY_REGEX = /^(password|passwd|pwd|.*_?token|.*_?secret|authorization|api[_-]?key|private[_-]?key|client[_-]?secret|x-?api-?key|x-?auth-?token|access[_-]?key)$/i;
const isSensitiveHeader = (name)=>SENSITIVE_HEADER_NAMES.has(name.toLowerCase());
const isSensitiveUrlParam = (name)=>SENSITIVE_URL_PARAM_NAMES.has(name.toLowerCase());
const isSensitiveBodyKey = (name)=>SENSITIVE_BODY_KEY_REGEX.test(name);
...
const redactUrlQueryStringFallback = (rawUrl)=>rawUrl.replace(URL_QUERY_PARAM_REGEX, (match, prefix, name)=>{
...
const redactSensitiveBodyKeysDeep = (value)=>{
...
const redactJsonBodyString = (body)=>{
    if (body.length === 0) {
        return body;
    }
    let parsed;
...
const serializeBody = (body)=>{
    ...
    const redacted = typeof body === 'string' ? redactJsonBodyString(serialized) : serialized;
    const { value, originalBytes, truncated } = (0, _truncatestringtoutf8bytebudgetutil.truncateStringToUtf8ByteBudget)(redacted, MAX_BODY_BYTES);
    return {
        body: value,
...
const buildHttpRequestStepLog = ({ input, output, durationMs })=>{
    const requestBody = serializeBody(input.body);
    const responseBody = serializeBody(output.result);
    const hasResponseMetadata = typeof output.status === 'number' || output.headers !== undefined && Object.keys(output.headers).length > 0 || responseBody.body !== undefined;
...
//# sourceMappingURL=build-http-request-step-log.util.js.map