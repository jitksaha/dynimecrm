...
const findMatchingBranch = ({ branches, stepFilterGroups, resolvedFilters })=>{
    for (const branch of branches){
        if ((0, _utils.isDefined)(branch.filterGroupId) && !stepFilterGroups.some((group)=>group.id === branch.filterGroupId)) {
            throw new _workflowstepexecutorexception.WorkflowStepExecutorException(`Branch "${branch.id}" references filter group "${branch.filterGroupId}", which does not exist`, _workflowstepexecutorexception.WorkflowStepExecutorExceptionCode.INVALID_STEP_INPUT);
        }
    }
