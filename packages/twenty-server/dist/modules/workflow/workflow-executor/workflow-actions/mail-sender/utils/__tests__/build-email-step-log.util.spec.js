...
const _buildemailsteplogutil = require("../build-email-step-log.util");
const baseInput = {
...
describe('buildEmailStepLog', ()=>{
    it('builds a SUCCESS email log preferring parsed recipients from the tool output', ()=>{
        const stepLog = (0, _buildemailsteplogutil.buildEmailStepLog)({
            mode: 'SEND',
            input: baseInput,
    ...
    it('falls back to splitting the comma-separated input when the tool output has no parsed recipients', ()=>{
        ...
        expect(stepLog.details.recipients.cc).toEqual([
            'dan@example.com'
        ]);
...
//# sourceMappingURL=build-email-step-log.util.spec.js.map