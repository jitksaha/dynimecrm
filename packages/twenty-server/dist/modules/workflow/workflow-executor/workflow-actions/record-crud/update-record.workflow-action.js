...
let UpdateRecordWorkflowAction = class UpdateRecordWorkflowAction {
    async execute({ currentStepId, steps, context, runInfo }) {
        ...
        const { formattedRecord: formattedObjectRecord, joinColumnNamesByMorphFieldName } = (0, _formatworkflowrecordrelationfieldsutil.formatWorkflowRecordRelationFields)(workflowActionInput.objectRecord, objectMetadataInfo);
        ...
        const expandedFieldsToUpdate = workflowActionInput.fieldsToUpdate?.flatMap((fieldName)=>joinColumnNamesByMorphFieldName[fieldName] ?? [
                fieldName
            ]);
        const filteredFieldsToUpdate = expandedFieldsToUpdate?.filter((fieldName)=>fieldName in filteredObjectRecord);
