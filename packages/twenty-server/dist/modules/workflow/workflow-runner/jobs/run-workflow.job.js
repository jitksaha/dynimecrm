...
let RunWorkflowJob = class RunWorkflowJob {
    async handle({ workflowRunId, lastExecutedStepId, stepIdsToRetry, workspaceId }) {
        this.logger.log(`Running workflow run ${workflowRunId} in workspace ${workspaceId}`);
        const authContext = (0, _buildsystemauthcontextutil.buildSystemAuthContext)(workspaceId);
        await this.workspaceOrmManager.executeInWorkspaceContext(async ()=>{
            try {
                if ((0, _utils.isDefined)(stepIdsToRetry)) {
    ...
    async retryWorkflowExecution({ workflowRunId, stepIdsToRetry, workspaceId }) {
        const workflowRun = await this.workflowRunWorkspaceService.getWorkflowRunOrFail({
            workflowRunId,
            workspaceId
        });
    ...
    constructor(workflowCommonWorkspaceService, codeStepBuildService, workflowExecutorWorkspaceService, workflowRunWorkspaceService, metricsService, workspaceOrmManager){
