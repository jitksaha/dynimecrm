...
const _updateworkflowversionsteptool = require("../update-workflow-version-step.tool");
const mockStep = {
...
const buildTool = ({ validationResult = {
    valid: true,
    errors: [],
    warnings: []
} } = {})=>{
    ...
    const tool = (0, _updateworkflowversionsteptool.createUpdateWorkflowVersionStepTool)({
...
describe('createUpdateWorkflowVersionStepTool', ()=>{
    ...
    it('should validate by default and return a compact summary', async ()=>{
        ...
        expect(workflowValidationService.validateWorkflowVersion).toHaveBeenCalled();
        ...
        expect(validation.errors[0]).not.toHaveProperty('availablePaths');
    });
    it('should skip validation entirely when validate is false', async ()=>{
        ...
        expect(workflowValidationService.validateWorkflowVersion).not.toHaveBeenCalled();
        expect(result).not.toHaveProperty('validation');
    });
    it('should still return the step result when validation throws', async ()=>{
        const { tool, workflowValidationService } = buildTool();
        workflowValidationService.validateWorkflowVersion.mockRejectedValue(new Error('boom'));
        const result = await tool.execute(baseInput);
...
//# sourceMappingURL=update-workflow-version-step.tool.spec.js.map